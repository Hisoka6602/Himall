...
namespace Himall.OpenApi
{
    ...
    public class ProductHelper : _BaseHelper
    {
        ...
        {
            _ProductService = Himall.ServiceProvider.Instance<ProductService>.Create;
        ...
        public product_item_model GetProduct(int num_iid, string app_key)
        {
            ...
            long proid = num_iid;
            var prodata = _ProductService.GetProduct(proid);
        ...
        {
            long proid = num_iid;
            var product = _ProductService.GetProduct(proid);
            if (product == null)
            {
            ...
            {
                skudata = _ProductService.GetSku(sku_id);
                long oldstock = skudata.Stock;
                long nowstock = GetStockQuantity(oldstock, quantity, type);
                ProductManagerApplication.SetSkuStock(StockOptionType.Add, sku_id, nowstock - oldstock);
            }
            else
            {
                ...
                if (skus.Count > 0)
                {
                    ...
                    _ProductService.UpdateProduct(product);   //直接修改整个商品数据
                }
                else
                {
                    //不做操作
            ...
            //获取商品新数据
            product = _ProductService.GetProduct(proid);
                ...
                try
                {
                    switch (dataps)
                    {
                        case ProductStatus.In_Stock:
                            ...
                            _ProductService.SaleOff(num_iid, shopId);
                            break;
                        case ProductStatus.On_Sale:
                            _ProductService.OnSale(num_iid, shopId);
                            break;
                        case ProductStatus.Un_Sale:
                            _ProductService.SaleOff(num_iid, shopId);
                            break;
        ...
        private product_item_model ProductInfoMapChange(Entities.ProductInfo data)
        {
            ...
            var locpath = _RegionService.GetFullName(prolocid, ",");
