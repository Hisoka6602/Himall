...
namespace Himall.OpenApi
{
    ...
    public class UserHelper : _BaseHelper
    {
        ...
        public UserHelper()
        {
            ...
            _iMemberIntegralService = Himall.ServiceProvider.Instance<MemberIntegralService>.Create;
        ...
        public QueryPageModel<user_list_model> GetUsers(DateTime? start_time, DateTime? end_time, int page_no, int page_size)
        {
            ...
            //获取数据
            var datalist = _MemberService.GetMembers(userque);
            //转换数据
        ...
        {
            var data = _MemberService.GetMemberByName(user_name);
        ...
        {
            long result = 0;
            var user = _MemberService.Register(user_name, password, (int)PlatformType.PC, mobile, email, 0);
            ...
            {
                var dreg = _RegionService.GetRegionByName(district);
                if (dreg != null)
                {
            ...
            else
            {
                ...
                {
                    var creg = _RegionService.GetRegionByName(city);
                    if (creg != null)
                    {
                ...
                else
                {
                    ...
                    {
                        var sreg = _RegionService.GetRegionByName(state);
                        if (sreg != null)
                        {
            ...
            _MemberService.UpdateMemberInfo(user);
            int num = CouponApplication.RegisterSendCoupon(user.Id, user.UserName, out List<CouponModel> couponlist);
            result = user.Id;
            return result;
        }
