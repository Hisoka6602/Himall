...
namespace Himall.API
{
    public class GiftsController : BaseApiController
    {
        ...
        {
            _iGiftService = ObjectContainer.Current.Resolve<GiftService>();
            _iGiftsOrderService = ObjectContainer.Current.Resolve<GiftsOrderService>();
            _MemberService = ObjectContainer.Current.Resolve<MemberService>();
            _iMemberGradeService = ObjectContainer.Current.Resolve<MemberGradeService>();
        }
        ...
        {
            SlideAdsService _ISlideAdsService = ObjectContainer.Current.Resolve<SlideAdsService>();
        ...
        public List<IntegralMallAdInfo> GetIntegralMallAd()
        {
            ...
            if (robj != null)
            {
                robj.LinkUrl = SiteSettingApplication.GetCurDomainUrl() + "/m-app/BigWheel/index/" + robj.ActivityId;
                result.Add(robj);
            }
            var cobj = _iGiftService.GetAdInfo(IntegralMallAdInfo.AdActivityType.ScratchCard, IntegralMallAdInfo.AdShowPlatform.APP);
            if (cobj != null)
            {
                cobj.LinkUrl = SiteSettingApplication.GetCurDomainUrl() + "/m-app/ScratchCard/index/" + cobj.ActivityId;
                result.Add(cobj);
            }
            return result;
        }
        ...
        public Result SubmitOrder(GiftConfirmOrder value)
        {
            ...
            var curUser = _MemberService.GetMember(CurrentUser.Id);
            if (curUser == null)
            {
                isdataok = false;
                result.success = false;
        ...
        /// <param name="pagesize"></param>
        /// <returns></returns>
        public object GetMyOrderList(string skey = "", Himall.Entities.GiftOrderInfo.GiftOrderStatus? status = null, int page = 1, int pagesize = 10)
        {
            if (CurrentUser == null)
            {
