    ...
    public class OrderRefundController : BaseApiController
    {
        ...
        public object GetOrderRefundModel(long id, long? itemId = null, long? refundId = null)
        {
            ...
            try
            {
                ...
                var refundser = ServiceProvider.Instance<RefundService>.Create;
                ...
                if (order.OrderType == OrderInfo.OrderTypes.Virtual)
                {
                    ...
                    if (item.EnabledRefundAmount.HasValue)
                    {
                        ...
                        refundModel.MaxRefundAmount = refundModel.MaxRefundAmount > ((decimal)(item.EnabledRefundAmount - item.RefundPrice)) ? (decimal)(item.EnabledRefundAmount - item.RefundPrice) : refundModel.MaxRefundAmount;
                    }
                }
        ...
        public object PostRefundApply(OrderRefundApplyModel value)
        {
            ...
            try
            {
                ...
                if (order.OrderType != OrderInfo.OrderTypes.Virtual && info.ReturnQuantity < 0) throw new Himall.Core.HimallException("错误的退货数量");
                ...
                if (order.OrderType != OrderInfo.OrderTypes.Virtual)
                {
                    ...
                    {
                        if (order.OrderStatus != Entities.OrderInfo.OrderOperateStatus.WaitDelivery && order.OrderStatus != Entities.OrderInfo.OrderOperateStatus.WaitSelfPickUp) throw new Himall.Core.HimallException("错误的订单退款申请,订单状态有误");
                        info.IsReturn = false;
                        info.ReturnQuantity = 0;
        ...
        {
            CheckUserLogin();
            var orderser = ServiceProvider.Instance<OrderService>.Create;
            var refundser = ServiceProvider.Instance<RefundService>.Create;
            var vshopser = ServiceProvider.Instance<VShopService>.Create;
            ...
            var list = refunds.Models.Select(item =>
            {
                ...
                return new
                {
                    ...
                    Amount = item.Amount,
                    Img = orderItem.ThumbnailsUrl.Contains("skus") ? HimallIO.GetRomoteImagePath(orderItem.ThumbnailsUrl) : Core.HimallIO.GetRomoteProductSizeImage(orderItem.ThumbnailsUrl, 1, (int)Himall.CommonModel.ImageSize.Size_350),
                    ShopId = item.ShopId,
                    ...
                    {
                        ThumbnailsUrl = e.ThumbnailsUrl.Contains("skus") ? HimallIO.GetRomoteImagePath(e.ThumbnailsUrl) : Core.HimallIO.GetRomoteProductSizeImage(e.ThumbnailsUrl, 1, (int)Himall.CommonModel.ImageSize.Size_350),
        ...
        public object GetRefundDetail(long id)
        {
            ...
            d.IsOrderRefundTimeOut = _OrderService.IsRefundTimeOut(refundinfo.OrderId);
