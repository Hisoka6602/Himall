    ...
    public class ShopOrderRefundController : BaseShopLoginedApiController
    {
        ...
        private List<OrderRefundApiModel> FullModel(List<OrderRefund> refunds)
        {
            ...
            var result = refunds.Select(item =>
            {
                ...
                {
                    string pic = orderItem.ThumbnailsUrl.Contains("skus") ? HimallIO.GetRomoteImagePath(orderItem.ThumbnailsUrl) : Core.HimallIO.GetRomoteProductSizeImage(orderItem.ThumbnailsUrl, 1, (int)Himall.CommonModel.ImageSize.Size_100);
                    orderItem.ThumbnailsUrl = pic;
                    Entities.TypeInfo typeInfo = ServiceProvider.Instance<TypeService>.Create.GetTypeByProductId(orderItem.ProductId);
        ...
        public object PostReply(OrderRefundReplyModel reply)
        {
            ...
            if(shopAddress == null&& refund.RefundMode== OrderRefundInfo.OrderRefundMode.ReturnGoodsRefund)
