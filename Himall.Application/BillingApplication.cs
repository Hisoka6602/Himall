...
namespace Himall.Application
{
    public class BillingApplication
    {
        ...
        private static BillingService _iBillingService = ObjectContainer.Current.Resolve<BillingService>();
        private static MemberCapitalService _MemberCapitalService = ObjectContainer.Current.Resolve<MemberCapitalService>();
        ...
        public static OrderSettlementDetail GetPendingOrderSettlementDetail(long orderId, long? shopId = null)
        {
            ...
            detail.DiscountAmount = model.DiscountAmount - model.DiscountAmountReturn;
            return detail;
        }
        ...
        public static List<SettledOrders> GetAllSettlementOrders(SettlementOrderQuery query)
        {
            ...
            foreach (var m in models)
            {
                ...
                o.PaymentTypeName = m.PaymentTypeName;
                o.IntegralDiscount = m.IntegralDiscount;
                o.DiscountAmount = m.DiscountAmount;
                o.DiscountAmountReturn = m.DiscountAmountReturn;
                settledOrders.Add(o);
            }
        ...
        {
            var result= _iBillingService.StatisticsPendingSettlementOrders(query);
            var shopnamelist = ShopApplication.GetShopNames(result.Models.Select(p=>p.ShopId).ToList<long>());
            result.Models.ForEach(item=> {
                item.ShopName = shopnamelist[item.ShopId];
            });
            return result;
        }
        ...
        public static ChargePayModel PaymentList(long shopId, decimal balance, string webRoot)
        {
            ...
            Himall.Entities.ChargeDetailShopInfo model = new Himall.Entities.ChargeDetailShopInfo()
            {
            ...
            var models = payments.Select(item =>
            {
                ...
                try
                {
                    ...
                    if (item.PluginInfo.PluginId.ToLower().IndexOf("weixinpay_native") != -1)
                    {
                        requestUrl = requestUrl + "&orderIds=" + orderIds.ToString() + "&type=shopcharge";
                    }
                    #endregion
                }
                catch (Exception ex)
        ...
        public static void ShopRecharge(long Id, string TradNo, string ChargeWay)
        {
            ...
            if (model.ChargeStatus != Himall.Entities.ChargeDetailInfo.ChargeDetailStatus.ChargeSuccess)
            {
                model.ChargeStatus = Himall.Entities.ChargeDetailInfo.ChargeDetailStatus.ChargeSuccess;
        ...
        public static ShopWithDrawConfirmResult ShopApplyWithDraw(long Id, Himall.CommonModel.WithdrawStaus status, string Remark, string IpAddress = "", string UserName = "", string webRoot = "")
        {
            ...
            else if (model.CashType == Himall.CommonModel.WithdrawType.ALipay)//支付宝
            {
                try
                {
                    ...
                    _iBillingService.UpdateShopWithDraw(model);
                    //资金处理
                    UpdateAccount(model.ShopId, model.CashAmount, Himall.CommonModel.ShopAccountType.WithDraw, payresult.TradNo, "支付宝提现", Id);
        ...
        /// <returns></returns>
        public static List<ShopWithDrawConfirmResult> ShopApplyWithDrawBatch(string ids, Himall.CommonModel.WithdrawStaus status, string Remark, string IpAddress = "", string UserName = "", string webRoot = "")
        {
            ...
            foreach (var model in modelList)
            {
                ...
                else if (model.CashType == Himall.CommonModel.WithdrawType.BankCard)//银行卡
                {
                    ...
                    _iBillingService.UpdateShopWithDraw(model);
                    //资金处理
                    UpdateAccount(model.ShopId, model.CashAmount, Himall.CommonModel.ShopAccountType.WithDraw, DateTime.Now.ToString("yyyyMMddHHmmssffff"), "银行卡提现", model.Id);
                ...
                else if (model.CashType == Himall.CommonModel.WithdrawType.ALipay)//支付宝
                {
                    try
                    {
                        ...
                        _iBillingService.UpdateShopWithDraw(model);
                        //资金处理
                        UpdateAccount(model.ShopId, model.CashAmount, Himall.CommonModel.ShopAccountType.WithDraw, payresult.TradNo, "支付宝提现", model.Id);
                        //操作日志
                        OperationLogApplication.AddPlatformOperationLog(new Entities.LogInfo
                        {
                            Date = DateTime.Now,
                ...
                else if (model.CashType == Himall.CommonModel.WithdrawType.WeiChat)//微信
                {
                    ...
                    if (plugins != null)
                    {
                        try
                        {
                            ...
                            _iBillingService.UpdateShopWithDraw(model);
                            //资金处理
                            UpdateAccount(model.ShopId, model.CashAmount, Himall.CommonModel.ShopAccountType.WithDraw, payresult.TradNo, "微信提现", model.Id);
        ...
        public static QueryPageModel<ShopWithDrawItem> GetShopWithDraw(WithdrawQuery query)
        {
            ...
            foreach (ShopWithDrawInfo mInfo in model.Models)
            {
                ...
                swdi.Id = mInfo.Id;
                swdi.Account = mInfo.Account+"<br/>"+mInfo.BankName;
        ...
        //    {
        //        var model = _MemberCapitalService.GetApplyWithDrawInfo(Convert.ToInt64(ids[i]));
        //        batch_fee += amount[i];
        //        outPayId[i] = DateTime.Now.ToString("yyyyMMddHHmmssfff") + i.ToString();
        //        //更新 PayNo
        //        _MemberCapitalService.UpdateApplyWithDrawInfoPayNo(model.Id, outPayId[i]);
        //        if (i != 0)
        //        {
        //            detail_data += "|";
        //        }
