...
{
    public class CartApplication : BaseApplicaion<CartService>
    {
        private static BranchCartService _iBranchCartService = ObjectContainer.Current.Resolve<BranchCartService>();
        /// <summary>
        /// 添加商品至购物车
        /// </summary>
        ...
        public static void AddToCart(string skuId, int count, long memberId,long roomId)
        {
            Service.AddToCart(skuId, count, memberId,roomId);
        }
        ...
        /// <param name="roomId">直播间ID</param>
        public static void UpdateShopBranchCart(string skuId, int count, long memberId, long shopbranchId, long roomId = 0)
        {
            _iBranchCartService.UpdateCart(skuId, count, memberId, shopbranchId, roomId);
        }
        /// <summary>
        /// 取会员门店购物车项
        /// </summary>
        ...
        private static ShoppingCartInfo GetShoppingCart(long member)
        {
            ...
            string carts = WebHelper.GetCookie("HIMALL-CART");
        ...
        public static List<CartItem> GetCartItems(long memberId)
        {
            ...
            var skus = GetService<ProductService>().GetSKUs(skuIds);
            var products = GetService<ProductService>().GetProducts(skus.Select(p => p.ProductId).ToList());
            var shops = GetService<ShopService>().GetShops(products.Select(p => p.ShopId).ToList());
            ...
            //阶梯价商品
            var allLadders = GetService<ProductLadderPriceService>().GetLadderPricesByProductIds(products.Where(p => p.IsOpenLadder).Select(p => p.Id).ToList());
