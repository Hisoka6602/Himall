...
{
    public class FreightTemplateApplication : BaseApplicaion<FreightTemplateService>
    {
        ...
        public static void AddOrUpdateFreightTemplate(FreightTemplate templateInfo)
        {
            ...
            if (templateInfo.FreeTempContent != null)
            {
                ...
                {
                    var info = new Himall.Entities.ShippingFreeGroupInfo()
                    {
                        ConditionNumber = p.ConditionNumber,
                        ConditionType = p.ConditionType
                    };
                    info.ShippingFreeRegionInfo = new List<Himall.Entities.ShippingFreeRegionInfo>();
                    p.FreightAreaDetail.ForEach(a =>
                    {
                        info.ShippingFreeRegionInfo.Add(new Himall.Entities.ShippingFreeRegionInfo()
                        {
        ...
        /// <param name="productType">商品类型(0实物，1虚拟商品)</param>
        /// <param name="buynumber">购买件数</param>
        /// <returns></returns>
        public static string GetFreightStr(long productId, FreightTemplateInfo freightTemplate, long memberId, sbyte productType = -1, int buynumber = 1)
        {
            ...
            //是否虚拟商品，如是虚拟商品免运费
            if (productType == 1)
                return freightStr; //虚拟商品无运费

            int addressId = 0;
            if (memberId > 0)
            {
                var addressInfo = ShippingAddressApplication.GetAddress(memberId).FirstOrDefault(p => p.IsDefault);
                if (addressInfo != null)
                {
            ...
            ProductService productService = GetService<ProductService>();
