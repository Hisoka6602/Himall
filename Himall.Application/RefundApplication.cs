...
{
    public class RefundApplication:BaseApplicaion<RefundService>
    {
        ...
        /// 虚拟订单自动退款
        /// </summary>
        public static void AutoVirtualRefund()
        {
            var result = Service.AutoVirtualRefund();
            foreach (var item in result)
                MessageApplication.SendMessageOnRefundApply(item.UserId, item.OrderId, item.OrderItemId, (int)OrderRefundInfo.OrderRefundMode.OrderRefund, item.RefundId);
        }
        ...
        {
            var timeout = SiteSettingApplication.SiteSettings.AS_ShopConfirmTimeout;
            var exprieTime = DateTime.Now.AddDays(-timeout);
            var refunds = Service.GetAuditTimeout(exprieTime);
            foreach (var item in refunds)
                ...
                {
                    Log.Error($"AutoAuditRefund:{item},{ex.Message}", ex);
                }
        }
        /// <summary>
        /// 自动关闭寄货超时退款单
        ...
        {
            var timeout = SiteSettingApplication.SiteSettings.AS_SendGoodsCloseTimeout;
            var exprieTime = DateTime.Now.AddDays(-timeout);
            var refunds = Service.GetDeliverTimeOut(exprieTime);
            foreach (var item in refunds)
            {
                try
                {
                ...
                {
                    Log.Error($"AutoCloseByDeliveryExpired{item},{ ex.Message}", ex);
                }
            }
        }
        /// <summary>
        ...
        {
            var timeout = SiteSettingApplication.SiteSettings.AS_ShopNoReceivingTimeout;
            var exprieTime = DateTime.Now.AddDays(-timeout);
            var refunds = Service.GetConfirmTimeOut(exprieTime);
            foreach (var item in refunds)
            {
                try
                {
                ...
                {
                    Log.Error($"AutoShopConfirmArrival{item},{ ex.Message}", ex);
                }
            }
        }
        ...
        public static List<OrderRefundExportModel> GetAllFullOrderReFunds(RefundQuery refundQuery)
        {
            ...
            var shopbranchs = ShopBranchApplication.GetStores(orders.Select(p => p.ShopBranchId).Distinct().ToList());
            ...
            foreach (var item in data)
            {
                ...
                if (order != null && order.ShopBranchId > 0 && shopbranchs!=null)
                {
                    var shopBranchInfo = shopbranchs.Where(t => t.Id == order.ShopBranchId).FirstOrDefault();
                ...
                var refundModel = new OrderRefundExportModel()
                {
                    ...
                    ManagerConfirmDate = item.ManagerConfirmStatus == OrderRefundInfo.OrderRefundConfirmStatus.Confirmed ? item.ManagerConfirmDate.ToString() : string.Empty,//退款成功有退款时间
        ...
        //{
        //    _RefundService.AutoAuditRefund();
        //}
        ///// <summary>
        ///// 自动关闭过期未寄货退款(job)
        ///// </summary>
        //public static void AutoCloseByDeliveryExpired()
        //{
        //    _RefundService.AutoCloseByDeliveryExpired();
        //}
        ///// <summary>
        ///// 自动商家确认到货(job)
        ///// </summary>
        //public static void AutoShopConfirmArrival()
        //{
        //    _RefundService.AutoShopConfirmArrival();
        //}
    }
}
