...
{
    public class ShopBonusApplication:BaseApplicaion<ShopBonusService>
    {
        /// <summary>
        ///  获取红包列表
        /// </summary>
        ...
        public static ShopBonusInfo GetShopBonus(long Id) {
        ...
        /// 订单支付完成时，生成红包详情
        /// </summary>
        public static long GenerateBonusDetail(ShopBonusInfo model, long orderid, string receiveurl,decimal ordermoney)
        {
            return Service.GenerateBonusDetail(model, orderid, receiveurl, ordermoney);
        }
        ...
        public static List<ShopBonus> BuildShopBonus(List<ShopBonusReceiveInfo> receive)
        {
            ...
            var shops = GetService<ShopService>().GetShops(bouns.Select(p => p.ShopId));
            var vshops = GetService<VShopService>().GetVShopsByShopIds(shops.Select(p => p.Id));
        ...
        public static ShopBonusData GetAvailableBonusByShop(long shopId) =>
            Service.GetAvailableBonus().FirstOrDefault(p => p.ShopId == shopId);
    }
}
