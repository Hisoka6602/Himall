...
namespace Himall.Application
{
    ...
    /// 门店应用服务
    /// </summary>
    public class ShopBranchApplication : BaseApplicaion<ShopBranchService>
    {
        private static AppMessageService _appMessageService = ObjectContainer.Current.Resolve<AppMessageService>();
        private static CouponService _CouponService = ObjectContainer.Current.Resolve<CouponService>();
        ...
        public static ShopBranchData GetStores(long id) =>
            Service.GetShopBranchData(id);
        public static List<ShopBranchData> GetStores(List<long> list) =>
            Service.GetShopBranchData(list);
        ...
        public static QueryPageModel<ShopBranch> GetShopBranchs(ShopBranchQuery query)
        {
            ...
            var shops = GetService<ShopService>().GetShops(branchs.Select(p => p.ShopId).ToList());
        ...
        /// <returns></returns>
        public static List<DTO.SKU> GetSkusByProductIds(long shopBranchId, IEnumerable<long> pids)
        {
            var sku = ProductManagerApplication.GetSKUByProducts(pids);
            var shopBranchSkus = Service.GetSkusByIds(shopBranchId, sku.Select(e => e.Id).ToList());
            foreach (var item in sku)
            {
                var branchSku = shopBranchSkus.FirstOrDefault(e => e.SkuId == item.Id);
        ...
        public static QueryPageModel<ProductInfo> GetShopBranchProducts(ShopBranchProductQuery query)
        {
            ...
            var saleCount = Service.GetProductSaleCount(query.ShopBranchId, pids, null, null);
        ...
        /// <param name="query"></param>
        /// <returns></returns>
        public static QueryPageModel<ShopBranchProduct> GetShopBranchProductList(ShopBranchProductQuery query)
        {
            var pageModel = Service.GetShopBranchProductList(query);

            if (pageModel.Total > 0)
            {
                var productids = pageModel.Models.Select(t => t.ProductId).ToList();

                var products = ProductManagerApplication.GetAllProductByIds(productids);
                var shopcategorys = ShopCategoryApplication.GetCategorysByProduct(productids);
                var skus = ProductManagerApplication.GetSKUsByProduct(productids);
                var prosaleCount = Service.GetProductSaleCount(query.ShopBranchId, productids, null, null);

                pageModel.Models.ForEach(item =>
                {
                    var profirst = products.Where(t => t.Id == item.ProductId).FirstOrDefault();
                    var shopcategorylfrst = shopcategorys.Where(t => t.ProductId == item.ProductId).FirstOrDefault();
                    var prosku = skus.Where(t => t.ProductId == item.ProductId);

                    item.CategoryId = profirst != null ? profirst.CategoryId : 0;
                    item.CategoryId = profirst != null ? profirst.CategoryId : 0;
                    item.ProductName = profirst != null ? profirst.ProductName : string.Empty;
                    item.UpdateTime = profirst != null ? profirst.UpdateTime : DateTime.MinValue;
                    item.ProductType = profirst != null ? profirst.ProductType : (sbyte)0;
                    item.Image = profirst != null ? profirst.GetImage(ImageSize.Size_50) : string.Empty;
                    item.SalePrice = profirst != null ? profirst.MinSalePrice : 0;

                    item.ShopCategoryId = shopcategorylfrst != null ? shopcategorylfrst.ShopCategoryId : 0;
                    item.ShopCategoryName = shopcategorylfrst != null ? shopcategorylfrst.ShopCategoryName : string.Empty;
                    item.MinSalePrice = prosku != null ? prosku.Min(t => t.SalePrice) : 0;
                    item.MaxSalePrice = prosku != null ? prosku.Max(t => t.SalePrice) : 0;

                    item.SaleCount = (prosaleCount != null && prosaleCount.ContainsKey(item.ProductId)) ? prosaleCount[item.ProductId] : 0;//商品销售
                });
            }
        ...
        {
            query.IsSelectShopBranchProductSaleCount = true;//要查下门店销量统计
            var pageModel = Service.SearchProduct(query);
            return pageModel;
        }
        ...
        public static bool CheckShopCategoryProductIsExist(long shopId, long shopBranchId, long shopCategoryId)
        {
            return Service.CheckShopCategoryProductIsExist(shopId, shopBranchId, shopCategoryId);
        }
        ...
        /// <param name="query"></param>
        /// <returns></returns>
        public static bool IsExstsAboveSelf(ShopBranchQuery query)
        {
            var shopbranches=Service.GetAllShopBranchs(query).Where(shop=>shop.IsAboveSelf==true);
            return shopbranches != null && shopbranches.Count() > 0;
        }
        ...
        public static void CorrectBranchProductSkus(ProductInfo product, long shopId)
        {
            ...
            Service.SetClearNotExistBranchSku(product.Id); //商品规格编辑变动，先清除该门店已不存在的规格

            var allSkus = ProductManagerApplication.GetSKUsByProductId(product.Id);
            var shopBranchSkus = Service.GetShopBranchSkusByProductId(product.Id);
            if(shopBranchSkus!=null && shopBranchSkus.Count > 0)
            {
                var addBranchSkus = new List<Entities.ShopBranchSkuInfo>();
                foreach (var itemId in shopBranchSkus.Select(t => t.ShopBranchId).Distinct())
                {
                    var oldskus = shopBranchSkus.Where(t => t.ShopBranchId == itemId).Select(t=>t.SkuId);//它原先规格
                    ...
                    if (skus != null)
                        addBranchSkus.AddRange(skus);
                }

                if (addBranchSkus != null && addBranchSkus.Count > 0)
                    Service.AddSkus(addBranchSkus);
            }
        }
        ...
        /// <param name="ids"></param>
        /// <returns></returns>
        public static void RecommendShopBranch(long[] ids)
        {
            Service.RecommendShopBranch(ids);
        }
        ...
        /// <returns></returns>
        public static void RecommendChangeSequence(long oriShopBranchId, long newShopBranchId)
        {
             Service.RecommendChangeSequence(oriShopBranchId, newShopBranchId);
        }
        ...
        /// <returns></returns>
        public static void ResetShopBranchRecommend(long shopBranchId)
        {
            Service.ResetShopBranchRecommend(shopBranchId);
        }
        ...
        {
            if (id <= 0) throw new HimallException("修改目标标签不可为空");
            if (string.IsNullOrEmpty(title)) throw new HimallException("标签名称不可为空");
        ...
        /// <param name="Id"></param>
        /// <returns></returns>
        public static QueryPageModel<ShopBranchTagInfo> GetShopBranchTags(int pageNo = 1, int pageSize = 10, string title = "")
        {
            return Service.GetShopBranchTags(pageNo, pageSize, title);
        }
        ...
        public static int CouponIsUse(Entities.CouponInfo couponinfo, long userId)
        {
            ...
            {//检验当前会员是否可领
                ...
                QueryPageModel<Entities.CouponRecordInfo> pageModel = _CouponService.GetCouponRecordList(crQuery);
