...
namespace Himall.Application
{
    ...
    {
        private static StatisticsService _StatisticsService = ObjectContainer.Current.Resolve<StatisticsService>();
        ...
        public static void SettlementPayProduct(DateTime date)
        {
            _StatisticsService.SettlementPayProduct(date, date.AddDays(1));
        }

        public static void InitShopVisit()
        {
            _StatisticsService.InitShopVisit();
        }
        ...
        public static void StatisticPlatVisitUserCount()
        {
            ...
            Task.Factory.StartNew(() =>
            {
                lock (_platVisitLocker)
                {
                    ...
                    {
                        _StatisticsService.AddPlatVisitUser(nowDate, 1);
                        count = 0;
                    }
        ...
        public static void StatisticShopVisitUserCount(long shopId)
        {
            ...
            StatisticPlatVisitUserCount();//小程序端、app端不好整体执行平台统计，则在店铺统计时也还执行下平台统计
        }
        ...
        {
            var cookieKey = CommonConst.HIMALL_SHOPBRANCH_VISIT_COUNT(shopBranchId.ToString());
            var shopVisit = WebHelper.GetCookie(cookieKey);
            var nowTicks = DateTime.Now.Date.Ticks;
            if (!string.IsNullOrWhiteSpace(shopVisit))
            {
            ...
            Task.Factory.StartNew(() =>
            {
                ...
                {
                    string key = CommonConst.HIMALL_SHOPBRANCH_VISIT_COUNT(shopBranchId.ToString());
                    int count = 1;
                    if (Core.Cache.Exists(key))
                    {
                        count = Core.Cache.Get<int>(key);
                    ...
                    if (count >= CommonConst.HIMALL_SHOPBRANCH_VISIT_COUNT_MAX)
                    {
        ...
        {
            return GetService<OrderService>().GetOrderCountStatistics(query);
        }
        /// <summary>
        /// 获取平台数据汇总
        /// </summary>
        ...
        public static SellerConsoleModel GetSellerConsoleModel(long shopId)
        {
            ...
            var spaceUsage = GetService<ShopService>().GetShopUsageSpace(shop.Id);
        ...
        public static OrderCountStatistics GetMemberOrderStatistic(long userId, bool isOngoing = false,bool? isVirtual=null)
        {
            ...
            //待发货订单 减掉拼团订单
            //var fight = GetService<OrderService>().GetFightGroupOrderCountByUser(userId);
        ...
        public static void SettlementOrder(DateTime date)
        {
            _StatisticsService .SettlementOrder(date, date.AddDays(1));
        }
        #endregion
