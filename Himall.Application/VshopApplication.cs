...
{
    public class VshopApplication:BaseApplicaion<VShopService>
    {
        /// <summary>
        /// 根据条件查询所有微店
        /// </summary>
        ...
        /// <param name="UseArea"></param>
        /// <returns></returns>
        public static List<VShop> GetVshopsByCouponInfo(long shopId, long coupId, int UseArea)
        {
            List<VShop> vshoplist = new List<VShop>();
            //当平台券的userArea为1时，代表是指定商家券
            if (shopId == 0 && UseArea == 1)
            {
                var shopIds = CouponApplication.GetCouponShopsByCouponId(coupId).Select(c => c.ShopId).ToList();
                List<VShop> vshops = GetVShopsByShopIds(shopIds);
                if (vshops != null && vshops.Count > 0)
                {
                    vshoplist.AddRange(vshops);
                }
            }
            else
            {
                ...
                shopIds.Add(shopId);
                var vshop = GetVShopsByShopIds(shopIds);
                if (vshop != null && vshop.Count > 0)
                {
                    vshoplist.AddRange(vshop);
                }
            }
            if (vshoplist == null)
            {
                vshoplist = new List<VShop>();
            }
            return vshoplist;
        }
    }
