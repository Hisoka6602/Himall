...
namespace Himall.Entities
{
    public partial class ProductInfo
    {
        ...
        {
            string imagePath = Core.HimallIO.GetProductSizeImage(ImagePath, imageIndex, (int)imageSize);
            if (imagePath.IndexOf("?") > -1)
            {
                return imagePath += "&r=" + UpdateTime.ToString("yyyyMMddHHmmss");
            }
            else
            {
                return imagePath += "?r=" + UpdateTime.ToString("yyyyMMddHHmmss");
