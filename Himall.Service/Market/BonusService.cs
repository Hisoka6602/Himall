...
{
    public class BonusService : ServiceBase
    {
        ...
        public List<GeneralRecord> GetMemberBonus(long memberId, List<long> choiceBouns = null)
        {
            var db = DbFactory.Default.Get<ShopBonusReceiveInfo>()
                .LeftJoin<ShopBonusGrantInfo>((r, g) => r.BonusGrantId == g.Id)
                .LeftJoin<ShopBonusInfo, ShopBonusGrantInfo>((s, g) => s.Id == g.ShopBonusId)
                .Where(p => p.UserId == memberId && p.State == ShopBonusReceiveInfo.ReceiveState.NotUse);
            if (choiceBouns != null)
            {
                if (choiceBouns.Count == 0)
                    return new List<GeneralRecord>();
                db.Where(p => p.Id.ExIn(choiceBouns));
            }
            var result = db.Select(p => new
            {
                p.Id,
                Amount = p.Price
            }).Select<ShopBonusInfo>(p => new
            {
                MarketId = p.Id,
                ShopId = p.ShopId,
                Title = p.Name,
                StartTime = p.BonusDateStart,
                EndTime = p.BonusDateEnd,
                OrderAmount = p.UsrStatePrice
            })
           .ToList<GeneralRecord>();
            result.ForEach(item =>
            {
                item.RecordType = GeneralRecordType.Bouns;
            });
            return result;
        }
            ...
            if (obj != null)
            {
                ...
                DbFactory.Default
                    .InTransaction(() =>
                    {
                        ...
                        if (!flag) return false;
                        //Log.Error("--4--flag:" + flag+ "--openId:"+ openId+ "--ReceivePrice：" + bonus.ReceivePrice);
                        var ynbon = DbFactory.Default.Update(obj) > 0;
                        //Log.Error("--4--ynbon:" + ynbon);
                        return ynbon;
                    });
                //if(selfIsAttention )
                //{
                ...
                //    {
                //        Himall.ServiceProvider.Instance<WXApiService>.Create.Subscribe( openId );
                //    } );
                //}
        ...
        public void GetSuccessSendWXMessage(BonusReceiveInfo data, string openId)
        {
            ...
            //        //Himall.ServiceProvider.Instance<WXApiService>.Create.SendMessageByTemplate(siteSetting.WeixinAppId, siteSetting.WeixinAppSecret, openId, wxmsgtmpl.TemplateId, "#000000", url, msgdata);
            //    }
            //}
            #endregion
        }
        ...
        /// 生成红包详情
        /// </summary> 
        private void GenerateBonusDetail(Himall.Entities.BonusInfo model)
        {
        ...
        private bool IsAttentionByRPC(string openId)
        {
            ...
            {
                accessToken = new WXApiService().TryGetToken(siteSetting.WeixinAppId, siteSetting.WeixinAppSecret);
            }
            else
            {
        ...
        private void DepositToMember(string openId, decimal price, string unionId = "")
        {
            ...
            if (model != null)
            {
                ...
                {
                    UserId = model.UserId,
                    SourceType = Himall.Entities.CapitalDetailInfo.CapitalDetailType.RedPacket,
                    Amount = price,
        ...
        {
            //获取某个OpenId对应的红包记录(在himall3.3版本，考虑到小程序首页配置了现金红包跳转到红包页面的情况，将之前的openId改为UnionId，并兼容以前的数据)
