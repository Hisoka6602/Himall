...
{
    public class DistributionService : ServiceBase
    {
        ...
        public void AddSpreadProducts(IEnumerable<long> productIds, long shopId)
        {
            ...
            if (dbrokerage == null)
            {
                throw new HimallException("请先设置商家默认分佣比");
            }
        ...
        private GetBuilder<DistributionBrokerageInfo> GetBrokerageProductBuilder(BrokerageProductQuery query)
        {
            ...
            db.Select(p => new
            {
                ...
                Settlement = "sum(case when BrokerageStatus = 1 then RealPayAmount*(Himall_DistributionBrokerage.BrokerageRate1+Himall_DistributionBrokerage.BrokerageRate2+Himall_DistributionBrokerage.BrokerageRate3)/100 else 0 end)".ExSql(),
                NoSettlement = "sum(case when BrokerageStatus = 0 then RealPayAmount*(Himall_DistributionBrokerage.BrokerageRate1+Himall_DistributionBrokerage.BrokerageRate2+Himall_DistributionBrokerage.BrokerageRate3)/100 else 0 end)".ExSql()
            })
        ...
        private void SetDistributorsInfo(List<DistributorInfo> result)
        {
            ...
            {
                var csql = @"select MemberId,d.SuperiorId as SuperiorId,d.SubNumber as SubNumber,g.SubNumber2 as SubNumber2 from Himall_Distributor as d
                        LEFT JOIN(
                        SELECT SuperiorId, sum(SubNumber) SubNumber2
                        FROM Himall_Distributor
                        WHERE SuperiorId IN (SELECT MemberId FROM `Himall_Distributor` WHERE SuperiorId in ({sids}))
                        GROUP BY SuperiorId
                        ) as g
        ...
        private GetBuilder<DistributorInfo> GetNewDistributorsBuilder(DistributorSubQuery query)
        {
            ...
            {
                var noSettlementSql = "(select sum(ROUND(case when SuperiorId1 = Himall_Distributor.MemberId then brokeragerate1 when SuperiorId2 = Himall_Distributor.MemberId then brokeragerate2 when SuperiorId3 = Himall_Distributor.MemberId then brokeragerate3  else 0 end * RealPayAmount/100,2)) from Himall_DistributionBrokerage where BrokerageStatus = 0)";
        ...
        public DistributionProduct GetProduct(long productId)
        {
            ...
            result.MaxDistributionLevel = Himall.ServiceProvider.Instance<SiteSettingService>.Create.SiteSettings.DistributionMaxLevel;
            #endregion
...
ROUND(`Himall_DistributionBrokerage`.`BrokerageRate1` * `Himall_DistributionBrokerage`.`RealPayAmount` / 100,2)
+ROUND(`Himall_DistributionBrokerage`.`BrokerageRate2` * `Himall_DistributionBrokerage`.`RealPayAmount` / 100,2)
+ROUND(`Himall_DistributionBrokerage`.`BrokerageRate3` * `Himall_DistributionBrokerage`.`RealPayAmount` / 100,2)) AS NoResult
            FROM
				`Himall_DistributionBrokerage`
			WHERE
                (
						`Himall_DistributionBrokerage`.`ProductId` = `Himall_DistributionProduct`.`ProductId`  
						AND `Himall_DistributionBrokerage`.`BrokerageStatus` = 0
				)
		),0)".ExSql(),
            });
            db.Select<ProductInfo>(d => new
        ...
        public QueryPageModel<DistributionProduct> GetProducts(DistributionProductQuery query)
        {
            ...
            foreach (var item in result)
            {
                ...
                item.MaxDistributionLevel = Himall.ServiceProvider.Instance<SiteSettingService>.Create.SiteSettings.DistributionMaxLevel;
            }
            #endregion
        ...
        public List<CategoryInfo> GetHaveDistributionProductTopCategory()
        {
            ...
            var result = DbFactory.Default.Get<CategoryInfo>()
        ...
        public void SetBrokerageStatus(long orderId, DistributionBrokerageStatus status, long? orderItemId = null)
        {
            ...
            {
                decimal realPay = item.RealTotalPrice - item.RefundPrice;
        ...
        public void TreatedOrderDistribution(long memberId, List<long> orders)
        {
            var links = GetLinks(memberId);
            if (links.Count == 0)
            {
                Log.Info("分销日志：无分销关系");
                return;//无分销关系
            ...
            var items = DbFactory.Default.Get<OrderItemInfo>().Where(d => d.OrderId.ExIn(orders)).ToList();
            var list = items.Select(p => p.ProductId).Distinct().ToList();

            var products = DbFactory.Default.Get<DistributionProductInfo>(p => p.ProductId.ExIn(list) && p.ProductStatus == (long)DistributionProductStatus.Normal).ToList();
            if (products.Count == 0)
            {
                Log.Info("分销日志：无分销商品,订单商品ID：" + string.Join(",", list));
                return;//无分销商品
            }
            var brokerages = new List<DistributionBrokerageInfo>();
            foreach (var item in items)
            {
                var product = products.FirstOrDefault(p => p.ProductId == item.ProductId);
                if (product == null)
                    continue;
                var brokerage = new DistributionBrokerageInfo
                {
                    ...
                    ShopId = item.ShopId,
                    BrokerageStatus = DistributionBrokerageStatus.NotAvailable,
                };
                if (links.Count > 0)
                {
                    brokerage.SuperiorId1 = links[0];
                    brokerage.BrokerageRate1 = product.BrokerageRate1;
                }
                if (links.Count > 1)
                {
                    brokerage.SuperiorId2 = links[1];
                    brokerage.BrokerageRate2 = product.BrokerageRate2;
                }
                if (links.Count > 2)
                {
                    brokerage.SuperiorId3 = links[2];
                    brokerage.BrokerageRate3 = product.BrokerageRate3;
                }
                brokerages.Add(brokerage);
            }
            DbFactory.Default.Add(brokerages);
        }
        public void TreatedOrderPaidDistribution(long orderId)
        {
            var brokerages = DbFactory.Default.Get<DistributionBrokerageInfo>().Where(d => d.OrderId == orderId).ToList(); ;
            if (brokerages.Count == 0)
                return;

            var orderitems = DbFactory.Default.Get<OrderItemInfo>(p => p.OrderId == orderId).ToList();
            var obj = brokerages.FirstOrDefault();
            var buyMember = obj.MemberId;
            var links = new List<long> { obj.SuperiorId1, obj.SuperiorId2, obj.SuperiorId3 }.Where(p => p > 0).ToList();
            var quantity = brokerages.Sum(p => p.Quantity);
            var amount = 0M;

            DbFactory.Default.InTransaction(() =>
            {
                DbFactory.Default.Set<DistributionBrokerageInfo>()
                    .Where(p => p.OrderId == orderId)
                    .Set(p => p.BrokerageStatus, DistributionBrokerageStatus.NotSettled)
                    .Execute();

                foreach (var brokerage in brokerages)
                {
                    var item = orderitems.FirstOrDefault(p => p.Id == brokerage.OrderItemId);
                    DbFactory.Default.Set<DistributionBrokerageInfo>()
                    .Where(p => p.Id == brokerage.Id)
                        .Set(p => p.RealPayAmount, item.RealTotalPrice)
                        .Execute();
                    amount += item.RealTotalPrice;
                    DbFactory.Default.Set<DistributionProductInfo>()
                        .Where(p => p.ProductId == brokerage.ProductId)
                        .Set(p => p.SaleCount, p => p.SaleCount + brokerage.Quantity)
                        .Set(p => p.SaleAmount, p => p.SaleAmount + item.RealTotalPrice)
                        .Execute();
                }

                DbFactory.Default.Set<DistributorInfo>()
                    .Where(p => p.MemberId == buyMember)
                    .Set(p => p.SaleAmount, p => p.SaleAmount + amount)
                    .Set(p => p.ProductCount, p => p.ProductCount + quantity)
                    .Execute();
                DbFactory.Default.Set<DistributorInfo>()
                    .Where(p => p.MemberId.ExIn(links))
                    .Set(p => p.SubProductCount, p => p.SubProductCount + quantity)
                    .Set(p => p.SubSaleAmount, p => p.SubSaleAmount + amount)
                    .Execute();
            });
        }
        ...
        private List<long> GetLinks(long member)
        {
            var setting = ServiceProvider.Instance<SiteSettingService>.Create.SiteSettings;
            var level = setting.DistributionMaxLevel;
            var result = new List<long>();
            var distributor = DbFactory.Default.Get<DistributorInfo>().Where(d => d.MemberId == member).FirstOrDefault();
            if (distributor != null && distributor.DistributionStatus == (int)DistributorStatus.Audited && setting.DistributionCanSelfBuy)
            {
                result.Add(distributor.MemberId);//允许自购
            }
            if (result.Count >= level || distributor == null || distributor.SuperiorId == 0)
                return result;

            //一级
            distributor = DbFactory.Default.Get<DistributorInfo>().Where(d => d.MemberId == distributor.SuperiorId).FirstOrDefault();
            if (distributor == null && distributor.DistributionStatus != (int)DistributorStatus.Audited)
            {
                Log.Info("分销日志：上级分销员状态不正确");
                return result;//非分销员
            }
            result.Add(distributor.MemberId);
            if (result.Count >= level || distributor.SuperiorId == 0)
                return result;
        ...
        public void RefusedWithdraw(long withdrawId, string operatorName, string remark)
        {
            ...
            //发送消息
            var member = Himall.ServiceProvider.Instance<MemberService>.Create.GetMember(model.MemberId);
            ...
            Task.Factory.StartNew(() => ServiceProvider.Instance<MessageService>.Create.SendMessageOnMemberWithDrawFail(model.MemberId, message));
        }
        #endregion
        ...
        /// 获取待结算订单
        /// </summary>
        private List<DistributionBrokerageModel> GetWaitSettlement(DateTime settlementTime) =>
            DbFactory.Default.Get<DistributionBrokerageInfo>()
                    .LeftJoin<OrderInfo>((a, b) => a.OrderId == b.Id)
                    .Where(n => n.BrokerageStatus == DistributionBrokerageStatus.NotSettled)
                    .Where<OrderInfo>(n =>
                    (n.FinishDate < settlementTime && n.OrderStatus == OrderInfo.OrderOperateStatus.Finish && n.OrderType != OrderInfo.OrderTypes.Virtual)
                    || (n.OrderStatus == OrderInfo.OrderOperateStatus.Finish && n.OrderType == OrderInfo.OrderTypes.Virtual)
                    || n.OrderStatus == OrderInfo.OrderOperateStatus.Close
                    )
                    .Select(a => new
                    {
                        Id = a.Id,
                        ...
                        BrokerageStatus = a.BrokerageStatus,
                        BrokerageRate1 = a.BrokerageRate1,
                        BrokerageRate2 = a.BrokerageRate2,
                        BrokerageRate3 = a.BrokerageRate3,
                        MemberId = a.MemberId,
                        OrderId = a.OrderId,
                        ...
                        ShopId = a.ShopId,
                        SuperiorId1 = a.SuperiorId1,
                        SuperiorId2 = a.SuperiorId2,
                        SuperiorId3 = a.SuperiorId3
                    })
                    .Select<OrderInfo>(b => b.FinishDate, b => b.OrderStatus)
                    .ToList<DistributionBrokerageModel>();

        private List<DistributionOrderMemberBrokerageModel> Calculate(DistributionBrokerageModel order)
        {
            var result = new List<DistributionOrderMemberBrokerageModel>();
            if (order.SuperiorId1 > 0 && order.BrokerageRate1 > 0)
            {
                var brokerage = GetDivide100Number(order.RealPayAmount * order.BrokerageRate1);
                if (brokerage > 0 && AddCapital(order.SuperiorId1, brokerage, order.OrderId))
                {
                    result.Add(new DistributionOrderMemberBrokerageModel
                    {
                        OrderId = order.OrderId,
                        MemberId = order.SuperiorId1,
                        Brokerage = brokerage,
                        SettledDate = DateTime.Now
                    });
                }
            }
            if (order.SuperiorId2 > 0 && order.BrokerageRate2 > 0)
            {
                var brokerage = GetDivide100Number(order.RealPayAmount * order.BrokerageRate2);
                if (brokerage > 0 && AddCapital(order.SuperiorId2, brokerage, order.OrderId))
                {
                    result.Add(new DistributionOrderMemberBrokerageModel
                    {
                        OrderId = order.OrderId,
                        MemberId = order.SuperiorId2,
                        Brokerage = brokerage,
                        SettledDate = DateTime.Now
                    });
                }
            }
            if (order.SuperiorId3 > 0 && order.BrokerageRate3 > 0)
            {
                var brokerage = GetDivide100Number(order.RealPayAmount * order.BrokerageRate3);
                if (brokerage > 0 && AddCapital(order.SuperiorId3, brokerage, order.OrderId))
                {
                    result.Add(new DistributionOrderMemberBrokerageModel
                    {
                        OrderId = order.OrderId,
                        MemberId = order.SuperiorId3,
                        Brokerage = brokerage,
                        SettledDate = DateTime.Now
                    });
                }
        ...
        public List<DistributionOrderMemberBrokerageModel> Settlement(DateTime settlementTime)
        {
            var result = new List<DistributionOrderMemberBrokerageModel>();
            try
            {
                var models = GetWaitSettlement(settlementTime);
                //退款中订单
                var order_Refunding = DbFactory.Default.Get<OrderRefundInfo>()
                    .Where(p => p.ManagerConfirmStatus != OrderRefundInfo.OrderRefundConfirmStatus.Confirmed && p.SellerAuditStatus != OrderRefundInfo.OrderRefundAuditStatus.UnAudit)
                    .Select(p => p.OrderId).ToList<long>();

                foreach (var order in models)
                {
                    if (order_Refunding.Contains(order.OrderId))
                        continue;
                    bool isSuccess = true;
                    DbFactory.Default.InTransaction(() =>
                    {
                        try
                        {
                            var brokerages = Calculate(order);
                            var total = brokerages.Sum(p => p.Brokerage);
                            result.AddRange(brokerages);
                            order.SettlementTime = DateTime.Now;
                            order.BrokerageStatus = DistributionBrokerageStatus.Settled;
                            DbFactory.Default.Update(order);
                            //处理商品结算
                            if (total > 0)
                            {
                                var prodata = DbFactory.Default.Get<DistributionProductInfo>().Where(d => d.ProductId == order.ProductId).FirstOrDefault();
                                if (prodata != null)
                                {
                                    prodata.SettlementAmount += total;
                                    DbFactory.Default.Update(prodata);
                                }
                            }
                            return isSuccess;
                        }
                ...
                Log.Debug("【DBJ】操作成功：" + string.Join(",", models.Select(d => d.Id).ToList()));
            }
            catch (Exception ex)
            {
                Log.Debug("【DBJ】有问题啦:" + ex.Message + ":" + ex.StackTrace);
            }
            return result;
        }
        public bool AddCapital(long memberId, decimal Amount, long orderId)
        {
            bool result = false;
            var duobj = DbFactory.Default.Get<DistributorInfo>().Where(e => e.MemberId == memberId).FirstOrDefault();
            if (duobj == null)
            {
                return result;
            }
            var dtype = DistributorRecordType.Brokerage;
            DistributorRecordInfo data = new DistributorRecordInfo
            {
                Amount = Amount,
                Type = dtype,
                Balance = duobj.Balance,
                CreateTime = DateTime.Now,
                MemberId = duobj.MemberId,
                Remark = "订单号:" + orderId.ToString()
            };
            duobj.Balance += Amount;
            duobj.SettlementAmount += Amount;
            //处理等级
            var gobj = DbFactory.Default.Get<DistributorGradeInfo>()
            .Where(d => d.Quota <= duobj.SettlementAmount)
            .OrderByDescending(d => d.Quota)
            .FirstOrDefault();
            if (gobj != null)
            {
                duobj.GradeId = gobj.Id;
            }
            result = DbFactory.Default.InTransaction(() =>
            {
                try
                {
                    DbFactory.Default.Update(duobj);
                    DbFactory.Default.Add(data);
                    return true;
                }
                catch (Exception ex)
