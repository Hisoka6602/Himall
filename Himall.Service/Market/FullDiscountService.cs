...
{
    public class FullDiscountService : ServiceBase
    {
        /// <summary>
        /// 当前活动类型
        /// </summary>
        ...
        public void AddActive(ActiveInfo data, IEnumerable<FullDiscountRuleInfo> rules, IEnumerable<ActiveProductInfo> products)
        {
            ...
            CacheManager.ClearAvailableFullDiscount();
        }
        /// <summary>
        /// 更新满减活动
        /// </summary>
        ...
        public IEnumerable<FullDiscountData> GetNotEnd() =>
              CacheManager.GetAvailableFullDiscount(() =>
              {
                  var data = DbFactory.Default.Get<ActiveInfo>()
                  .Where(p => p.ActiveType == CurrentActiveType && p.EndTime > DateTime.Now)
                  .ToList<FullDiscountData>();
                  var list = data.Select(p => p.Id).ToList();

                  var products = DbFactory.Default.Get<ActiveProductInfo>(p => p.ActiveId.ExIn(list)).ToList();
                  var rules = DbFactory.Default.Get<FullDiscountRuleInfo>(p => p.ActiveId.ExIn(list)).ToList();
                  foreach (var item in data)
                  {
                      item.Products = products.Where(p => p.ActiveId == item.Id).Select(p => p.ProductId).ToList();
                      item.IsAllProduct = item.Products.Contains(-1);
                      item.Rules = rules.Where(p => p.ActiveId == item.Id)
                      .OrderBy(p => p.Quota)
                      .Select(p => new FullDiscountRuleData
                      {
                          Discount = p.Discount,
                          Quota = p.Quota
                      }).ToList();
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<FullDiscountData> GetGoing() =>
          GetNotEnd().Where(p => p.StartTime < DateTime.Now);
        public IEnumerable<FullDiscountData> GetGoingByShop(long shopId) =>
            GetGoing().Where(p => p.ShopId == shopId);
