...
namespace Himall.Service
{
    ...
    /// 限时购服务实现
    /// </summary>
    public class LimitTimeBuyService : ServiceBase
    {
        #region 平台
        ...
        public void UpdateConfig(FlashSaleConfigModel data)
        {
            ...
            CacheManager.ClearAvailableFlashSale(model.Id);
        }

        public List<FlashSaleInfo> GetLastModify()
        {
            var curDate = DateTime.Now;
            var date = DateTime.Now.AddMinutes(-15);
            return DbFactory.Default.Get<FlashSaleInfo>().Where(e => (e.BeginDate <= curDate && e.BeginDate >= date) || (e.EndDate < curDate && e.EndDate >= date)).ToList();
        }

        public void AddFlashSaleDetails(List<FlashSaleDetailInfo> list)
        {
            DbFactory.Default.Add<FlashSaleDetailInfo>(list);
            foreach (var item in list.Select(p => p.FlashSaleId).Distinct())
                CacheManager.ClearAvailableFlashSale(item);
        }
        ...
        public void Delete(long id, long shopId)
        {
            ...
            CacheManager.ClearAvailableFlashSale(id);
        }
        ...
        public void UpdateServiceSetting(LimitTimeBuySettingModel model)
        {
            ...
            CacheManager.ClearAvailableFlashSale(market.Id);
        }

        public List<FlashSaleItemSimp> GetLimitItems(long id) =>
             DbFactory.Default.Get<FlashSaleDetailInfo>(p => p.FlashSaleId == id)
                .LeftJoin<SKUInfo>((f, s) => f.SkuId == s.Id)
                .Select(p => new
                {
                    p.SkuId,
                    Number = p.TotalCount,
                ...
                {
                    p.Stock
                }).ToList<FlashSaleItemSimp>();

        /// <summary>
        /// 未结束的活动
        /// </summary>
        public IEnumerable<FlashSaleData> GetNotEnd() =>
             CacheManager.GetAvailableFlashSale(() =>
             {
                 var data = DbFactory.Default.Get<FlashSaleInfo>(p => p.EndDate > DateTime.Now).OrderBy(p => p.BeginDate).ToList<FlashSaleData>();
                 var list = data.Select(p => p.Id).ToList();
                 var items = DbFactory.Default.Get<FlashSaleDetailInfo>(p => p.FlashSaleId.ExIn(list)).ToList();
                 foreach (var item in data)
                 {
                     item.Items = items.Where(p => p.FlashSaleId == item.Id).Select(p => new FlashSaleItemData
                     {
                         SkuId = p.SkuId,
                         Price = p.Price,
                     }).ToList();
        ...
        public IEnumerable<FlashSaleData> GetGoingByProduct(List<long> products) =>
            GetNotEnd().Where(p => products.Contains(p.ProductId) && p.BeginDate <= DateTime.Now);
        /// <summary>
        /// 获取活动中
        /// </summary>
        public FlashSaleData GetGoing(long id) =>
            GetNotEnd().FirstOrDefault(p => p.BeginDate < DateTime.Now && p.Id == id);

        public FlashSaleData GetFlashSaleData(long id) =>
            CacheManager.GetFlashSale(id, () =>
            {
                var data = DbFactory.Default.Get<FlashSaleInfo>(p => p.Id == id).FirstOrDefault<FlashSaleData>();
                data.Items = DbFactory.Default.Get<FlashSaleDetailInfo>(p => p.FlashSaleId == id).ToList<FlashSaleItemData>();
                return data;
            });
        ...
        public void ClearRemind(List<long> list) =>
            DbFactory.Default.Del<FlashSaleRemindInfo>(d => d.Id.ExIn(list));
        ...
        public FlashSaleModel GetFlaseSaleByProductId(long pid)
        {
            ...
            if (model != null)
            {
                var product = ServiceProvider.Instance<ProductService>.Create.GetProduct(model.ProductId);
                result.Id = model.Id;
        ...
        private void CheckFlashSale(FlashSaleModel model)
        {
            ...
            var endtime = ServiceProvider.Instance<MarketService>.Create.GetServiceEndTime(co.Id);
        ...
        public void UpdateFlashSale(FlashSaleModel model)
        {
            ...
            if (flashSale == null)
            {
                throw new HimallException("此活动已被删除");
            }
            DbFactory.Default.InTransaction(() =>
            {
        ...
        /// 减库存
        /// </summary>
        public bool Decrease(long flashSaleId, string skuId, int quantity) =>
            DbFactory.Default.Set<FlashSaleDetailInfo>()
                .Where(p => p.FlashSaleId == flashSaleId && p.SkuId == skuId && p.TotalCount >= quantity)
                .Set(p => p.TotalCount, p => p.TotalCount - quantity)
                .Succeed();
        /// <summary>
        /// 还原库存
        /// </summary>
        public bool Restore(long flashSaleId, string skuId, int quantity) =>
            DbFactory.Default.Set<FlashSaleDetailInfo>()
                .Where(p => p.FlashSaleId == flashSaleId && p.SkuId == skuId)
                .Set(p => p.TotalCount, p => p.TotalCount + quantity)
                .Succeed();
        /// <summary>
        /// 增加销量
        /// </summary>
        public void Increase(long flashSaleId, int quantity) =>
        ...
        public void ReduceSaleCount(OrderRefundInfo refund)
        {
            ...
            var orderItem = Himall.ServiceProvider.Instance<OrderService>.Create.GetOrderItem(refund.OrderItemId);
            //限时购销量
        ...
        public List<FlashSalePrice> GetPriceByProducrIds(List<long> ids)
        {
            ...
            var list = db.Select(n => new { n.Id, n.ProductId, n.MinPrice }).ToList<FlashSalePrice>();

            #region 补充商品名
            var products = DbFactory.Default.Get<ProductInfo>().Where(t => t.Id.ExIn(ids));
            if (products != null && products.Count() > 0)
            {
                list.ForEach(item =>
                {
                    var proinfo = products.Where(t => t.Id == item.ProductId).FirstOrDefault();
                    if (proinfo != null)
                        item.ProductName = proinfo.ProductName;
                });
            }
            #endregion
        ...
        public void EndActive(long id)
        {
            ...
            if (data != null)
            {
                if (data.BeginDate >= DateTime.Now)
                    data.BeginDate = DateTime.Now.AddMinutes(-1);
        ...
        public void AddRemind(long flashSaleId, string openId)
        {
            ...
            var product = ServiceProvider.Instance<ProductService>.Create.GetProduct(flashSale.ProductId);
            ...
            //处理url
            var _iwxtser = Himall.ServiceProvider.Instance<WXMsgTemplateService>.Create;
            //string url = _iwxtser.GetMessageTemplateShowUrl(MessageTypeEnum.SubscribeLimitTimeBuy);
            //Task.Factory.StartNew(() => ServiceProvider.Instance<WXMsgTemplateService>.Create.SendMessageByTemplate(MessageTypeEnum.SubscribeLimitTimeBuy, 0, msgdata, url, openid));
        }


        #endregion

        public int GetBuyCount(long pId, long userId)
        {
            int count=0;
            var list = DbFactory.Default.Get<OrderItemInfo>()
                .InnerJoin<OrderInfo>((oii, oi) => oii.OrderId == oi.Id)
        ...
        public long GetBuyCount(long flashId, long userId, long productId)
        {
            return DbFactory.Default.Get<OrderItemInfo>()
                 .LeftJoin<OrderInfo>((oii, oi) => oii.OrderId == oi.Id)
                .Where(p => p.ProductId == productId && p.FlashSaleId == flashId)
        ...
        {
            return DbFactory.Default.Get<FlashSaleDetailInfo>().Where(s => s.FlashSaleId.ExIn(flashSaleIds)).ToList();
        }
        public List<FlashSaleInfo> GetFlashSaleInfos(IEnumerable<long> flashSaleIds)
        {
            return DbFactory.Default.Get<FlashSaleInfo>().Where(s => s.Id.ExIn(flashSaleIds)).ToList();
        }
        ...
        /// <param name="stocksku"></param>
        /// <returns></returns>
        public long UpdateStockBySkuId(string skuId, long stocksku)
        {
            DbFactory.Default.Set<FlashSaleDetailInfo>().Set(n => n.TotalCount, n => n.TotalCount - stocksku).Where(n => n.SkuId == skuId).Succeed();
            var totalcount = DbFactory.Default.Get<FlashSaleDetailInfo>().Where(n => n.SkuId == skuId).FirstOrDefault().TotalCount;
            return totalcount;
        }

        public List<FlashSaleSummary> GetSummary(IEnumerable<long> list)
        {
            var flashs = DbFactory.Default.Get<FlashSaleInfo>()
            ...
            var details = DbFactory.Default.Get<FlashSaleDetailInfo>()
                .Where(p => p.FlashSaleId.ExIn(list))
                .GroupBy(p => p.FlashSaleId)
                .Select(p => new
                {
                    Item1 = p.FlashSaleId,
                    Item2 = p.TotalCount.ExSum()
                }).ToList<SimpItem<long, int>>();

            return flashs.Select(item =>
            {
                var detail = details.FirstOrDefault(p => p.Item1 == item.Item1);
                return new FlashSaleSummary
                {
                    Id = item.Item1,
                    SaleCount = item.Item2,
                    Total = detail?.Item2 ?? 0
                };
            }).ToList();
        }
        ...
        /// <returns></returns>
        public long GetFlashSaleSaleCount(long id)
        {
            return DbFactory.Default.Get<FlashSaleInfo>().Where(p => p.Id == id).Select(p => p.SaleCount).Sum(p => p.SaleCount);
        }
    }
