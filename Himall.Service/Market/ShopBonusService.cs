...
{
    public class ShopBonusService : ServiceBase
    {
        ...
        public List<ShopBonusData> GetAvailableBonus() =>
            CacheManager.GetAvailableBonus(() =>
                 DbFactory.Default.Get<ShopBonusInfo>(p => p.IsInvalid == false && p.DateEnd > DateTime.Now).ToList<ShopBonusData>()
            ).Where(p => p.DateStart < DateTime.Now && p.DateEnd > DateTime.Now).ToList();
        ...
        public ShopBonusGrantInfo GetGrantByUserOrder(long orderid, long userid)
        {
            ...
            var result = DbFactory.Default
                ...
                .Where<ShopBonusInfo>(p => p.DateEnd > DateTime.Now && p.IsInvalid == false)
        ...
        public void Add(ShopBonusInfo model, long shopid)
        {
            ...
            CacheManager.ClearAvailableBonus();
            #region 同步微信
            if (model.SynchronizeCard == true)
            {
                ...
                ServiceProvider.Instance<WXCardService>.Create.Add(wxdata);
            }
            #endregion
        }
        ...
        public bool IsOverDate(DateTime bonusDateEnd, DateTime dateEnd, long shopid)
        {
            ...
            var time = ServiceProvider.Instance<MarketService>.Create.GetServiceEndTime(market.Id);
        ...
        public void SetBonusToUsed(long userid, List<OrderInfo> orders, long rid)
        {
            ...
            DbFactory.Default.Update(model);
            //同步核销卡券
            Himall.ServiceProvider.Instance<WXCardService>.Create.Consume(model.Id, WXCardLogInfo.CouponTypeEnum.Bonus);
        }

        public void UesRecord(long id, long order) {
            DbFactory.Default.Set<ShopBonusReceiveInfo>()
                .Where(p => p.Id == id)
                .Set(p => p.State, ShopBonusReceiveInfo.ReceiveState.Use)
                .Set(p => p.UsedTime, DateTime.Now)
                .Set(p => p.UsedOrderId, order)
                .Execute();
        ...
        /// 生成红包详情
        /// </summary>
        public long GenerateBonusDetail(ShopBonusInfo model, long orderid, string receiveurl,decimal ordermoney)
        {
            ...
            if (model.UseState==ShopBonusInfo.UseStateType.FilledSend&&model.GrantPrice > ordermoney) {
                Log.Info("此活动未达到实付金额 , shopid = " + model.ShopId);
                return 0;
            }
