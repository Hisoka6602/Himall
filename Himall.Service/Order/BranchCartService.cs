...
{
    public class BranchCartService : ServiceBase
    {
        ...
        public void UpdateCart(string skuId, int count, long memberId, long shopbranchId, long roomId = 0)
        {
            CheckCartItem(skuId, count, memberId, shopbranchId);
            ...
            {
                long productId = long.Parse(skuId.Split('_')[0]);//SKU第一节为商品Id
                DbFactory.Default.Add(new ShoppingCartInfo() { UserId = memberId, Quantity = count, SkuId = skuId, ProductId = productId, AddTime = DateTime.Now, ShopBranchId = shopbranchId, RoomId = roomId });
                if (roomId > 0)
                {
