...
{
    public class ExpressService : ServiceBase
    {
        ...
        public void ChangeExpressStatus(long id, CommonModel.ExpressStatus status)
        {
            ...
            if (!flag) throw new HimallException("快递公司不存在");
        }
        ...
        public ExpressData GetExpressData(string expressCompanyName, string shipOrderNumber,string orderId,string phoneNumber)
        {
            var expressData = new ExpressData();//创建对象
            var expressInfo= GetExpress(expressCompanyName);
            if (expressInfo != null)
            {
                expressData = GetExpressDataByKuai(expressInfo.KuaidiNiaoCode, shipOrderNumber,orderId,phoneNumber);
            }
            return expressData;
        }
        ...
        {
            var settting = Himall.ServiceProvider.Instance<SiteSettingService>.Create.SiteSettings;
        ...
        /// <returns></returns>
        private ExpressData GetExpressDataByKuai(string shipperCode, string logisticsCode,string orderId,string phoneNumber)
        {
            var settting = Himall.ServiceProvider.Instance<SiteSettingService>.Create.SiteSettings;
            string timestamp = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            //签名
            //string si = string.Format("app_key{0}logisticsCode{1}shipperCode{2}timestamp{3}OrderId{4}{5}", settting.KuaidiApp_key, logisticsCode, shipperCode, timestamp, orderId, settting.KuaidiAppSecret);
            //string sign = FormsAuthentication.HashPasswordForStoringInConfigFile(si, "MD5");
            IDictionary<string, string> dic = new Dictionary<string, string>();
            dic.Add("app_key", settting.KuaidiApp_key);
            dic.Add("timestamp", timestamp);
            dic.Add("shipperCode", shipperCode);
            dic.Add("logisticsCode", logisticsCode);
            dic.Add("OrderId", orderId);
            dic.Add("ReceiveMobile", phoneNumber);
            string sign = SignTopRequest(dic, settting.KuaidiAppSecret);
            var expressData = new ExpressData();//创建对象
            string url = string.Format("https://enterprise.wuliu.kuaidiantong.cn/api/logistics?app_key={0}&timestamp={1}&shipperCode={2}&logisticsCode={3}&sign={4}&OrderId={5}&ReceiveMobile={6}", settting.KuaidiApp_key, timestamp, shipperCode, logisticsCode, sign, orderId, phoneNumber);//快递查询地址
            HttpWebRequest request = (HttpWebRequest)HttpWebRequest.Create(url);
            request.Timeout = 8000;
            request.Method = "GET";
            request.ContentType = "application/json;charset=UTF-8";
        ...
        public static string SignTopRequest(IDictionary<string, string> parameters, string appSecret)
        {
            ...
            //加上密码
            if (!string.IsNullOrEmpty(appSecret)) { query.Append(appSecret); }
        ...
        public int GetSurplusQuantity()
        {
            ...
            string si = string.Format("app_key{0}timestamp{1}{2}", settting.KuaidiApp_key, timestamp, settting.KuaidiAppSecret);
            string sign = FormsAuthentication.HashPasswordForStoringInConfigFile(si, "MD5");

            string url = string.Format("https://enterprise.wuliu.kuaidiantong.cn/api/surplusquantity?app_key={0}&timestamp={1}&sign={2}", settting.KuaidiApp_key, timestamp, sign);//快递查询地址
            HttpWebRequest request = (HttpWebRequest)HttpWebRequest.Create(url);
            request.Timeout = 8000;
            request.Method = "GET";
            request.ContentType = "application/json;charset=UTF-8";
            ...
            try
            {
                ...
                if (response.StatusCode == HttpStatusCode.OK)
                {
                    ...
                    if (content.Length > 0)
                    {
                        ...
                        var m = Newtonsoft.Json.JsonConvert.DeserializeAnonymousType(content.ToString(), expressObj);
                        if (m.success && m.surplusQuantity > 0)
                        {
                            quantity = m.surplusQuantity;
                        }
                    }
                }
            }
        ...
        { //使用快递100查询快递数据
            ...
            try
            {
                ...
                var result = new { result = false, returnCode = "", message = "" };
                ...
                if (!m.result)
                {
                    Core.Log.Error("物流通知订阅失败：" + result.message + " returnCode=" + result.returnCode + ",物流公司代码：" + kuaidi100Code + ",物流单号:" + number + ",物流到达城市:" + city + ",快递Key:" + kuaidi100Key + "回调地址：" + redirectUrl);
                }
            }
            catch (Exception ex)
            {
