...
namespace Himall.Service
{
    public class BrandService : ServiceBase
    {
        ...
        public QueryPageModel<BrandInfo> GetShopBrands(long shopId, int pageNo, int pageSize,string keyword="")
        {

            var shopBrandIds=new List<long>();

            if (!string.IsNullOrEmpty(keyword))
            {
                shopBrandIds = DbFactory.Default.Get<ShopBrandInfo>().InnerJoin<BrandInfo>((s, b) => s.BrandId == b.Id && s.ShopId == shopId && b.IsDeleted == false&&b.Name.Contains(keyword)).Select(a => a.BrandId).ToList<long>();
            }
            else {
                shopBrandIds=DbFactory.Default.Get<ShopBrandInfo>().InnerJoin<BrandInfo>((s, b) => s.BrandId == b.Id && s.ShopId == shopId && b.IsDeleted == false).Select(a => a.BrandId).ToList<long>();
            }
