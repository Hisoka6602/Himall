...
namespace Himall.Service
{
    public class CommentService : ServiceBase
    {
        ...
        public GetBuilder<ProductCommentInfo> WhereBuilder(CommentQuery query) {
            ...
            switch (query.Sort) {
                case "ReplyAppendDate":
                    if (query.IsAsc)
                    {
                        db.OrderBy(c => c.ReplyAppendDate);
                    }
                    else {
                        db.OrderByDescending(c=>c.ReplyAppendDate);
                    }
                    break;
                default:
                    if (query.IsAsc)
        ...
        /// 获取评论概览+门店id条件
        /// </summary>
        public CommentSummaryData GetCommentSummary(long productId, long shopBranchId = 0) =>
            CacheManager.GetProductCommentSummary(productId, shopBranchId, () =>
                 GetCommentSummaryData(productId, shopBranchId));
        ...
        /// <returns></returns>
        private CommentSummaryData GetCommentSummaryData(long productId, long shopBranchId = 0)
        {
            var db = DbFactory.Default.Get<ProductCommentInfo>().Where(p => p.ProductId == productId && p.IsHidden == false);
            if (shopBranchId > 0)
            {
                //TODO:FG 查询待优化
                var orderitemIds = DbFactory.Default.Get<OrderItemInfo>().LeftJoin<OrderInfo>((oii, oi) => oii.OrderId == oi.Id)
                    .Where<OrderInfo>((item, o) => o.ShopBranchId == shopBranchId).Select(t => t.Id);

                db.Where(d => d.SubOrderId.ExIsNotNull() && d.SubOrderId.ExIn(orderitemIds));
            }
            return db.Select(item => new
            {
                Total = item.ExCount(false),
                Average = "avg(reviewMark)".ExSql(),
                Positive = "sum(case when reviewMark>=4 then 1 else 0 end)".ExSql(),
                Neutral = "sum(case when reviewMark=3 then 1 else 0 end)".ExSql(),
                Negative = "sum(case when reviewMark<=2 then 1 else 0 end)".ExSql()
            }).FirstOrDefault<CommentSummaryData>();
        }
        ...
        {
            var siteSetting = ServiceProvider.Instance<SiteSettingService>.Create.SiteSettings;
            var token = new WXApiService().TryGetToken(siteSetting.WeixinAppId, siteSetting.WeixinAppSecret);
        ...
        public void AppendComment(List<AppendCommentModel> list)
        {
            ...
            DbFactory.Default.InTransaction(() =>
            {
                ...
                foreach (var model in list)
                {
                    ...
                    userid = m.UserId;
                    userName = m.UserName;
                    var orderItem = ServiceProvider.Instance<OrderService>.Create.GetOrderItem(m.SubOrderId);
        ...
        /// <summary>
        /// 获取商品好评数列表（只获取好评数，主要用于门店读取商品列表好评数）
        /// </summary>
        /// <param name="productIds"></param>
        /// <param name="shopId"></param>
        /// <returns></returns>
        public List<long> GetProductCommentHightStatisticList(List<long> productIds, long shopId, long? shopBranchId = null, bool iso2o = false)
        {
            //以前评论条数是搜索表里的 统计的所有评论条数(其他上面linq查询很慢，还是用sql查询)
            string selComments = string.Format("select ProductId FROM himall_productcomment WHERE ShopId = {0} and ReviewMark>= {1} and ProductId in({2})",
                shopId, (int)ProductCommentMarkType.High, string.Join(",", productIds));//

            if (shopBranchId.HasValue)
            {
                selComments += string.Format(" AND SubOrderId in (SELECT b.Id from himall_orderitem b LEFT JOIN himall_order a on b.orderId = a.id where a.ShopBranchId={0})", shopBranchId.Value);
            }

            return DbFactory.Default.Query<long>(selComments).ToList();
        }
