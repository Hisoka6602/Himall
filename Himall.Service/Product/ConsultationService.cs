...
{
    public class ConsultationService : ServiceBase
    {
        ...
        public void ReplyConsultation(long id, string replyContent, long shopId)
        {
            ...
            if (!flag || shopId == 0) throw new Himall.Core.HimallException("不存在该商品评论");
        }
        ...
        public int GetConsultationCount(long product) =>
            CacheManager.GetConsultationCount(product, () => DbFactory.Default.Get<ProductConsultationInfo>(c => c.ProductId == product).Count());
    }
}
