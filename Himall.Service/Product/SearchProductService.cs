...
{
    public class SearchProductService : ServiceBase
    {

        public void AddSearchProduct(long productId)
        {
            var sql = new Sql();
            sql.Append("INSERT INTO `himall_searchproduct` (`ProductId`, `ProductName`, `ShopId`, `ShopName`, `BrandId`, `BrandName`, `FirstCateId`, `FirstCateName`, `SecondCateId`, `SecondCateName`, `ThirdCateId`, `ThirdCateName`, `AttrValues`, `Comments`, `SaleCount`, `SalePrice`, `OnSaleTime`, `ImagePath`, `CanSearch`, `BrandLogo`, `ProductType`) ");
            sql.Append("select a.Id,a.ProductName,a.ShopId,b.ShopName,a.BrandId,c.`Name` as BrandName,SUBSTRING_INDEX(a.CategoryPath, '|', 1) AS FirstCateId, d.Name as FirstCateName,SUBSTRING_INDEX(SUBSTRING_INDEX(CategoryPath, '|', 2), '|', -1) AS SecondCateId, e.Name as SecondCateName,SUBSTRING_INDEX(CategoryPath, '|', -1) AS ThirdCateId, f.Name as ThirdCateName,g.AttrValues,0,0,a.MinSalePrice,a.AddedDate,a.ImagePath,(case when a.SaleStatus=1 and a.AuditStatus=2 and a.IsDeleted=0 then 1 else 0 end) as CanSearch,c.Logo,a.ProductType from himall_product a ");
            sql.Append("left join himall_shop b on a.ShopId = b.Id ");
            sql.Append("left join himall_brand c on a.BrandId = c.Id ");
            sql.Append("left join himall_category d on SUBSTRING_INDEX(a.CategoryPath, '|', 1) = d.Id ");
            sql.Append("left join himall_category e on SUBSTRING_INDEX(SUBSTRING_INDEX(a.CategoryPath, '|', 2), '|', -1) = e.Id ");
            sql.Append("left join himall_category f on SUBSTRING_INDEX(a.CategoryPath,'|', -1) = f.Id ");
            sql.Append("left join (select ProductId, group_concat(ValueId) as AttrValues from himall_productattribute group by productId) g on a.Id = g.ProductId ");
            sql.Append("where a.Id=@0", productId);

            DbFactory.Default.Execute(sql);//, new { ProductId = productId });
        }

        public List<SearchProductInfo> GetSalePriceByIds(List<long> proIds)
        {
            var sql = new Sql("select * from himall_searchproduct where ProductId in(" + string.Join(",",proIds) +")");
            return DbFactory.Default.Query<SearchProductInfo>(sql).ToList();
        }

        public void UpdateSearchProduct(long productId)
        {
            var sql = new Sql();
            sql.Append("update himall_searchproduct a ");
            sql.Append("left join himall_product b on a.ProductId = b.Id ");
            sql.Append("left join himall_category c on SUBSTRING_INDEX(b.CategoryPath, '|', 1) = c.Id ");
            sql.Append("left join himall_category d on SUBSTRING_INDEX(SUBSTRING_INDEX(b.CategoryPath, '|', 2), '|', -1) = d.Id ");
            sql.Append("left join himall_category e on SUBSTRING_INDEX(b.CategoryPath, '|', -1) = e.Id ");
            sql.Append("left join (select ProductId, group_concat(ValueId) as AttrValues from himall_productattribute group by productId) f on a.ProductId = f.ProductId ");
            sql.Append("left join himall_shop g on b.ShopId = g.Id ");
            sql.Append("left join himall_brand h on b.BrandId = h.Id ");
        ...
        {
            var sql = new Sql("update himall_searchProduct set ShopName=@0 where ShopId=@1", shopName, shopId);
        ...
        {
            var sql = new Sql("update himall_searchproduct a left join himall_product b on a.productid=b.id set a.cansearch=(case when b.SaleStatus=1 and b.AuditStatus=2 and b.IsDeleted=0 then 1 else 0 end) where a.productid=@0", productId);
        ...
        {
            var sql = new Sql("update himall_searchproduct a left join himall_product b on a.productid=b.id set a.cansearch=(case when b.SaleStatus=1 and b.AuditStatus=2 and b.IsDeleted=0 then 1 else 0 end) where a.productid in (@0)", productIds);

            DbFactory.Default.Execute(sql);
        }
        public void UpdateSearchStatusByShop(long shopId)
        {
            var sql = new Sql("update himall_searchproduct a left join himall_product b on a.productid = b.id set CanSearch = 0 where b.shopid = @0 and cansearch=1; ", shopId);
            sql.Append(" update himall_searchproduct a left join himall_product b on a.productid = b.id set CanSearch = 1 where a.cansearch=0 and a.shopid = @0 and b.AuditStatus = 2 and b.SaleStatus = 1 and b.IsDeleted = 0; ", shopId);
            DbFactory.Default.Execute(sql);
        ...
        {
            var sql = new Sql("update himall_searchProduct set BrandName=@0,BrandLogo=@1 where BrandId=@2", brand.Name, brand.Logo, brand.Id);
        ...
        {
            var sql = "update himall_searchProduct set {0}CateName=@0 where {0}CateId=@1";
        ...
        public SearchProductResult SearchProduct(SearchProductQuery query)
        {
            ...
            string selComments = "(SELECT COUNT(*) FROM himall_productcomment pc WHERE pc.productid=s.productid AND pc.Ishidden=0) as Comments";//搜索这里评论数获取排除隐藏的条数

            SearchProductResult result = new SearchProductResult();
            string countsql = "select count(1) from himall_searchproduct s  ";
            string sql = "select (p.SaleCounts+ IFNULL(p.VirtualSaleCounts,0)) as  SaleCount,p.UpdateTime as UpdateTime, s.ProductId,s.ProductName,s.SalePrice,s.ImagePath,s.ShopId,s.ShopName,s.ThirdCateId,s.ActivityId,s.ActiveType,p.DisplaySequence,p.ShopDisplaySequence,p.ProductType,IFNULL(p.VirtualSaleCounts,0) as  VirtualSaleCounts,p.MinSalePrice," + selComments + "   from himall_searchproduct s ";
            ...
            index += string.Format(" left join Himall_Product p on p.Id=s.ProductId  ");//FORCE INDEX要放在join之前
            countsql += string.Format(" left join Himall_Product p on p.Id=s.ProductId  ");
            ...
            FillMarketing(result.Data);
            return result;
        }

        private void FillMarketing(List<ProductView> data)
        {
        ...
        public SearchProductFilterResult SearchProductFilter(SearchProductQuery query)
        {
            try
            {
                ...
                //查出符合条件的属性值
                string attrSql = "select AttrValues from himall_searchproduct s  left join Himall_Product p on p.Id=s.ProductId  " + where.SQL;
                ...
                //查询符合条件的品牌
                var brandSql = "select DISTINCT s.BrandId Id,BrandName Name,BrandLogo Logo from himall_searchproduct s  left join Himall_Product p on p.Id=s.ProductId  " + where.SQL + " and s.BrandId is not null AND s.BrandId<>0";
                result.Brand = DbFactory.Default.Query<BrandView>(brandSql, where.Arguments).ToList();

                var allCategorys = ObjectContainer.Current.Resolve<CategoryService>().GetCategories();

                //查询符合条件的分类
                var categorySql = "select FirstCateId,MAX(FirstCateName) FirstCateName,SecondCateId,MAX(SecondCateName) SecondCateName,ThirdCateId,MAX(ThirdCateName) ThirdCateName from himall_searchproduct s  left join Himall_Product p on p.Id=s.ProductId  " + where.SQL + " group by  FirstCateId , SecondCateId , ThirdCateId ";
        ...
        private void GetSearchWhere(SearchProductQuery query, Sql where)
        {
            ...
            if (query.VShopId != 0)
            {
                where.Append(" AND s.ShopId IN (SELECT ShopId FROM himall_vshop where Id=@0) ", query.VShopId);
                //parms.Add("@VShopId", query.VShopId);
            }

            if ((query.VShopId != 0 || query.ShopId != 0) && query.ShopCategoryId != 0)
            {
                where.Append(" AND s.ProductId IN (select ProductId from himall_productshopcategory where ShopCategoryId in(select id from Himall_ShopCategory where ShopId = @0 and(id = @1 or ParentCategoryId = @1))) ", query.ShopId, query.ShopCategoryId);
            ...
            {
                where.Append("  AND s.ProductId IN (SELECT DISTINCT ProductId FROM Himall_ProductAttribute ");
                //此处属性筛选，要取交集非并集
                foreach (var item in query.AttrValIds)
                {
                    where.Append(" INNER JOIN (SELECT DISTINCT ProductId FROM Himall_ProductAttribute WHERE  (ValueId = " + item + " ) ) t" + query.AttrValIds.IndexOf(item) + " USING (ProductId) ");
                }
                where.Append(")");
            ...
            {
                where.Append(" AND s.ProductId IN (select DISTINCT ProductId from himall_shopbranchsku where ShopBranchId =@0) ", query.ShopBranchId);
            }
            ///优惠券指定部分商品使用
            if (query.CouponId>0)
            {
                where.Append(" AND s.ProductId IN (select ProductId from himall_couponproduct where couponid =@0) ", query.CouponId);
            }
            if (query.FilterNoStockProduct)
            {
               
                where.Append(" AND s.ProductId IN(SELECT DISTINCT ProductId FROM(select ProductId, SUM(Stock) AllStock from himall_sku GROUP BY ProductId)sku where AllStock > 0) ");
            }
            #endregion
        }
        ...
        private void GetAppletSearchWhere(SearchProductQuery query, Sql where)
        {
            ...
            if ((query.VShopId != 0 || query.ShopId != 0) && query.ShopCategoryId != 0)
                where.Append(" AND ps.ProductId IN (select ProductId from himall_productshopcategory where ShopCategoryId in (select id from Himall_ShopCategory where ShopId = @1 and(id = @0 or ParentCategoryId = @0))) ", query.ShopCategoryId, query.ShopId);
            ...
            if (query.AttrValIds.Count > 0)
                where.Append(" AND ps.ProductId IN (SELECT DISTINCT ProductId FROM Himall_ProductAttribute WHERE ValueId IN (@0)) ", query.AttrValIds);
        ...
        public void UpdateSearchProductActivty(long productId, long shopId, decimal newPrice,ProductInfo.ProductActiveType activeType,long activtyId)
        {
            DbFactory.Default.Set<SearchProductInfo>().Set(p => p.SalePrice, newPrice)
                .Set(p=>p.ActiveType,activeType).Set(p=>p.ActivityId,activtyId)
                .Where(p => p.ProductId == productId && p.ShopId == shopId).Succeed();
