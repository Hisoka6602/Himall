...
namespace Himall.Service
{
    ...
    public class BillingService : ServiceBase
    {
        ...
        {
            return DbFactory.Default.Get<ShopWithDrawInfo>().Where(s => s.ShopId == shopId && (s.Status == WithdrawStaus.WatingAudit || s.Status == WithdrawStaus.PayPending || s.Status == WithdrawStaus.Fail)).Sum<decimal>(s => s.CashAmount);
        }
        ...
        public QueryPageModel<StatisticsPendingSettlement> StatisticsPendingSettlementOrders(StatisticsPendingSettlementQuery query)
        {
            ...
            db = db.Select(p => new { ShopId = p.ShopId, Amount = p.SettlementAmount.ExSum() }).GroupBy(p => p.ShopId);
        ...
        private GetBuilder<PendingSettlementOrderInfo> ToWhere(PendingSettlementOrderQuery query)
        {
            ...
            {
                var end = query.CreateDateEnd.Value.Date.AddDays(1);
