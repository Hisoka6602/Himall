...
namespace Himall.Service
{
    public class ShopService : ServiceBase
    {
        ...
        public GetBuilder<ShopInfo> WhereBuilder(ShopQuery query)
        {
            ...
            {
                var list = query.ShopIds.Split(',');
                db.Where(item => item.Id.ExIn(list));
            }
            ...
            {
                var ms = DbFactory.Default.Get<ManagerInfo>().Where(m => m.UserName.Contains(query.ShopAccount) && m.ShopId != 0 && m.RoleId == 0).Select(p => p.ShopId).ToList<long>();
                if (ms != null && ms.Count > 0)
                {
                    db.Where(item => item.Id.ExIn(ms));
                }
                else
                {
                    ms = new List<long>();
        ...
        public bool IsOpenHichat(long id)
        {
            var shop = DbFactory.Default.Get<ShopInfo>().Where(p => p.Id == id).FirstOrDefault();
            if (shop != null) return shop.IsOpenHiChat;
            return false;
        }
        ...
        public void UpdateShop(ShopInfo shop)
        {
            ...
            {
                UpdateOtherTableShopName(shop.Id, shop.ShopName);
            }
            ...
            actual.OrderPayIsSendSMS = shop.OrderPayIsSendSMS;
            ...
            var msg = ServiceProvider.Instance<MessageService>.Create;
        ...
        public static bool UpdateOtherTableShopName(long shopId, string shopName)
        {
            ...
            CouponService _CouponService = ServiceProvider.Instance<CouponService>.Create;
            ...
            _CouponService.SetCouponShopName(shopId, shopName);
            return true;
        }
        public void ClearShopCache(long id)
        {
            ...
            CacheManager.ClearShop(id);
        }
        /// <summary>
        /// 获取发票配置
        /// </summary>
        public InvoiceConfig GetInvoiceConfig(long shopId) =>
            CacheManager.GetOrCreate($"invoiceConfig:{shopId}", () => DbFactory.Default.Get<ShopInvoiceConfigInfo>(p => p.ShopId == shopId).FirstOrDefault<InvoiceConfig>());
        /// <summary>
        /// 保存发票配置
        /// </summary>
        public void SetProvideInvoice(ShopInvoiceConfigInfo info)
        {
            ...
            CacheManager.Clear($"invoiceConfig:{info.ShopId}");
        }

        public void SetAllotOrder(long id, bool enable)
        {
        ...
        {
            var categories = DbFactory.Default.Get<CategoryInfo>().Where(p => p.Id.ExIn(ids)).ToList();

            var result = new Dictionary<long, string>();
            if (categories != null && categories.Count > 0)
            {
                var categoryService = ServiceProvider.Instance<CategoryService>.Create;
                var source = categoryService.GetCategories();

                foreach (var item in categories)
                    result.Add(item.Id, FullCategoryNamePath(source, item.Id));
            }
            return result;
        ...
        /// <summary>
        /// 获取经营类目佣金比例GetCategories
        /// </summary>
        public Dictionary<long, decimal> GetBusiness(long shopId) =>
            CacheManager.GetOrCreate($"businessCategory:{shopId}", () =>
            {
                var data = DbFactory.Default.Get<BusinessCategoryInfo>()
                    .Where(p => p.ShopId == shopId)
                    .ToList();
                return data.ToDictionary(k => k.CategoryId, v => v.CommisRate);
            });
        ...
        public List<long> GetCategories(long shopId) =>
             DbFactory.Default.Get<BusinessCategoryInfo>().Where(b => b.ShopId == shopId).Select(p => p.CategoryId).ToList<long>();

       
        public QueryPageModel<BusinessCategoryInfo> GetBusinessCategoryList(BusinessCategoryQuery query)
        {
            var db = DbFactory.Default.Get<BusinessCategoryInfo>().Where(a => a.ShopId == query.ShopId);
            if (!string.IsNullOrEmpty(query.CategoryName))
            {
                var dbcategoryids = DbFactory.Default.Get<CategoryInfo>().Where(t => t.Depth == 3 && t.IsDeleted == false && t.Name.Contains(query.CategoryName)).Select(t => t.Id);
                db.Where(p => p.CategoryId.ExIn(dbcategoryids));
            }
            else
            {
                var dbcategoryids = DbFactory.Default.Get<CategoryInfo>().Where(t => t.IsDeleted == false).Select(t => t.Id);//有效分类
                db.Where(p => p.CategoryId.ExIn(dbcategoryids));
            }

            var categories = db.ToPagedList(query.PageNo, query.PageSize);
            var map = GetCategoryNamePath(categories.Select(p => p.CategoryId).ToList());
            categories.ForEach(item =>
            {
                if (map.ContainsKey(item.CategoryId))
        ...
        private void UpdateBusinessCategory(long shopId, List<long> categoryIds)
        {
            ...
            CacheManager.ClearCategories(shopId);
        }
        ...
        public void SaveBusinessCategory(long shopId, Dictionary<long, decimal> bCategoryList)
        {
            if (bCategoryList == null || bCategoryList.Count <= 0)
                return;

            var businesscategoryids = bCategoryList.Keys.ToList();
            var bcateups = DbFactory.Default.Get<BusinessCategoryInfo>().Where(t => t.ShopId == shopId && t.Id.ExIn(businesscategoryids));

            if (bcateups == null)
                return;

            foreach (var item in bcateups.ToList())
            {
                item.CommisRate = bCategoryList[item.Id];
                DbFactory.Default.Update(item);
            }
            CacheManager.ClearCategories(shopId);
        }

        public void AddBusinessCategory(BusinessCategoryInfo category)
        {
            var oldbusiness = DbFactory.Default.Get<BusinessCategoryInfo>().Where(t => t.ShopId == category.ShopId && t.CategoryId == category.CategoryId).FirstOrDefault();
            if (oldbusiness != null && oldbusiness.Id > 0)
            {
                throw new MessageException("新建失败,该类目已经存在!");
            }

            DbFactory.Default.Add(category);
            CacheManager.ClearCategories(category.ShopId);
        }
        ...
        public void SaveBusinessCategory(long id, decimal commisRate)
        {
            ...
            var result = DbFactory.Default.Set<BusinessCategoryInfo>()
                ...
                .Where(p => p.Id == id)
                .Succeed();
            var category = DbFactory.Default.Get<BusinessCategoryInfo>(p => p.Id == id).FirstOrDefault();
            CacheManager.ClearCategories(category.ShopId);
        }
        ...
        /// <param name="categoryid">经营分类id</param>
        public void DeleteBusinessCategory(long shopid, long categoryid)
        {
            DbFactory.Default.Del<BusinessCategoryInfo>(b => b.ShopId == shopid && b.CategoryId == categoryid);
            CacheManager.ClearCategories(shopid);
        }
        ...
        public ShopMarksData GetMarks(long id) =>
            CacheManager.GetShopMarks(id, () =>
            {
                var summary = DbFactory.Default.Get<OrderCommentInfo>(p => p.ShopId == id)
                .Select(item => new
                {
                    PackMark = "avg((PackMark+DeliveryMark)/2)".ExSql(),
                    ServiceMark = "avg(ServiceMark)".ExSql(),
                }).FirstOrDefault<ShopMarksData>();
                summary.ComprehensiveMark = Math.Round((summary.PackMark + summary.ServiceMark) / 2, 2);
                summary.ServiceMark = Math.Round(summary.ServiceMark, 2);
                summary.PackMark = Math.Round(summary.PackMark, 2);
                return summary;
            });
        ...
        public void UpdateShopStatus(long shopId, ShopInfo.ShopAuditStatus status, string comments = "", int TrialDays = 0)
        {
            ...
            if (status == ShopInfo.ShopAuditStatus.Open)
            {
                ...
                shopMessage.SiteName = Himall.ServiceProvider.Instance<SiteSettingService>.Create.SiteSettings.SiteName;
                ...
                Task.Factory.StartNew(() => ServiceProvider.Instance<MessageService>.Create.SendMessageOnShopAudited(uid, shopMessage));
                CopyTemplate(shopId);
            }
            if (status == ShopInfo.ShopAuditStatus.WaitPay)
            {
                ...
                //shopMessage.ShopName = shop.ShopName;
                //shopMessage.SiteName = Himall.ServiceProvider.Instance<SiteSettingService>.Create.SiteSettings.SiteName;
                //Task.Factory.StartNew(() => ServiceProvider.Instance<MessageService>.Create.SendMessageOnShopSuccess(shopId, shopMessage));
            }
        ...
        public void AutoExpire()
        {
            DbFactory.Default
               .InTransaction(() =>
               {
                   try
                   {
                       DbFactory.Default.Execute("update Himall_FlashSale set EndDate=date_format(EndDate,'%Y/%m/%d 23:59:59') where Status = 2 AND ShopId in (select id from himall_shop where ShopStatus=6 or TIMESTAMPDIFF(day, date_format(EndDate,'%Y/%m/%d'),'" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "')>0)");

                   }
                   catch (Exception ex)
                   {
                       Log.Error("ShopJob : AutoSaleOffProductByShopExpiredOrFreeze 有异常" + ex.Message + "::" + ex.StackTrace);
                       return false;
                   }

                   try
                   {
                       var expiredTime = DateTime.Now.Date.AddDays(1);
                       var expiredShop = DbFactory.Default.Get<ShopInfo>().Where(p => p.ShopStatus == ShopInfo.ShopAuditStatus.Freeze || p.EndDate < expiredTime).Select(p => p.Id).ToList<long>();
                       if (expiredShop.Count > 0)
                       {
                           //下架门店商品
                           DbFactory.Default.Set<ProductInfo>()
                          .Set(p => p.SaleStatus, ProductInfo.ProductSaleStatus.InStock)
                          .Where(p => p.SaleStatus == ProductInfo.ProductSaleStatus.OnSale && p.ShopId.ExIn(expiredShop))
                          .Succeed();
                           //下架门店商品
                           DbFactory.Default.Set<ShopBranchSkuInfo>()
                          .Set(p => p.Status, ShopBranchSkuStatus.InStock)
                          .Where(p => p.Status == ShopBranchSkuStatus.Normal && p.ShopId.ExIn(expiredShop))
                          .Succeed();
                           //设置门店过期
                           DbFactory.Default.Set<ShopInfo>()
                          .Set(p => p.ShopStatus, ShopInfo.ShopAuditStatus.HasExpired)
                          .Where(p => p.ShopStatus == ShopInfo.ShopAuditStatus.Open && p.Id.ExIn(expiredShop))
                          .Succeed();
                       }
                       //隐藏商品索引
                       var products = DbFactory.Default.Get<ProductInfo>(p => p.SaleStatus == ProductInfo.ProductSaleStatus.InStock).Select(p => p.Id);
                       DbFactory.Default.Set<SearchProductInfo>()
                       .Set(p => p.CanSearch, false)
                       .Where(p => p.CanSearch == true && p.ProductId.ExIn(products))
                       .Succeed();
                       //显示商品索引
                       products = DbFactory.Default.Get<ProductInfo>(p => p.AuditStatus == ProductInfo.ProductAuditStatus.Audited && p.SaleStatus == ProductInfo.ProductSaleStatus.OnSale && p.IsDeleted == false).Select(p => p.Id);
                       DbFactory.Default.Set<SearchProductInfo>()
                       .Set(p => p.CanSearch, true)
                       .Where(p => p.CanSearch == false && p.ProductId.ExIn(products))
                       .Succeed();

                   }
                   catch (Exception ex)
                   {
                       Log.Error("ShopJob : AutoCloseMarketingActionByShopExpiredOrFreeze 有异常" + ex.Message + "::" + ex.StackTrace);
                       return false;
                   }
                   return true;
               });
        ...
        public long GetShopUsageSpace(long shopId)
        {
            ...
            return HimallIO.GetDirMetaInfo(path).ContentLength / 1024 / 1024;
        }
        /// <summary>
        /// 获取门店剩余空间
        /// </summary>
        ...
        public void AddFavoriteShop(long memberId, long shopId)
        {
            ...
            CacheManager.ClearFavoriteShop(memberId);
        }
        private List<long> GetFavorite(long memberId) =>
            CacheManager.GetFavoriteShop(memberId, () => DbFactory.Default.Get<FavoriteShopInfo>(p => p.UserId == memberId).OrderByDescending(p => p.Id).Select(p => p.ShopId).ToList<long>());

        public bool IsFavoriteShop(long memberId, long shopId) =>
            GetFavorite(memberId).Contains(shopId);
        ...
        public void AddShopRenewRecord(ShopRenewRecordInfo record, bool isShopAccount = false)
        {
            ...
            DbFactory.Default.InTransaction(() =>
            {
                ...
                if (PlatAccount == null)
                {
                    PlatAccount = new PlatAccountInfo();
                    PlatAccount.Balance = 0;
                    PlatAccount.PendingSettlement = 0;
                    PlatAccount.Settled = 0;

                    DbFactory.Default.Add(PlatAccount);//如加一条默认初始数据
                }
                #endregion
        ...
        public List<CategoryRateModel> GetThirdBusinessCategory(long id, long shopId)
        {
            ...
            var categoriesAll = DbFactory.Default.Get<CategoryInfo>().Where(c => c.IsDeleted == false).ToList();
        ...
        public ShopData GetShopData(long id)
        {
            return CacheManager.GetShop(id, () =>
            {
                var shop = DbFactory.Default.Get<ShopInfo>(p => p.Id == id).FirstOrDefault<ShopData>();
                return shop;
            });
        }
        ...
        /// <returns></returns>
        public QueryPageModel<ShopInfo> GetShopList(ShopQuery query)
        {

            var db = DbFactory.Default.Get<ShopInfo>().LeftJoin<VShopInfo>((s, v) => s.Id == v.ShopId)
                .Where<ShopInfo>(item => item.GradeId > 0 && item.ShopStatus == ShopInfo.ShopAuditStatus.Open)
                .Where<VShopInfo>(v => v.State == VShopInfo.VshopStates.Normal).Select<ShopInfo>();
        ...
        public void SetShopHiChat(long id)
        {
            DbFactory.Default.Set<ShopInfo>().Where(p => p.Id == id).Set(p => p.IsOpenHiChat, true).Succeed();
        }
        ...
        public void OrderPaySendMsgToShop(long shopId, string sendCon)
        {
            var messagePlugin = PluginsManagement.GetPlugin<IMessagePlugin>("Himall.Plugin.Message.SMS");

            var shop = GetShop(shopId);
            if (shop == null || !shop.OrderPayIsSendSMS)
            {
                return;
                // throw new HimallException("店铺未开启订单支付发送短信提醒功能！");
            }
            List<string> phones = new List<string>();
            if (messagePlugin.Biz.CheckDestination(shop.ContactsPhone))
            {
                phones.Add(shop.ContactsPhone);
            }

            if (phones.Count == 0)
            {
                throw new HimallException("店铺手机号码错误！");
            }
            var siteName = ServiceProvider.Instance<SiteSettingService>.Create.SiteSettings.SiteName;
            ...
            ServiceProvider.Instance<WXMsgTemplateService>.Create.AddSendRecord(sendRecord);
        }
    }
}
