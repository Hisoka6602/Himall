...
namespace Himall.Service
{
    public class NavigationService : ServiceBase
    {
        ...
        public void AddSellerNavigation(BannerInfo model)
        {
            ...
            if (model.Platform == Himall.Core.PlatformType.WeiXin && DbFactory.Default.Get<BannerInfo>().Where(item => item.ShopId == model.ShopId && item.Platform == PlatformType.WeiXin).Count() >= 5)
