...
namespace Himall.Service
{
    ...
    /// 订单和销售统计
    /// </summary>
    public class OrderAndSaleStatisticsService : ServiceBase
    {
        /// <summary>
        /// 获取会员沉睡日
        /// </summary>
        ...
        /// <param name="categoryId"></param>
        /// <param name="userId"></param>
        public void SynchronizeMemberBuyCategory(long categoryId, long userId)
        {
            //如果有记录删除，重新添加
            var model = GetMemberBuyCategory(categoryId, userId);
            if (model != null)
            {
                model.OrdersCount += 1;
                UpdateMemberBuyCategory(model);
            }
            else
                AddMemberBuyCategory(new Entities.MemberBuyCategoryInfo() { CategoryId = categoryId, UserId = userId, OrdersCount = 1 });
        }
