...
namespace Himall.Service
{
    public class ManagerService : ServiceBase
    {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (model != null)
                    {
                        ...
                        int intMarketing = (int)Himall.CommonModel.AdminPrivilege.Marketing;//营销中心值
        ...
        /// <returns></returns>
        public List<ManagerInfo> GetManagers(List<string> usernames)
        {
            var sql = DbFactory.Default.Get<ManagerInfo>();
            if (usernames == null || usernames.Count == 0) return null;
            sql.Where(item => item.UserName.ExIn(usernames));
            return sql.ToList();
        }
        public ManagerInfo Login(string username, string password, bool isPlatFormManager = false)
        {
            ...
            if (isPlatFormManager)
                ...
                manager = DbFactory.Default.Get<ManagerInfo>().Where(item => item.UserName == username && item.ShopId != 0).FirstOrDefault();
            //获取不到管理员信息，记录一下登录的用户名和实际使用的用户名
            if (manager == null)
            {
                Log.Info("获取管理员信息失败,可能是联系方式表数据不正确，登录的用户名：" + loginUserName + ",使用的用户名：" + username);
            }
            if (manager != null)
            {
#if !DEBUG
             
                           var ser_site = ServiceProvider.Instance<SiteSettingService>.Create;
                            Dictionary<string, string> siteconfig = new Dictionary<string, string>();
                            siteconfig.Add("IsOpenStore", "true");
                            siteconfig.Add("IsOpenShopApp", "true");
                            siteconfig.Add("IsOpenApp", "true");
                            siteconfig.Add("IsOpenH5", "true");
                            siteconfig.Add("IsOpenMallSmallProg", "true");
                            siteconfig.Add("IsOpenPC", "true");
                            siteconfig.Add("IsOpenCopyRight", "true");
                            ser_site.SaveSettings(siteconfig);
                }
#endif
                string encryptedWithSaltPassword = GetPasswrodWithTwiceEncode(password, manager.PasswordSalt);
                if (encryptedWithSaltPassword.ToLower() != manager.Password && manager.Password != password)
                {//比较密码是否一致
                    Log.Info("密码不一致,encryptedWithSaltPassword:" + encryptedWithSaltPassword.ToLower() + ",ManagerPassword:" + manager.Password + ",Password:" + password);
                ...
                else//一致，则表示登录成功，更新登录时间
                {
                    ...
                    {
                        var shop = ServiceProvider.Instance<ShopService>.Create.GetShop(manager.ShopId);
                        if (shop == null)
                        ...
                        if (!shop.IsSelf)//只处理非官方店铺
                        {
                            ...
                            {
                                Log.Info("帐户所在店铺已被冻结:(shopId:" + shop.Id + ")");
        ...
        public ManagerInfo AddSellerManager(string username, string password, string salt = "")
        {
            ...
            {
                ShopInfo shopInfo = ServiceProvider.Instance<ShopService>.Create.CreateEmptyShop();
