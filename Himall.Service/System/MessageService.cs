...
namespace Himall.Service
{
    public class MessageService : ServiceBase
    {
        ...
        /// 使用订阅消息
        /// </summary>
        private static bool IsSubscribeMessage = true;
        public void UpdateMemberContacts(MemberContactInfo info)
        {
            ...
            CacheManager.ClearMemberData(info.UserId); //清用户缓存
        }
        ...
        public void SendMessageCode(string destination, string pluginId, Core.Plugins.Message.MessageUserInfo info)
        {
            try
            {
                ...
                if (messagePlugin == null || messagePlugin.Biz == null)
                {
                    Log.Error("获取插件" + pluginId.ToLower() + "异常");
                    throw new HimallException("获取插件" + pluginId.ToLower() + "异常");
                }
                if (string.IsNullOrEmpty(destination) || !messagePlugin.Biz.CheckDestination(destination))
                    throw new HimallException(pluginId.ToLower().Contains("sms") ? "手机" : "邮箱" + "错误");
            ...
            {
                Log.Error(string.Format("destination:{0},pluginId:{1}, info:{2}", destination, pluginId, info == null ? "null" : info.CheckCode));
                Log.Error(ex);
                throw ex;
            }
        }
        ...
        public void SendMessageOnOrderCreate(long userId, MessageOrderInfo info)
        {
            ...
            else
            {
                ...
                if (userinfo != null)
                {
                    string strName = !string.IsNullOrEmpty(userinfo.Nick) ? userinfo.Nick : userinfo.CellPhone;
                    if (string.IsNullOrEmpty(strName))
                        strName = userinfo.UserName;

                    var msgdata = new WX_MsgTemplateSendDataModel();
                    msgdata.first.value = "尊敬的会员（" + strName + "），您还有订单未成功付款，尽快支付别让好货错过！";
        ...
        public void SendMessageOnOrderPay(long userId, MessageOrderInfo info)
        {
            ...
            if (info.MsgOrderType == MessageOrderType.Applet)
            {
                #region 小程序模板它没有支付成功模板消息，则下面代码后面注释
            ...
            else
            {
                ...
                if (userinfo != null)
                {
                    ...
                    var msgdata = new WX_MsgTemplateSendDataModel();
                    msgdata.first.value = "尊敬的会员（" + strName + "），您的订单已支付成功，我们会尽快为您发货。";
                    msgdata.first.color = "#000000";
        ...
        public void SendMessageOnOrderRefund(long userId, MessageOrderInfo info, long refundid = 0, string refundPayTypeName = "")
        {
            ...
            if (info.MsgOrderType == MessageOrderType.Applet)
            {
                ...
                if (userinfo)
                {
                    ...
                    {
                        #region 订阅消息
                        var formmodel = _iwxtser.GetWXAppletFromDataById(MessageTypeEnum.OrderRefundFail, info.OrderId.ToString());
                        string templateId = "";
                        if (formmodel != null)
                        {
                            templateId = formmodel.FormId;
                        }
                        else
                        {
                            var templateInfo = _iwxtser.GetWXAppletMsgTemplate(MessageTypeEnum.OrderRefundFail);
                            templateId = templateInfo != null ? templateInfo.TemplateId : "";
                        }

                        if (!string.IsNullOrEmpty(templateId))
                        {
                            var page = _iwxtser.GetWXAppletMessageTemplateShowUrl(MessageTypeEnum.OrderRefundFail);//小程序跳转地址
                            page = page.Replace("{id}", info.OrderId.ToString());
                            if (refundid > 0)
                            {
                                page = page.Replace("{id}", refundid.ToString());
                            }
                            Log.Info("小程序退款成功url=" + page);
                            var data = new Dictionary<string, string>
                            {
                                { "thing6", "退款成功，已退款"+info.RefundMoney.ToString("F2") + "元" },
                                { "character_string4", info.OrderId },
                                { "thing5", info.ProductName.Length>20?info.ProductName.Substring(0,17)+"...":info.ProductName }
                            };

                            if (!string.IsNullOrEmpty(templateId))
                            {
                                Log.Info("退款成功提醒订阅消息模板ID为：" + templateId);
                                _iwxtser.SendSubscribeMessage(openId, templateId, page, data);
                                _iwxtser.RemoveSubscribeMessage(info.OrderId, templateId, MessageTypeEnum.OrderRefundFail);
                            }
                        }
                        else
                        {
                            Log.Info("退款成功提醒小程序 模板ID为空 订单号：" + info.OrderId + "MessageTypeEnum.OrderRefundFail:" + (int)MessageTypeEnum.OrderRefundFail);
                        }
                        #endregion
                    }
                }
            ...
            else
            {
                ...
                if (userinfo != null)
                {
                    ...
                    var msgdata = new WX_MsgTemplateSendDataModel();
                    msgdata.first.value = "尊敬的会员(" + strName + ")，您的订单已完成退款，" + info.RefundMoney.ToString("F2") + "已经退回您的付款账户（或预存款账户），请留意查收。";
                    msgdata.first.color = "#000000";
                    msgdata.keyword1.value = info.OrderId.ToString();
                    msgdata.keyword1.color = "#000000";
        ...
        public void SendMessageOnRefundDeliver(long userId, MessageOrderInfo info, long refundid = 0)
        {
            ...
            if (userinfo != null)
            {
                ...
                var msgdata = new WX_MsgTemplateSendDataModel();
                msgdata.first.value = "尊敬的会员(" + strName + ")，您的订单(" + info.OrderId + ")退货已审核通过，请及时发货。";
                msgdata.first.color = "#000000";
                msgdata.keyword1.value = info.OrderId + "";
                msgdata.keyword1.color = "#FF0000";
                msgdata.keyword2.value = info.ProductName + "";
        ...
        public void SendMessageOnOrderRefundFail(long userId, MessageOrderInfo info, int refundMode, long refundid = 0)
        {
            ...
            if (info.MsgOrderType == MessageOrderType.Applet)
            {
                ...
                if (userinfo)
                {
                    ...
                    {
                        #region 订阅消息
                        formmodel = _iwxtser.GetWXAppletFromDataById(MessageTypeEnum.OrderRefundFail, info.OrderId.ToString());
                        string templateId = "";
                        if (formmodel != null)
                        {
                            templateId = formmodel.FormId;
                        ...
                        if (!string.IsNullOrEmpty(templateId))
                        {
                            ...
                            Log.Info("退款拒绝提醒订阅消息模板ID为：" + templateId);
                            _iwxtser.SendSubscribeMessage(openId, templateId, page, data);
                            _iwxtser.RemoveSubscribeMessage(info.OrderId, templateId, MessageTypeEnum.OrderRefundFail);
                        }
                        else
                        {
                            Log.Info("退款拒绝提醒小程序 模板ID为空 订单号：" + info.OrderId + "MessageTypeEnum.OrderRefundFail:" + (int)MessageTypeEnum.OrderRefundFail);
                        }
                        #endregion
                    }
                }
            ...
            else
            {
                ...
                if (userinfo != null)
                {
                    ...
                    if (refundStatus == MessageTypeEnum.OrderRefundFail)
                    {
                        msgdata.first.value = "尊敬的会员(" + strName + ")，您的订单退款申请被拒绝，请知悉。";
                        msgdata.first.color = "#000000";
                        msgdata.keyword1.value = info.OrderId.ToString();
                        msgdata.keyword1.color = "#000000";
                        msgdata.keyword2.value = info.ProductName;
                    ...
                    {
                        msgdata.first.value = "尊敬的会员(" + strName + ")，您的订单退货申请被拒绝，请知悉。";
                        msgdata.first.color = "#000000";
        ...
        public void SendMessageOnOrderShipping(long userId, MessageOrderInfo info)
        {
            ...
            if (info.MsgOrderType == MessageOrderType.Applet)
            {
                ...
                if (userinfo)
                {
                    ...
                    {
                        #region 订阅消息
                        WXAppletFormDataInfo formmodel = null;
                        var orderInfo = Himall.ServiceProvider.Instance<OrderService>.Create.GetOrder(long.Parse(info.OrderId));
                        if (orderInfo != null)
                        {
                            Dictionary<string, string> data = null;
                            //快递配送
                            if (orderInfo.DeliveryType == DeliveryType.Express)
                            {
                                formmodel = _iwxtser.GetWXAppletFromDataById(MessageTypeEnum.OrderShipping, info.OrderId.ToString());
                                if (formmodel != null)
                                {
                                    data = new Dictionary<string, string>
                                    {
                                        { "character_string1", info.OrderId },
                                        { "thing2", info.ProductName.Length>20?info.ProductName.Substring(0,17)+"...":info.ProductName },
                                        { "thing7", info.ShippingCompany },
                                        { "character_string4", info.ShippingNumber },
                                        { "thing6", "商家已安排发货，请注意查收" }
                                    };
                                }
                                else
                                {
                                    Log.Info("快递订单发货提醒小程序 模板ID为空 订单号：" + info.OrderId + "MessageTypeEnum.OrderShipping:" + (int)MessageTypeEnum.OrderShipping);
                                    //如果快递配送模板不存在，则使用门店配送模板
                                    formmodel = _iwxtser.GetWXAppletFromDataById(MessageTypeEnum.ShopOrderShipping, info.OrderId.ToString());
                                    if (formmodel != null)
                                    {
                                        data = new Dictionary<string, string>
                                        {
                                            { "character_string1", info.OrderId },
                                            { "thing2", info.ProductName.Length>20?info.ProductName.Substring(0,17)+"...":info.ProductName },
                                            { "date5", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") },
                                            { "thing8", "您的订单门店已安排发货，请注意查收" }
                                        };
                                    }
                                }
                            }
                            else
                            {
                                ...
                                {
                                    Log.Info("配送订单发货提醒小程序 模板ID为空 订单号：" + info.OrderId + "MessageTypeEnum.ShopOrderShipping:" + (int)MessageTypeEnum.ShopOrderShipping);
                                }
                            }
                            if (formmodel != null)
                            {
                                string templateId = formmodel.FormId;
                                if (!string.IsNullOrEmpty(templateId))
                                {
                                    Log.Info("发货提醒订阅消息模板ID为：" + templateId);
                                    _iwxtser.SendSubscribeMessage(openId, templateId, page, data);
                                    _iwxtser.RemoveSubscribeMessage(info.OrderId, templateId, MessageTypeEnum.OrderShipping);
                                }
                            }
                        }
                        #endregion
            ...
            else
            {
                ...
                {
                    var msgdata = new WX_MsgTemplateSendDataModel();
                    msgdata.first.value = "尊敬的" + userinfo.Nick + "，宝贝已启程了，好想快点来到你身边。";
                    msgdata.first.color = "#000000";
                    msgdata.keyword1.value = info.OrderId.ToString();
                    msgdata.keyword1.color = "#000000";
        ...
        public void SendMessageOnSelfTakeOrderPay(long userId, MessageOrderInfo info)
        {
            ...
            if (info.MsgOrderType == MessageOrderType.Applet)
            {
                ...
                if (userinfo != null)
                {
                    ...
                    {
                        #region 订阅消息
                        Log.Info("自提订单支付成功订阅消息逻辑处理在：WXMsgTemplateService.AuthorizedSubscribeMessage()");
                        #endregion
                    }
                }
            }
            else
            {
                ...
                if (userinfo != null)
                {
                    ...
                    var msgdata = new WX_MsgTemplateSendDataModel();
                    msgdata.first.value = "尊敬的会员(" + strName + ")，您的自提订单已支付成功。";
                    msgdata.first.color = "#000000";
                    msgdata.keyword1.value = info.OrderId + "";
                    msgdata.keyword1.color = "#FF0000";
        ...
        public void SendMessageOnAlreadyVerification(long userId, MessageOrderInfo info)
        {
            ...
            if (userinfo != null)
            {
                ...
                var msgdata = new WX_MsgTemplateSendDataModel();
                msgdata.first.value = "尊敬的会员（" + strName + "），您的订单已成功核销。";
                msgdata.first.color = "#000000";
        ...
        public void SendMessageOnMemberWithDrawApply(long userId, MessageWithDrawInfo info)
        {
            ...
            if (userinfo != null)
            {
                ...
                var msgdata = new WX_MsgTemplateSendDataModel();
                msgdata.first.value = "尊敬的会员(" + strName + ")，您的提现申请正在受理中。";
                msgdata.first.color = "#000000";
        ...
        public void SendMessageOnMemberWithDrawSuccess(long userId, MessageWithDrawInfo info)
        {
            ...
            if (userinfo != null)
            {
                ...
                var msgdata = new WX_MsgTemplateSendDataModel();
                msgdata.first.value = "尊敬的会员(" + strName + ")，您申请的提现已经到帐了，请注意查收。";
                msgdata.first.color = "#000000";
        ...
        public void SendMessageOnMemberWithDrawFail(long userId, MessageWithDrawInfo info)
        {
            ...
            if (userinfo != null)
            {
                ...
                var msgdata = new WX_MsgTemplateSendDataModel();
                msgdata.first.value = "尊敬的会员" + strName + "，您的提现申请被拒绝了。";
                msgdata.first.color = "#000000";
                msgdata.keyword1.value = info.Amount.ToString("F2");
                msgdata.keyword1.color = "#FF0000";
                msgdata.keyword2.value = info.ApplyTime.ToString("yyyy-MM-dd HH:mm:ss");
        ...
        public void SendMessageOnShopDelivering(MessageOrderInfo info)
        {
            ...
            {
                var msgdata = new WX_MsgTemplateSendDataModel();
                msgdata.first.value = "尊敬的商家(" + info.ShopName + ")，您有一个售后申请待处理。";
                msgdata.first.color = "#000000";
                msgdata.keyword1.value = info.OrderId.ToString();
                msgdata.keyword1.color = "#FF0000";
                msgdata.keyword2.value = info.ProductName + "...";
        ...
        public void SendMessageOnShopOrderShipping(MessageOrderInfo info)
        {
            ...
            foreach (var shopId in info.ShopIds)
            {
                ...
                {
                    var msgdata = new WX_MsgTemplateSendDataModel();
                    msgdata.first.value = "尊敬的商家(" + shop.ShopName + ")，您有一个新的待发货订单。";
                    msgdata.first.color = "#000000";
                    msgdata.keyword1.value = orderIds[i];
                    msgdata.keyword1.color = "#000000";
                    //msgdata.keyword2.value = info.TotalMoney.ToString();
                    msgdata.keyword2.value = (info.ShopTotalMoney == null ? info.TotalMoney : info.ShopTotalMoney[shopId]).ToString();
                    msgdata.keyword2.color = "#FF0000";
        ...
        public void SendMessageOnEditPayPassWord(long userId, MessageUserInfo info)
        {
            ...
            if (userinfo != null)
            {
                ...
                var msgdata = new WX_MsgTemplateSendDataModel();
                msgdata.first.value = "尊敬的会员(" + strName + ")，您的帐号交易密码修改成功。";
                msgdata.first.color = "#000000";
        ...
        public void SendMessageOnEditLoginPassWord(long userId, MessageUserInfo info)
        {
            ...
            if (userinfo != null)
            {
                ...
                var msgdata = new WX_MsgTemplateSendDataModel();
                msgdata.first.value = "尊敬的会员(" + strName + ")，您的帐号登录密码修改成功。";
                msgdata.first.color = "#000000";
                msgdata.keyword1.value = info.UserName;
                msgdata.keyword1.color = "#FF0000";
                msgdata.keyword2.value = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
