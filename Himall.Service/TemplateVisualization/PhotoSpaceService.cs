...
{
    public class PhotoSpaceService : ServiceBase
    {
        ...
        public void DeletePhoto(IEnumerable<long> Ids) {
            DbFactory.Default.Del<PhotoSpaceInfo>().Where(p=>p.Id.ExIn(Ids)).Succeed();
        }

        public List<PhotoSpaceInfo> GetPhotoSpaceByCategoryId(long categoryId) {
           return DbFactory.Default.Get<PhotoSpaceInfo>().Where(p => p.PhotoCategoryId == categoryId).ToList() ;
        }
