...
{
    public class VShopService : ServiceBase
    {
        public QueryPageModel<VShopInfo> GetVShopByParamete(VshopQuery vshopQuery)
        {
            ...
            if (!string.IsNullOrEmpty(vshopQuery.Name))
            {
                ...
                var shoplist = DbFactory.Default.Get<ShopInfo>().Where(p => p.ShopName.Contains(vshopQuery.Name)).Select(s => s.Id);
                vshops.Where(a => a.Name.Contains(vshopQuery.Name) || a.ShopId.ExIn(shoplist));
            ...
            var shopenty = DbFactory.Default.Get<ShopInfo>().Where(t => t.Id.ExIn(rets.Select(a => a.ShopId))).ToList();
            rets.ForEach(item =>
            {
                item.ShopAccount = shopenty.Where(t => t.Id == item.ShopId).FirstOrDefault().ShopName;
            });
        ...
        public VShopInfo GetTopShop()
        {
            ...
            var shopsql = DbFactory.Default.Get<ShopInfo>().Where(s => s.ShopStatus == ShopInfo.ShopAuditStatus.Open).Select(s => s.Id);
            return DbFactory.Default
                .Get<VShopInfo>()
                .Where(a => a.State != VShopInfo.VshopStates.Close && a.ExExists(vseisql) && a.ShopId.ExIn(shopsql)).FirstOrDefault();
        }
        ...
        public VShopInfo GetVshopDataByShopId(long shopId) =>
            CacheManager.GetVShop(shopId, () => DbFactory.Default.Get<VShopInfo>().Where(item => item.ShopId == shopId).FirstOrDefault());
        ...
        public void CreateVshop(VShopInfo vshopInfo)
        {
            ...
            var shopInfo = ServiceProvider.Instance<ShopService>.Create.GetShop(vshopInfo.ShopId);
        ...
        public void UpdateVShop(VShopInfo vshopInfo)
        {
            ...
            if (!string.IsNullOrWhiteSpace(vshopInfo.Tags))
            {
                ...
                {
                    if (tag.Length > 4)
                        throw new Himall.Core.InvalidPropertyException("每个标签限4个字");
                }
            }
        ...
        public List<VShopInfo> GetVShops(int page, int pageSize, out int total, VShopInfo.VshopStates state, bool? IsOpenVshop)
        {
            var shops = DbFactory.Default
             .Get<ShopInfo>()
             .Where(n => n.ShopStatus == ShopInfo.ShopAuditStatus.Open).Select(s => s.Id);
            var vshop = DbFactory.Default.Get<VShopInfo>().Where(item => item.State == state && item.ShopId.ExIn(shops));
        ...
        public List<VShopInfo> GetHotShops(int page, int pageSize, out int total, long shopcategoryId = 0)
        {
            var vshopex = DbFactory.Default
                        .Get<VShopExtendInfo>()
            ...
            var vshops = DbFactory.Default
                .Get<VShopInfo>()
                .Where(n => n.State == VShopInfo.VshopStates.Normal && n.Id.ExIn(vshopex)).Select();

            var shops = DbFactory.Default
               .Get<ShopInfo>()
               .Where(n => n.ShopStatus == ShopInfo.ShopAuditStatus.Open).Select(s => s.Id);

            vshops.Where(v => v.ShopId.ExIn(shops));

            if (shopcategoryId > 0)
            {
                var categoryIds = Himall.ServiceProvider.Instance<CategoryService>.Create.GetSecondAndThirdLevelCategories(shopcategoryId).Select(c => c.Id);


                var busvshop = DbFactory.Default.Get<BusinessCategoryInfo>().Where(c => c.CategoryId.ExIn(categoryIds)).Select(p => p.ShopId).Distinct();

                vshops.Where(t => t.ShopId.ExIn(busvshop));
            }

            var rets = vshops.Select(n => new { Sequence = sequencesql.ExResolve<int>() }).OrderBy(d => "Sequence").OrderByDescending(item => item.CreateTime).ToPagedList(page, pageSize);
            total = rets.TotalRecordCount;
            return rets;
        }
