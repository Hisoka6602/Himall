...
{
    public class WXCardService : ServiceBase
    {
        #region 私有
        /// <summary>
        /// 库存最大数 1000000
        ...
        public bool Add(Entities.WXCardLogInfo info)
        {
            ...
            if (wxlogo.IndexOf("http://") == -1 && wxlogo.IndexOf("https://") == -1)
            {
                wxlogo = siteSetting.SiteUrl + imagefile;
            }
            var shopser = ServiceProvider.Instance<ShopService>.Create;
            ...
            Card_BaseInfoBase _BaseInfo = new Card_BaseInfoBase()
            {
                ...
                date_info = new Card_BaseInfo_DateInfo()
                {
                    ...
                    begin_timestamp = DateTimeHelper.GetUnixDateTime(info.BeginTime),
                    end_timestamp = DateTimeHelper.GetUnixDateTime(info.EndTime),
                },
            ...
            if (info.ShopId > 0)
            {
                //_BaseInfo.custom_url = siteSetting.SiteUrl + "/Shop/Home/" + info.ShopId.ToString();
            }
            ...
            if (vshopSetting != null && shopinfo != null && vshopinfo != null)
            {
                if (!string.IsNullOrWhiteSpace(vshopSetting.AppId) && !string.IsNullOrWhiteSpace(vshopSetting.AppSecret))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(acctoken))
                    {
                        ...
                        {
                            wxlogo = Core.HimallIO.GetImagePath(vshopinfo.WXLogo);
            ...
            */
            ...
            if (!issuccess)
            {
                ...
                if (!string.IsNullOrWhiteSpace(siteSetting.WeixinAppId) && !string.IsNullOrWhiteSpace(siteSetting.WeixinAppSecret))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(acctoken))
                    {
                        _BaseInfo.brand_name = shopinfo != null ? shopinfo.ShopName : "平台";
                        if (!string.IsNullOrWhiteSpace(siteSetting.WXLogo))
                        {
                            wxlogo = Core.HimallIO.GetImagePath(siteSetting.WXLogo);
                            _BaseInfo.logo_url = wxlogo;
                        }
                        wxResultJson = CardApi.CreateCard(acctoken, cardData);
                        if (wxResultJson.errcode == Senparc.Weixin.ReturnCode.请求成功)
                ...
                if (info.ShopId < 1)
                {
                    //_BaseInfo.custom_url = siteSetting.SiteUrl + "/";
                }
                #endregion
            }
            ...
            if (!string.IsNullOrWhiteSpace(cardid))
            {
                ...
                if (card != null)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(apiticket))
                    {
                        ...
                        result.timestamp = JSSDKHelper.GetTimestamp();
                        result.nonce_str = "";
                        result.signature = JSSDKHelper.GetSignature(apiticket, result.nonce_str, result.timestamp, result.card_id);
        ...
        public void Event_Audit(string cardid, Entities.WXCardLogInfo.AuditStatusEnum auditstatus)
        {
            ...
            if (card != null)
            {
                ...
                {
                    case Entities.WXCardLogInfo.CouponTypeEnum.Coupon:
                        var couponser = ServiceProvider.Instance<CouponService>.Create;
                    ...
                    case Entities.WXCardLogInfo.CouponTypeEnum.Bonus:
                        var shopbonusser = ServiceProvider.Instance<ShopBonusService>.Create;
