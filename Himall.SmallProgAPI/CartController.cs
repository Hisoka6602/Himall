...
namespace Himall.SmallProgAPI
{
    public class CartController : BaseApiController
    {
        ...
        public JsonResult<Result<int>> GetAddToCart(string openId, string SkuID, int Quantity, int GiftID = 0, long roomId = 0)
        {
            //验证用户
            CheckUserLogin();
            ...
            try
            {
                cartHelper.AddToCart(SkuID, Quantity, userId, roomId);
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public JsonResult<Result<dynamic>> GetUpdateToCart(string openId, string SkuID, int Quantity, int GiftID = 0)
        {
            ...
            if (isOpenLadder)
            {
                ...
                //    var skuInfo = ServiceProvider.Instance<ProductService>.Create.GetSku(c.SkuId);
                //    if (skuInfo != null)
                //        cItem = c;
                //    return cItem;
        ...
        public JsonResult<Result<dynamic>> GetCartProduct(string openId = "")
        {
            CheckUserLogin();
            ...
            foreach (var item in cartItems)
            {
                ...
                var picimg = Core.HimallIO.GetRomoteProductSizeImage(product.RelativePath, 1, (int)ImageSize.Size_150);
                if (!string.IsNullOrEmpty(sku.ShowPic)) {
                    picimg = HimallIO.GetRomoteImagePath(sku.ShowPic);
                }
                var _tmp = new CartProductModel
                {
                    ...
                    Url = Core.HimallIO.GetRomoteImagePath("/m-IOS/product/detail/") + product.Id,
            ...
            var typeservice = ServiceProvider.Instance<TypeService>.Create;
            var shopBranchService = ServiceProvider.Instance<ShopBranchService>.Create;
            var shopCartHelper = ServiceProvider.Instance<BranchCartService>.Create;
            ...
            return JsonResult<dynamic>(new { Shop = cartShop, ShopBranch = sbCartShop });
        }


        private List<CartProductModel> PackageCartProducts(Himall.Entities.ShoppingCartInfo cart, decimal prodPrice, decimal discount, ProductService productService, ShopService shopService, ShopBranchService shopBranchService, VShopService vshopService, TypeService typeservice, bool isBranch = false)
        {
            List<CartProductModel> products = new List<CartProductModel>();
            ...
            foreach (var item in cart.Items)
            {
                ...
                if (null != shop)
                {
                    ...
                    if (sku != null)
                    {
                        ...
                        string picimg = HimallIO.GetRomoteProductSizeImage(product.RelativePath, 1, (int)Himall.CommonModel.ImageSize.Size_150);
                        if (!string.IsNullOrEmpty(sku.ShowPic)) {
                            picimg = HimallIO.GetRomoteImagePath(sku.ShowPic);
                        }
                        var _tmp = new CartProductModel
        ...
        private int GetCartProductQuantity(Himall.Entities.ShoppingCartInfo cartInfo, long productId = 0, string skuId = "")
        {
            int cartQuantity = 0;
            if (cartInfo == null)
            {
