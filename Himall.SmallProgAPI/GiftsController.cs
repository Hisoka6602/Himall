...
namespace Himall.SmallProgAPI
{
    public class GiftsController: BaseApiController
    {
        ...
        public JsonResult<Result<dynamic>> GetIndexData(string openId = "")
        {
            ...
            query.skey = "";
            query.status = Himall.Entities.GiftInfo.GiftSalesStatus.Normal;
            query.PageSize = 4;
            query.PageNo = 1;
        ...
        /// </summary>
        /// <returns></returns>
        public JsonResult<Result<dynamic>> GetGiftDetail(long id)
        {
            ...
            if (result.CanBuy)
            {
                if (result.GetSalesStatus != Himall.Entities.GiftInfo.GiftSalesStatus.Normal)
                {
                    result.CanBuy = false;
        ...
        /// <returns></returns>
        public JsonResult<Result<bool>> GetCanBuy(long id, int count)
        {
            CheckUserLogin();
            var userInte = MemberIntegralApplication.GetMemberIntegral(CurrentUser.Id);
            //礼品信息
            ...
            if (giftdata.GetSalesStatus != Himall.Entities.GiftInfo.GiftSalesStatus.Normal)
            {
            ...
            return JsonResult<bool>(true, msg: "可以购买！", code: 1);
        }
        ...
        /// <param name="count"></param>
        /// <returns></returns>
        public JsonResult<Result<dynamic>> GetConfirmOrder(long id, int count, long? regionId = null)
        {
            CheckUserLogin();
            if (count < 1)
            {
        ...
        /// <param name="count"></param>
        /// <returns></returns>
        public JsonResult<Result<bool>> PostSubmitOrder(GiftConfirmOrderModel value)
        {
            CheckUserLogin();
            long id = value.ID;
            ...
            return JsonResult<bool>(true, msg: orderdata.Id.ToString(), code: 1);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public JsonResult<Result<dynamic>> GetOrderCount()
        {
            CheckUserLogin();
            dynamic result = new System.Dynamic.ExpandoObject();
            long curid = CurrentUser.Id;
            result.AllCount = GiftsOrderApplication.GetOrderCount(null, curid);
            result.WaitDeliveryCount = GiftsOrderApplication.GetOrderCount(Himall.Entities.GiftOrderInfo.GiftOrderStatus.WaitDelivery, curid);
            result.WaitReceivingCount = GiftsOrderApplication.GetOrderCount(Himall.Entities.GiftOrderInfo.GiftOrderStatus.WaitReceiving, curid);
            result.FinishCount = GiftsOrderApplication.GetOrderCount(Himall.Entities.GiftOrderInfo.GiftOrderStatus.Finish, curid);
            return JsonResult<dynamic>(result);
        }
        ...
        /// <returns></returns>
        public JsonResult<Result<dynamic>> GetGiftOrderDetail(long id)
        {
            CheckUserLogin();
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public JsonResult<Result<bool>> PostConfirmOrderOver(GiftConfirmOrderOver value)
        {
            CheckUserLogin();
            ...
            return JsonResult<bool>(msg: "订单完成", code: 1);
        }
        ...
        /// <param name="orderId"></param>
        /// <returns></returns>
        public JsonResult<Result<dynamic>> GetGiftExpressInfo(long orderId)
        {
            CheckUserLogin();
            ...
            {
                var expressData = ExpressApplication.GetExpressData(order.ExpressCompanyName, order.ShipOrderNumber, order.Id.ToString(), order.CellPhone);
