...
namespace Himall.SmallProgAPI
{
    ...
    public class OrderRefundController : BaseApiController
    {
            ...
            result.Data = refunds.Models.Select(refund =>
            {
                ...
                return new OrderRefundItem
                {
                    ...
                    LineItems = orderItems.Select(d => new OrderRefundSku
                    {
                        ...
                        Quantity = d.Quantity,
                        Image = d.ThumbnailsUrl.Contains("skus") ? HimallIO.GetRomoteImagePath(d.ThumbnailsUrl) : Core.HimallIO.GetRomoteProductSizeImage(d.ThumbnailsUrl, 1, (int)Himall.CommonModel.ImageSize.Size_150),
        ...
        public JsonResult<Result<RefundDetail>> GetRefundDetail(string openId, long RefundId)
        {
            ...
            {
                orderItems = _OrderService.GetOrderItemsByOrderId(order.Id);
            }
            else
            {
            ...
            var result = new RefundDetail()
            {
                ...
                IsOrderRefundTimeOut = _OrderService.IsRefundTimeOut(refundinfo.OrderId),
            ...
            ThumbnailsUrl =l.ThumbnailsUrl.Contains("skus")? HimallIO.GetRomoteImagePath(l.ThumbnailsUrl):Core.HimallIO.GetRomoteProductSizeImage(l.ThumbnailsUrl, 1, (int)Himall.CommonModel.ImageSize.Size_150),
                }).ToList()
            };
            ...
            try
            {
                ...
                if (order.OrderType == OrderInfo.OrderTypes.Virtual)
                {
                    ...
                    if (item.EnabledRefundAmount.HasValue)
                    {
                        ...
                        result.MaxRefundAmount = result.MaxRefundAmount > (item.EnabledRefundAmount.Value - item.RefundPrice) ? (item.EnabledRefundAmount.Value - item.RefundPrice) : result.MaxRefundAmount;
                    }
                }
