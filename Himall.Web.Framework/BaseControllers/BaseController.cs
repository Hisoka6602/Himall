...
namespace Himall.Web.Framework
{
    ...
    public abstract class BaseController : Controller
    {
        ...
        {
            string curUrl = WebHelper.GetPathAndQuery();
            string jumpUrl = curUrl.ToLower(); ;
            ...
            if (this.IsAutoJumpMobile && IsMobileTerminal)
            {
                if (Regex.Match(curUrl, @"\/m(\-.*)?").Length < 1)
                {
                    ...
                    #region 移动端和pc端页面 特殊页对应不是同路径，转变为移动端指定页面
                    if (jumpUrl.IndexOf("/search/searchad?") != -1)
                    {
                        jumpUrl = jumpUrl.ToLower().Replace("/search/searchad?", "/search/?");
        ...
        protected override void OnException(ExceptionContext filterContext)
        {
            ...
            if (innerEx is HttpRequestException)
            {
                ...
                result.Content += "<script>$(function(){$.dialog.errorTips('地址包含了特殊字符！',function(){window.history.back(-1)},2);});</script>";
                filterContext.Result = result;
            ...
            if (IsInstalled())
            {
                ...
                if (area == "mobile" || visitorTerminalInfo.Terminal == EnumVisitorTerminal.WeiXin || visitorTerminalInfo.Terminal == EnumVisitorTerminal.Moblie || visitorTerminalInfo.Terminal == EnumVisitorTerminal.PAD)
                {
                    ...
                    if (!allowUrl.Contains("/shopregisterjump/") && !allowUrl.Contains("/shopregister/") && !allowUrl.Contains("/topic/detail/") &&
                        !allowUrl.Contains("/bigwheel/") && !allowUrl.Contains("/scratchcard/") && !allowUrl.Contains("/payment/") &&
                        !allowUrl.Contains("/home/downloadapp") && !allowUrl.Contains("/home/weixindownload") && !allowUrl.Contains("/home/andrioddownload") && !allowUrl.Contains("/home/iosdownload") && !allowUrl.Contains("/home/downloaderror") //商家入驻，专题页，大转盘，刮刮卡，移动端支付控制器，app下载页，(小程序支付后异步处理需权限)
                        && !allowUrl.Contains("/login/registeragreement") && !allowUrl.Contains("/login/privacypolicy")
                        )
                    {
                        #region 后台或跳转地址特定页不检测h5,//sv=true：后台访问，nojumpfg=1：是跳转地址
                        bool isBackstage = ((Request.QueryString["sv"] == "True" || Request.QueryString["nojumpfg"] == "1") && allowUrl.Contains("/product/")) ? true : false;//是否后台访问商品页
                        if (!isBackstage)
                            isBackstage = (!allowUrl.Contains("/product/detail/") && !allowUrl.Contains("/product/details/") && !allowUrl.Contains("/product/getstroreinfo/"));
                        #endregion

                        if (!SiteSettings.IsOpenH5 && !isBackstage)
                        {
                ...
                {
                    if (!allowUrl.Contains("/pay/") && !allowUrl.Contains("/category/getcategory"))//支付不需验证,读取分类不需验证
                    {
                        if (!SiteSettings.IsOpenPC)
                        {
                            //如pc端没开通而移动到开通了，访问pc端地址直接跳移动到去
                            if (SiteSettings.IsOpenH5)
                            {
                                filterContext.Result = Redirect("/m-wap");
                                return;
                            }
