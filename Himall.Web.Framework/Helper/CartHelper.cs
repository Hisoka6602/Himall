...
namespace Himall.Web.Framework
{
    public class CartHelper
    {
        ...
        public void UpdateCartInfoFromCookieToServer(long memberId)
        {
            ...
            if (!string.IsNullOrWhiteSpace(cartInfo))
            {
                ...
                foreach (string cartItem in cartItems)
                {
                    ...
                    shoppingCartItems.Add(new Himall.Entities.ShoppingCartItem() { SkuId = cartItemParts[0], Quantity = int.Parse(cartItemParts[1]) });
                }
        ...
        public void AddToCart(string skuId, int count, long memberId,long roomId=0)
        {
            ...
            if (memberId > 0)
                CartApplication.AddToCart(skuId, count, memberId,roomId);
            else
            {
                string cartInfo = WebHelper.GetCookie(CookieKeysCollection.HIMALL_CART);
                if (!string.IsNullOrWhiteSpace(cartInfo))
        ...
        public void UpdateCartInfoFromCookieToServer(long memberId)
        {
            ...
            if (!string.IsNullOrWhiteSpace(cartInfo))
            {
                ...
                foreach (string cartItem in cartItems)
                {
                    ...
                    shoppingCartItems.Add(new Himall.Entities.ShoppingCartItem() { ShopBranchId = shopBranchId, SkuId = cartItemParts[0], Quantity = int.Parse(cartItemParts[1]) });
                }
