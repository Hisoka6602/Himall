...
namespace Himall.Web.Areas.Admin.Controllers
{
    ...
    public class DistributorController : BaseAdminController
    {
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult GetCanSuperDistributorList(string MemberName, long? memberId = null
            , int rows = 1, int page = 6)
        {
            //查询条件
            DistributorQuery query = new DistributorQuery();
            ...
            if (memberId.HasValue)
            {
                ...
                query.ExcludeMemberIds.Add(memberId.Value);
            }
            ...
            query.PageNo = page;
            query.IsIncludeMember = false;
            query.Status = DistributorStatus.Audited;
        ...
        public JsonResult ChangeSuper(long id, long superId)
        {
            ...
            if (id == superId)
            {
                throw new HimallException("不可以调整自己为自己的上级");
            }
            var super = DistributionApplication.GetDistributor(superId);
            if (super.SuperiorId > 0)
            {
                throw new HimallException("调整上级只可以为一级销售员");
            }
        ...
        public ActionResult Detail(long id)
        {
            ...
            if (model.Distributor == null || model.Distributor.DistributionStatus == (int)DistributorStatus.UnApply)
                throw new HimallException("错误的分销员ID");
        ...
        public JsonResult GetSubDistributorList(DistributorSubQuery query)
        {
            ...
            new
            {
                ...
                CellPhone = item.Member == null ? "" : item.Member.CellPhone,
                NickName = item.Member == null ? "" : item.Member.Nick,
