...
namespace Himall.Web.Areas.Admin.Controllers
{
    public class LoginController : BaseController
    {
        ...
        {
            _ManagerService = ManagerService; // 第三方容器.构造<T>();
        }
        ...
        public JsonResult Login(string username, string password, string checkCode, bool keeplogin)
        {
            ...
            try
            {
                ...
                var manager = _ManagerService.Login(username, password, true);
                if (manager == null)
                {
                ...
                //日龙修改
                //去除将过期店铺变为不可用店铺的功能
                //ObjectContainer.Current.Resolve<ManagerService>().UpdateShopStatus();
                if(keeplogin)
