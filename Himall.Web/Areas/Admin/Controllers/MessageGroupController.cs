...
namespace Himall.Web.Areas.Admin.Controllers
{
    public class MessageGroupController : BaseAdminController
    {
        ...
        WXMsgTemplateService _WXMsgTemplateService;

        public MessageGroupController(MemberLabelService MemberLabelService, MemberService MemberService, RegionService RegionService, WXMsgTemplateService WXMsgTemplateService)
        {
        ...
        public ActionResult WXGroupMessage()
        {
            ...
            //var topregion = _MemberService.GetAllTopRegion().Select(e => new SelectListItem
            //{
            //    Text = _RegionService.GetRegion(e).Name,
            //    Value = e.ToString()
        ...
        public JsonResult SendWXGroupMessage(string msgtype, string userdesc, string mediaid = "", string labelids = "", string sex = "", string msgcontent = "", string region = "")
        {
            ...
            if (Enum.TryParse<WXMsgType>(msgtype, out type))
            {
                ...
                var result = _WXMsgTemplateService.SendWXMsg(model);
                if (result.errCode == "0")
                {
        ...
        public JsonResult SendEmailMsg(string labelids, string title, string content, string labelinfos)
        {
            ...
            var pageMode = _MemberService.GetMembers(new MemberQuery
            {
                IsHaveEmail = true,
                Labels = lids,
                PageNo = pageNo,
            ...
            {//暂时循环处理
                ...
                pageNo += 1;
                pageMode = _MemberService.GetMembers(new MemberQuery
                {
                    IsHaveEmail = true,
                    Labels = lids,
                    PageNo = pageNo,
            ...
            _WXMsgTemplateService.AddSendRecord(sendRecord);
            return Json(new { success = true });
        }
        ...
        public JsonResult SendPhoneMsg(string labelids, string content, string labelinfos)
        {
            ...
            while (pageMode.Models.Count() > 0)
            {//暂时循环处理
                string[] dests = pageMode.Models.Where(t=> !string.IsNullOrEmpty(t.CellPhone)).Select(e => e.CellPhone).ToArray();
                if (dests!=null && dests.Count() > 0)
                {
                    messagePlugin.Biz.SendMessages(dests, content);
                }
        ...
        {
            string msg = CouponApplication.SendCouponMsg(labelids, labelinfos, couponIds, WebHelper.GetHost());
            bool success = false;
            if (msg.Equals(""))
            {
                success = true;
