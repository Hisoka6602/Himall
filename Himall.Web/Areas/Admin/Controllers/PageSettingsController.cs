...
namespace Himall.Web.Areas.Admin.Controllers
{
    ...
    public class PageSettingsController : BaseAdminController
    {
        ...
        SlideAdsService _iSlideAdsService;
        BrandService _iBrandService;
        TypeService _iTypeService;
        HomeCategoryService _iHomeCategoryService;
        CategoryService _iCategoryService;
        FloorService _iFloorService;
        ArticleCategoryService _iArticleCategoryService;
        ArticleService _iArticleService;
        ...
        public async Task<ActionResult> Save(bool use, string json, string tname, string name, int tid, string variableName, string script, string resultHtml, string footHtml, string themeJson,string keyword,string hotKeyWords)
        {
            ...
            html = html.Replace("<div class=\"j_flowScript\"></div>", (string.IsNullOrEmpty(site.FlowScript) ? "" : ("<div class=\"j_flowScript\">" + site.FlowScript + "</div>")));
            if (use && !string.IsNullOrWhiteSpace(html))
            {
                ...
                //更新底部html
                if (!string.IsNullOrEmpty(footHtml))
                {
                    fullName = this.Server.MapPath(_templateFootHtmlFileFullName);
                    using (var fs = new FileStream(fullName, FileMode.Create, FileAccess.Write))
                    {
        ...
        public JsonResult GetBrands()
        {
            var data = _iBrandService.GetBrands("").Select(item => new
            {
                ...
                item.Id,
                Logo= Himall.Core.HimallIO.GetImagePath(item.Logo),
            ...
            var result = data.OrderBy(p => p.RowNumber).Select(item =>
            {
                ...
                return new
                {
                    ...
                    .Select(p =>
                    {
                        ...
                        {
                            temp.Id,
                            Icon = Himall.Core.HimallIO.GetImagePath(temp.Icon),
                            temp.Name
                        };
                    }),
                ...
                homeFloorDetail.ProductModules = floorProducts.OrderBy(p => p.Id)
                    .Select(item =>
                    {
                        ...
                        return new HomeFloorDetail.ProductModule()
                        {
                            ...
                            productImg = HimallIO.GetProductSizeImage(product.ImagePath, 1, (int)ImageSize.Size_50),
        ...
        string TransferImage(string sourceFile, int tid, string type, int num = 0)
        {
            if (!string.IsNullOrWhiteSpace(sourceFile) && !sourceFile.Contains("/Storage/Plat/PageSettings/Template/"))
            {
                ...
                if (!string.IsNullOrWhiteSpace(sourceFile) && Core.HimallIO.ExistFile(sourceFile))
                {
                    if (sourceFile.Contains("/temp/"))
                    {
                        ...
                        sourceFile = newLogo;
                        return Core.HimallIO.GetImagePath(sourceFile);//返回新的文件路径
                    }
            ...
            return Core.HimallIO.GetImagePath(sourceFile);
        }
        ...
        {
            var topicList = ObjectContainer.Current.Resolve<TopicService>().GetTopics(new TopicQuery
            {
                PageNo = page,
                PageSize = rows,
