...
namespace Himall.Web.Areas.Admin.Controllers
{
    public class ProductController : BaseAdminController
    {
        ...
        public JsonResult List(long? categoryId, string brandName, string productCode, int? auditStatus, string ids, int page, int rows, string keyWords, string shopName, int? saleStatus, bool? isAsc, string sort = "", sbyte? productType = null)
        {
            ...
            var skus = ProductManagerApplication.GetSKUByProducts(products.Models.Select(p => p.Id).ToList());
            ...
            var list = products.Models.Select(item =>
           {
               ...
               return new ProductModel()
               {
                   ...
                   MaxPrice = skus.Where(p => p.ProductId == item.Id).Max(p => p.SalePrice),
                   ...
                   CheckDate = item.CheckTime.ToString("yyyy-MM-dd HH:mm"),
                   DisplaySequence = item.DisplaySequence,
                   VirtualSaleCounts = (int)item.VirtualSaleCounts,
                   ProductType = item.ProductType,
                   stock = skus.Where(p => p.ProductId == item.Id).Sum(p => p.Stock),
               };
           });
        ...
        public JsonResult Audit(long productId, int auditState, string message)
        {
            ...
            try
            {
                ...
                ProductManagerApplication.AuditProducts(productIds, status, message);
                #region 修改搜索商品状态
                ...
                var sitestting = SiteSettingApplication.SiteSettings;
                if (status != ProductInfo.ProductAuditStatus.AuditFailed &&
                    sitestting.OpenErp)
                {
                    List<long> proIdlist = new List<long>();
                    proIdlist.Add(productId);
                    WDTProductApplication.PushGoodsByProductIds(proIdlist);
        ...
        public JsonResult BatchAudit(string productIds, int auditState, string message)
        {
            ...
            try
            {
                ...
                ProductManagerApplication.AuditProducts(productIdsArr, status, message);
                ...
                if (status != ProductInfo.ProductAuditStatus.AuditFailed &&
                    sitestting.OpenErp)
                {
                    WDTProductApplication.PushGoodsByProductIds(productIdsArr);
                }
                result.success = true;
                result.msg = "审核成功！";
            }
