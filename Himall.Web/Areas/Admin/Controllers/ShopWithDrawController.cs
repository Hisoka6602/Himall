...
namespace Himall.Web.Areas.Admin.Controllers
{
    public class ShopWithDrawController : BaseAdminController
    {
        ...
        public JsonResult ConfirmPay(long id, int status, string remark)
        {
            ...
            {
                return Json(new { success = false, msg = "操作失败，可能证书设置错误或者扣款账号余额不足，检查完稍后再试。" });
            }
        }
        public JsonResult BatchConfirmApply(string ids, int status, string remark)
        {
            var result = BillingApplication.ShopApplyWithDrawBatch(ids, (Himall.CommonModel.WithdrawStaus)status, remark, Request.UserHostAddress, CurrentManager.UserName, webRoot: CurrentUrlHelper.CurrentUrlNoPort());
