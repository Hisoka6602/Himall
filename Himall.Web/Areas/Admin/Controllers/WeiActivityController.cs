...
namespace Himall.Web.Areas.Admin.Controllers
{
    ...
    public class WeiActivityController : Controller
    {
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult GetCouponByName(string text, int page, int pageSize, string endtime = "", int ReceiveType = -1,string couponsId="")
        {
            List<long> cou = new List<long>();
            if (!string.IsNullOrEmpty(couponsId)) {
                var strcoupns = couponsId.Split(',');
                foreach (var item in strcoupns) {
                    if (long.Parse(item) > 0)
                        cou.Add(long.Parse(item));
                }
            }
            DateTime dtime = DateTime.Parse("1900-01-01");
            if (!endtime.Equals(""))
                dtime = Convert.ToDateTime(endtime);
            var result = CouponApplication.GetCouponByName(text, dtime, 2, page, pageSize, cou);//主动发放
            var datas = result.Models.ToList();
