            ...
            { field: "ShowApplyTime", title: '申请时间', width: 155, sort: true },
            {
                ...
                formatter: function (value, row, index) {
                    ...
                    if (row.DistributionStatus != 3) {
                        ...
                        html += "<a class=\"btn-changesuper\" href=\"javascript:ShowChangeSuperDlg( '" + row.MemberName + "','" + row.SuperiorMemberName + "'," + row.MemberId + ")\">调整上级</a>";
                    }
                    if (row.DistributionStatus == 3) {
...
function ShowChangeSuperDlg(MemberName, SuperiorMemberName, MemberId) {
    $.dialog({
        title: '调整所属上级',
        lock: true,
        width: 550,
        ...
        init: function () {
            $("#spn_membername").html(MemberName);
            $("#spn_supername").html(SuperiorMemberName);
            $("#hf_MemberId").val(MemberId);
...
function QueryTopList() {
    var memberId = $("#hf_MemberId").val();
    //优惠券表格
    $("#DistributorSuperGrid").hiMallDatagrid({
