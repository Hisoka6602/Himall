...
$(function () {
    ...
    $('#inputStartDate').on('apply.daterangepicker', function (ev, picker) {//结束日期的监听
        $("[name=begin]").val($("[name=daterangepicker_start]").val().format("YYYY-MM-DD"));
        $("[name=end]").val($("[name=daterangepicker_end]").val().format("YYYY-MM-DD"));
        LoadChart();
    });
