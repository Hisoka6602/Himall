        ...
        [[
            ...
            { field: "IsSelf", title: "IsSelf", hidden: true },
            {
                field: "Name", title: "店铺名称", width: 140, formatter: function (value, row, index) {
                    if (row.Status != '被拒绝' && row.Status != '待付款' && row.Status != '待审核')
                    ...
                    if (row.Status == "不可用") {
                        ...
                        if (row.Status != '被拒绝') {
                            if (row.BusinessType == 0) {
                                html.push('<a href="./Edit?id=' + id + '" target="_black">编辑</a>');
                            } else {
                                html.push('<a href="./EditPersonal?id=' + id + '" target="_black">编辑</a>');
                            }
                        }
                        if (row.IsSelf.toString() == 'false' && row.Status != '被拒绝') {
                            html.push('<a href="./BusinessCategory?id=' + id + '" target="_black">经营类目</a>');
                            if (row.Status != '冻结') {
