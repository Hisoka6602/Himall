...
function Delete(id,username) {
    $.dialog.confirm('确定删除该用户吗？', function () {
        ...
        $.post("./Delete", { id: id, uname: username}, function (data) { $.dialog.tips(data.msg); query(); loading.close(); });
    });
}
function Lock(id) {
...
function BatchDelete() {
    ...
    if (selectedRows.length == 0) {
        ...
        $.dialog.confirm('删除后数据无法恢复，确定要删除会员吗？', function () {
...
function batchDelLabels() {
    ...
    $.dialog({
        ...
        ok: function () {
            ...
            var loading = showLoading();
            $.post('DelMembersLabel', { ids: ids.join(','), labelids: labelids.join(',') }, function (result) {
                if (result.Success) {
                    query();
                    $.dialog.tips('设置成功！');
                }
...
function batchOperation(type) {
    switch (type) {
        case "label": batchAddLabels(); break;
        case "dellabel": batchDelLabels(); break;
        case "member": BatchDelete(); break;
    }
}
