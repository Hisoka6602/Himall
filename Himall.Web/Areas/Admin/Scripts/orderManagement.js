...
$(function () {
    ...
    $("#list").hiMallDatagrid({
        ...
        rowHeadFormatter: function (target, index, row) {
            return '<tr class="child-title"><td colspan="7" style="padding:10px 15px; background:#fff;border:0;font-size: 12px; color:#6b6c6e;"><img src="' + row.IconSrc + '" title="' + (row.PlatformText == 'Android' ? 'App' : row.PlatformText) + '订单' + '" width="16" style="margin:0 8px 0 0;position:relative;top:-1px;" /> 订单号： ' + row.OrderId + (row.IsVirtual == true ? '&nbsp;<span style="margin-left:10px">(虚拟)</span>' : '') + (row.OrderType == 3 ? '<span style="margin-left:10px">(拼团订单)</span>' : '') + (row.IsLive == true ? '<span style="margin-left:10px">(直播订单)</span>' : '') + ' &nbsp;&nbsp;&nbsp;&nbsp; ' + row.OrderDate + '<span class="pull-right">' + (row.PaymentTypeStr || '') + '</span></td></tr>';
        },
    ...
    $('#searchButton').click(function (e) {
        ...
        $("#list").hiMallDatagrid('reload', {
            ...
            userContact: txtUserContact, IsVirtual: isVirtual, IsLive: isLive, InvoiceType: invoiceType
        });
    })
    ...
    $('.nav-tabs-custom li').click(function (e) {
        ...
        if ($(this).attr('type') == 'statusTab') {//状态分类
            ...
            $("#list").hiMallDatagrid('clearReload', { status: $(this).attr('value') == 0 ? null : $(this).attr('value'), shopBranchName: $.trim($('#txtShopBranchName').val()) });
        }
    });
});
...
function ExportExecl() {
    ...
    var isLive = fglive.checked;
    if (!isLive) {
        isLive = null;
    }

    var href = "/Admin/Order/ExportToExcel?status={0}&startDate={1}&endDate={2}&orderId={3}&shopName={4}&userName={5}&paymentTypeGateway={6}&paymentType={7}&userContact={8}&orderType={9}&shopBranchName={10}&IsVirtual={11}&isLive={12}&invoiceType={13}"
        .format(status, startDate, endDate, orderId, shopName, userName, paymentTypeGateway, selectPaymentType, txtUserContact, orderType, shopBranchName, isVirtual, isLive, invoiceType);
