    ...
    function typeChoose(val) {
        ...
        datacols[0] = datacols[0].concat([
        		{ checkbox: true, width: 39 },
                {
                    ...
                    {
                        field: "Amount", title: "退款", width: 80, align: "center",
                        formatter: function (value, row, index) {
        ...
        button: [
            {
                ...
                callback: function () {
                    ...
                    ConfirmRefundOffLine(data.RefundId, $('#txtRefundRemark').val());
                },
                focus: true
            },
            {
...
function OpenRefundReason(rowIndex) {
    ...
    if (data.IsVirtual) {
        ...
        if (data.RefundStatus == '退款成功') {
            dlgcontent = dlgcontent.concat([
            '<div class="form-group">',
                '<label class="label-inline fl">退款时间</label>',
                '<p class="help-top">' + data.ManagerConfirmDate+ '</p>',
                ' </div>']);
        }
        dlgcontent = dlgcontent.concat(['</div>']);
    }
...
//线下退款
function ConfirmRefundOffLine(refundId, managerRemark) {
    var loading = showLoading();
    $.post('./ConfirmRefundOffLine', { refundId: refundId, managerRemark: managerRemark }, function (result) {
        loading.close();
        if (result.success) {
            if (result.status == 2) {
                $.dialog.succeedTips("操作失败！" + result.msg);
            } else {
                $.dialog.succeedTips("操作成功！");
                var pageNo = $("#list").hiMallDatagrid('options').pageNumber;
                $("#list").hiMallDatagrid('reload', { pageNumber: pageNo });
