...
$(function () {
      ...
      columns:
        [[
          ...
          },
          {
            ...
            { field: "CheckDate", title: '审核时间', width: 95, align: "center", sort: true },
          {
            field: "s", title: "操作", width: 90, align: "center",
            formatter: function (value, row, index) {
              var html = '<span class="btn-a"><input class="thisId" type="hidden" value="' + row.id + '"/><input class="thisShopId" type="hidden" value="' + row.shopId + '"/><input class="thisName" type="hidden" value="' + row.name + '"/><input class="thisPrice" type="hidden" value="' + (row.price == row.MaxPrice ? row.price.toFixed(2) : row.price.toFixed(2) + "~" + row.MaxPrice.toFixed(2)) + '"/>';
  ...
  $('#searchButton').click(function () {
    ...
    $("#list").hiMallDatagrid('reload', { brandName: brandName, keyWords: keyWords, categoryId: categoryId, productCode: productId, shopName: shopName, productType: productType });
  })
...
function AutoComplete() {
    $('#shopName').autocomplete({
        source: function (query, process) {
            ...
            $.post("/Admin/Order/GetShopAndShopBranch", { "keyWords": $('#shopName').val(), "type": 1 }, function (respData) {
