...
namespace Himall.Web.Areas.Mobile.Controllers
{
    public class BranchProductController : BaseMobileTemplatesController
    {
        ...
        public BranchProductController(ShopService ShopService, VShopService VShopService, ProductService ProductService,
            CashDepositsService CashDepositsService, FreightTemplateService FreightTemplateService, RegionService RegionService
            , TypeService TypeService, MessageService MessageService, ShopBranchService ShopBranchService
            )
        {
        ...
        public JsonResult GetNeedRefreshProductInfo(long id = 0, long shopId = 0, long bid = 0)
        {
            ...
            return SuccessResult<dynamic>(data: new
            {
                ...
                saleCounts = salecount + Himall.Core.Helper.TypeHelper.ObjectToInt(productInfo.VirtualSaleCounts)
            });
        }
        ...
        public ActionResult Details(long id = 0, long partnerid = 0, int nojumpfg = 0, string shopBranchId = "")
        {
            ...
            if (!string.IsNullOrEmpty(shopBranchId)) {
                var branchsku = ShopBranchApplication.GetSkus(product.ShopId, long.Parse(shopBranchId));
                model.StockAll = branchsku.Sum(s=>s.Stock);
            }
            if (skus.Count > 0)
            {
            ...
            var customerServices = CustomerServiceApplication.GetMobileCustomerServiceAndMQ(product.ShopId, true, CurrentUser, product);
        ...
        public ActionResult VDetails(long id = 0, long partnerid = 0, int nojumpfg = 0, string shopBranchId = "")
        {
            ...
            model.VersionAlias = versionAlias;
            var skus = ProductManagerApplication.GetSKUs(product.Id);
            var shopbranchsku = ShopBranchApplication.GetSkus(product.ShopId, branchId);
            model.StockAll = shopbranchsku.Sum(s=>s.Stock);
            if (skus.Count() > 0)
            {
                long colorId = 0, sizeId = 0, versionId = 0;
                foreach (var sku in skus)
        ...
        public JsonResult GetCommentByProduct(long pId)
        {
            ...
            if (comments.Models != null && comments.Models.Count > 0)
            {
                foreach (ProductComment comment in comments.Models)
                {
                    if (comment.Images != null && comment.Images.Count > 0)
                    {
                        foreach (ProductCommentImage image in comment.Images)
                        {
        ...
        public JsonResult GetProductComment(long pId, int pageNo, int commentType = 0, int pageSize = 10, long shopBranchId = 0)
        {
            ...
            var data = comments.Select(item =>
            {
                ...
                return new
                {
                    Sku = _ProductService.GetSkuString(orderitem.SkuId),
                    UserName = item.UserName,
                    ReviewContent = item.ReviewContent,
