...
namespace Himall.Web.Areas.Mobile.Controllers
{
    public class CartController : BaseMobileMemberController
    {
        ...
        ShopBranchService _iShopBranchService;
        public CartController(ProductService ProductService, ShopService ShopService, VShopService VShopService, ShopBranchService ShopBranchService)
        {
            ...
            var products = cartItems.Select(item =>
                {
                    ...
                    var pic =HimallIO.GetProductSizeImage(product.RelativePath, 1, (int)ImageSize.Size_150);
                    if (!string.IsNullOrEmpty(sku.ShowPic))
                    {
                        pic =HimallIO.GetImagePath(sku.ShowPic);
                    }
                    return new
                    {
        ...
        public JsonResult GetBranchCartProducts(long shopBranchId)
        {
            ...
            var products = cart.Items.Select(item =>
            {
                ...
                if (null != shop)
                {
                    ...
                    //sku = _ProductService.GetSku(item.SkuId);
