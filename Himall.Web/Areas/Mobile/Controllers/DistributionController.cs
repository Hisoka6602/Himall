    ...
    public class DistributionController : BaseMobileMemberController
    {
        ...
        public JsonResult PostApplyDistributor(string logoWXmediaId, string shopname)
        {
            if (SiteSettings.DistributorApplyNeedQuota > 0)
            {
                ...
                if (_u.NetAmount < SiteSettings.DistributorApplyNeedQuota)
                {
                    throw new HimallException("需要累计消费金额达到" + SiteSettings.DistributorApplyNeedQuota + "元才可申请哦");
                }
            }
            if (CurrentDistributor == null)
            {
            ...
            if (string.IsNullOrWhiteSpace(logoWXmediaId))
            {
                ...
                {
                    throw new HimallException("请上传小店logo");
                }
            }
            if (string.IsNullOrWhiteSpace(shopname))
            {
                throw new HimallException("请填写小店名称");
            }
            if (shopname.Length > 10)
            {
                throw new HimallException("小店名称不能超过10个字符");
            }
            if (CurrentDistributor != null)
            {
                if (CurrentDistributor.DistributionStatus == (int)DistributorStatus.NotAvailable)
        ...
        public ActionResult PostWithdraw(DistributionApplyWithdraw post)
        {
            ...
            {
                var openid = WebHelper.GetCookie(CookieKeysCollection.HIMALL_USER_OpenID);
                ...
                {
                    string token = WXApiApplication.TryGetToken(SiteSettings.WeixinAppId, SiteSettings.WeixinAppSecret);
        ...
        public ActionResult InitPayPassowrd(string password)
        {
            if (MemberApplication.HasPayPassword(CurrentUser.Id))
                throw new HimallException("已设置过支付密码");
        ...
        public ActionResult Market()
        {
            ...
            result.ShareProductUrlTMP = SiteSettingApplication.GetCurDomainUrl() + Url.Action("Detail", "Product", new { id = "#id#", SpreadId = "#uid#" });
            ...
            if (CurrentSpreadId.HasValue)
            {
                ...
                {
                    shareIcon = HimallIO.GetRomoteImagePath(distributor.ShopLogo);
                }
            }
        ...
        public ActionResult MyShop()
        {
            ...
            DistributionMyShopViewModel result = Mapper.Map<DistributorInfo, DistributionMyShopViewModel>(CurrentDistributor);
            if (string.IsNullOrWhiteSpace(result.ShopLogo))
            {
                result.ShopLogo = CurrentUser.Photo;
            }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult SplittinRule()
        {
            var result = CurrentDistributor;
            if (result == null)
            {
            ...
            ViewBag.ReferralPosterUrl = DistributionApplication.GetWXReferralPostUrl(CurrentUser);
            ViewBag.Title = "我的名片";
            return View(result);
        }
    }
