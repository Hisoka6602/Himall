...
namespace Himall.Web.Areas.Mobile.Controllers
{
    public class OrderRefundController : BaseMobileMemberController
    {
        ...
        public OrderRefundController(OrderService OrderService, RefundService RefundService, ShopService ShopService, VShopService VShopService)
        {
        ...
        public ActionResult RefundApply(long orderid, long? itemId, long? refundid)
        {
            ...
            var ordser = ObjectContainer.Current.Resolve<OrderService>();
            ...
            if (isok)
            {
                ...
                {
                    //售后时间限制
                    if (_OrderService.IsRefundTimeOut(orderid))
                    {
                        isok = false;
                        errormsg = "订单已超过售后期";
                        return Redirect(jumpurl);
        ...
        public JsonResult RefundApply(OrderRefundInfo info)
        {
            ...
            if (order.OrderType != OrderInfo.OrderTypes.Virtual && (int)order.OrderStatus < 2) throw new Himall.Core.HimallException("错误的售后申请,订单状态有误");
            //售后时间限制
            if (order.OrderType != OrderInfo.OrderTypes.Virtual && _OrderService.IsRefundTimeOut(info.OrderId))
            {
                throw new Himall.Core.HimallException("订单已超过售后期");
            }
            ...
            var orderitem = _OrderService.GetOrderItem(info.OrderItemId);
            ...
            if (order.OrderType != OrderInfo.OrderTypes.Virtual && info.IsReturn == true && info.ReturnQuantity < 1) throw new Himall.Core.HimallException("错误的退货数量");
            ...
            if (order.OrderType != OrderInfo.OrderTypes.Virtual)
            {
                ...
                {
                    _RefundService.AddOrderRefund(info);
                }
            }
            else
            {
                ...
                try
                {
                    ...
                    var result = _RefundService.ConfirmRefund(info.Id, "虚拟订单申请售后自动退款", "", notifyurl);
                }
        ...
        public JsonResult List(int pageNo = 1, int pageSize = 10)
        {
            ...
            {
                var vshop = _VShopService.GetVShopByShopId(item.ShopId) ?? new Entities.VShopInfo() { Id = 0 };
                bool IsSelfTake = false;
                var order = _OrderService.GetOrder(item.OrderId, CurrentUser.Id);
                ...
                if (item.RefundMode == OrderRefundInfo.OrderRefundMode.OrderRefund)
                    orderItems = _OrderService.GetOrderItemsByOrderId(item.OrderId);
                else
                ...
                return new
                {
                    ...
                    Amount = item.Amount,
                    Img = orderItem.ThumbnailsUrl.Contains("skus")?HimallIO.GetRomoteImagePath(orderItem.ThumbnailsUrl) :HimallIO.GetProductSizeImage(orderItem.ThumbnailsUrl, 1, (int)ImageSize.Size_100),
                    ShopId = item.ShopId,
                    ...
                    {
                        ThumbnailsUrl = e.ThumbnailsUrl.Contains("skus") ? HimallIO.GetRomoteImagePath(e.ThumbnailsUrl) : HimallIO.GetProductSizeImage(e.ThumbnailsUrl, 1, (int)ImageSize.Size_100),
        ...
        {
            var refundinfo = _RefundService.GetOrderRefund(id, CurrentUser.Id);
            ViewBag.RefundPayType = refundinfo.RefundPayType.ToDescription();
            refundinfo.IsOrderRefundTimeOut = _OrderService.IsRefundTimeOut(refundinfo.OrderId);
            var order = _OrderService.GetOrder(refundinfo.OrderId, CurrentUser.Id);
        ...
        public ActionResult RefundProcessDetail(long id)
        {
            ...
            ViewBag.RefundLogs = _RefundService.GetRefundLogs(refundinfo.Id, curappnum, false);
            return View(refundinfo);
        }
        [HttpGet]
