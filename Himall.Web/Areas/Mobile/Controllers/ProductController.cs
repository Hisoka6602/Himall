...
namespace Himall.Web.Areas.Mobile.Controllers
{
    public class ProductController : BaseMobileTemplatesController
    {
        ...
        public ProductController(ShopService ShopService, VShopService VShopService, ProductService ProductService,
            CashDepositsService CashDepositsService, FreightTemplateService FreightTemplateService, RegionService RegionService
            , TypeService TypeService, MessageService MessageService
            )
        {
        ...
        public JsonResult GetNeedRefreshProductInfo(long id = 0, long shopId = 0)
        {
            ...
            long vShopId;
            var vshopinfo = _VShopService.GetVShopByShopId(productInfo.ShopId);
            if (vshopinfo == null)
                vShopId = -1;
            else
            ...
            {
                isFavorite = _ProductService.IsFavorite(productId, CurrentUser.Id);
            ...
            //刷新店铺关注人数
            var favoriteShopCount = _ShopService.GetShopFavoritesCount(shopId);
            //var skus = _ProductService.GetSkuList(productId)
            string skuId = "";
            ...
            string strUrlRef = WebHelper.GetAbsoluteUriReferrer();
            if (string.IsNullOrEmpty(strUrlRef))
            {
                strUrlRef = WebHelper.GetAbsoluteUri();
            }
            var weixinarg = Application.WXApiApplication.GetWeiXinShareArgs(strUrlRef);
            ...
            return SuccessResult<dynamic>(data: new
            {
                ...
                saleCounts = productInfo.SaleCounts + Himall.Core.Helper.TypeHelper.ObjectToInt(productInfo.VirtualSaleCounts),
        ...
        /// <returns></returns>
        public ActionResult Detail(long id, long partnerid = 0, int nojumpfg = 0,bool sv=false)
        {
            ...
            //限时购商品跳转
            var ltmbuy = ObjectContainer.Current.Resolve<LimitTimeBuyService>().GetLimitTimeMarketItemByProductId(id);
            if (ltmbuy != null)
            {
        ...
        {
            var _weiXinShareArgs = Application.WXApiApplication.GetWeiXinShareArgs(WebHelper.GetAbsoluteUri());
        ...
        /// <param name="nojumpfg">不跳转拼团</param>
        /// <returns></returns>
        public ActionResult Details(long id = 0, long partnerid = 0, int nojumpfg = 0,bool sv=false)
        {
            var shopService = _ShopService;
            var customerService = ObjectContainer.Current.Resolve<CustomerCustomerService>();
            string price = "";
            ...
            product = _ProductService.GetProduct(gid);
            ...
            shop = _ShopService.GetShop(product.ShopId);
            ...
            {
                isFavorite = _ProductService.IsFavorite(product.Id, CurrentUser.Id);
                var favoriteShopIds = _ShopService.GetFavoriteShopInfos(CurrentUser.Id).Select(item => item.ShopId).ToArray();//获取已关注店铺
            ...
            ViewBag.HasFightGroup = hasFightGroup;
        ...
        private string GetFreightStr(long productId, decimal discount, string skuId, int addressId, Entities.ProductInfo product)
        {
            ...
            {
                decimal freight = ObjectContainer.Current.Resolve<ProductService>().GetFreight(new List<long>() { productId }, new List<int>() { 1 }, addressId, true);
        ...
        public JsonResult LogProduct(long pid)
        {
            ...
            //_ProductService.LogProductVisti(pid);
        ...
        {
            int status = 0;
            _ProductService.AddFavorite(pid, CurrentUser.Id, out status);
        ...
        {
            _ProductService.DeleteFavorite(pid, CurrentUser.Id);
        ...
        public JsonResult GetProductComment(long pId, int pageNo, int commentType = 0, int pageSize = 10, long shopBranchId = 0)
        {
            ...
            //        FinishDate = order.FinishDate.Value.ToString("yyyy-MM-dd HH:mm:ss"),
            //        Images = commentImages.Where(a => a.CommentId == item.Id && a.CommentType == 0).Select(a => new { CommentImage = Himall.Core.HimallIO.GetImagePath(a.CommentImage) }).ToList(),
            //        AppendImages = commentImages.Where(a => a.CommentId == item.Id && a.CommentType == 1).Select(a => new { CommentImage = Himall.Core.HimallIO.GetImagePath(a.CommentImage) }).ToList(),
            //        ReviewDate = item.ReviewDate.ToString("yyyy-MM-dd HH:mm:ss"),
            //        ReplyContent = string.IsNullOrWhiteSpace(item.ReplyContent) ? "暂无回复" : item.ReplyContent,
            //        ReplyDate = item.ReplyDate.HasValue ? item.ReplyDate.Value.ToString("yyyy-MM-dd HH:mm:ss") : " ",
            //        ReviewMark = item.ReviewMark,
        ...
        private IEnumerable<Entities.CouponInfo> GetCouponList(long shopid)
        {
            ...
            var couponSetList = ObjectContainer.Current.Resolve<VShopService>().GetVShopCouponSetting(shopid).Where(a => a.PlatForm == Core.PlatformType.Wap).Select(item => item.CouponID);
        ...
        public ActionResult ShowSkuInfo(long id)
        {
            ...
            Entities.ProductInfo product = _ProductService.GetProduct(id);
        ...
        {
            var ProductDescription = ObjectContainer.Current.Resolve<ProductService>().GetProductDescription(pid);
        ...
        public ActionResult ShowDepotAddress(long ftid)
        {
            ...
            if (template != null)
            {
                var fullName = _RegionService.GetFullName(template.SourceAddress);
                if (fullName != null)
                {
        ...
        /// <returns></returns>
		[ChildActionOnly]
        public ActionResult ShowServicePromise(long id, long shopId,bool IsVirtual=false)
        {
            var model = _iCashDepositsService.GetProductEnsure(id);
            int regionId = 0;
            if (CurrentUser != null)
            {
        ...
        public ActionResult ProductCommentShow(long id, int top = 1, bool isshowtit = false)
        {
            ...
            if (comments.Count > 0 && top > 0)
            {
                ...
                {
                    Sku = _ProductService.GetSkuString(orderItems[c.SubOrderId].SkuId),
        ...
        public JsonResult GetDistributionInfo(long id)
        {
            ...
            model.ShareUrl = SiteSettingApplication.GetCurDomainUrl() + "/m-wap/product/Detail/" + id.ToString();
            ...
            model.WeiXinShareArgs = Application.WXApiApplication.GetWeiXinShareArgs(strUrlRef);
