...
namespace Himall.Web.Areas.Mobile.Controllers
{
    public class ShopBranchController : BaseMobileTemplatesController
    {
        ...
        public ShopBranchController(LimitTimeBuyService LimitTimeBuyService, CartService CartService, ProductService ProductService, TypeService TypeService, CouponService CouponService, ShopBranchService ShopBranchService)
        {
        ...
        public ActionResult Index(long id = 0)
        {
            ...
            if (shopBranch.Status == ShopBranchStatus.Freeze)
                throw new HimallException("此门店已冻结");
            ...
            {
                if (!ShopBranchApplication.CheckShopCategoryProductIsExist(shopBranch.ShopId, id, item.Id))
        ...
        public JsonResult List(int pageSize, int pageNo, string fromLatLng, string shopId)
        {
            ...
            return Json(new Result
            {
                ...
                data = new
                {
                    Models = homepageBranchs,
                    Total = shopBranchs.Total,
                    CityInfo = new { Id = (cityInfo != null ? cityInfo.Id : 0), Name = (cityInfo != null ? cityInfo.Name : "") },
        ...
        private List<HomePageShopBranch> ProcessBranchHomePageData(List<ShopBranch> list)
        {
            ...
            foreach (var sid in shopIds)
            {
                ...
                var settings = _CouponService.GetSettingsByCoupon(coupons.Select(p => p.Id).ToList());
                ...
                couponList = couponList.Concat(platcoupons).ToList();
        ...
        public JsonResult ProductList(int pageSize, int pageNo, string productId, string shopCategoryId, string shopId, string shopBranchId, string keyWords, int type)
        {
            ...
            var pageModel = ShopBranchApplication.GetShopBranchProductsMonth(query, (DateTime)query.SaleStartDate, dtNow);//普通分页   
            ...
            List<ProductShopCategory> pshopcate = new List<ProductShopCategory>();
            var pids = pageModel.Models.Select(t => t.Id);
            if(pids != null && pids.Count() > 0)
            {
                proCommentList = CommentApplication.GetProductCommentHightStatisticList(pids.ToList(), sId, shopBranchId: shopBrandId);
                sbskulist = ShopBranchApplication.GetSkusByProductIds(shopBrandId, pids);
                pshopcate = ShopCategoryApplication.GetCategorysByProduct(pids.ToList());
            }

            //如果商品Id不为空，则表示此商品需要置顶
            var product = pageModel.Models.ToList().Select(item =>
            {
                //var comment = CommentApplication.GetProductCommentStatistic(productId: item.Id, shopBranchId: shopBrandId);
                ...
                var shopCat = (pshopcate == null) ? null : pshopcate.Where(t => t.ProductId == item.Id).FirstOrDefault();
                return new
                {
                    Id = item.Id,
                    ...
                    ProductTypeId = (shopCat == null) ? 0 : shopCat.ShopCategoryId,
                    ProductType = (shopCat == null) ? "" : shopCat.ShopCategoryName,
            ...
            if (pId > 0)
            {
                ...
                var topModel = ShopBranchApplication.GetShopBranchProductsMonth(query, (DateTime)query.SaleStartDate, dtNow);//置顶商品
        ...
        {
            var result = _CouponService.GetCouponList(shopid);
            var couponSetList = ObjectContainer.Current.Resolve<VShopService>().GetVShopCouponSetting(shopid).Where(a => a.PlatForm == Core.PlatformType.Wap).Select(item => item.CouponID);
            if (result.Count() > 0 && couponSetList.Count() > 0)
            {
                ...
                var list = new List<Entities.CouponRecord>();
        ...
        public JsonResult GetUserCoupon(long couponId)
        {
            bool status = true;
            string message = "";
            var couponInfo = _CouponService.GetCouponInfo(couponId);
            ...
            var pageModel = _CouponService.GetCouponRecordList(crQuery);
            ...
            pageModel = _CouponService.GetCouponRecordList(crQuery);
            ...
            if (status)
            {
                ...
                _CouponService.AddCouponRecord(couponRecordInfo);
        ...
        public JsonResult GetStoreByProductAndSaleCount(string ids, long? productId)
        {
            ...
            return Json(new { success = true, data = result, productSaleCountOnOff = productSaleCountOnOff }, JsonRequestBehavior.AllowGet);
        }
