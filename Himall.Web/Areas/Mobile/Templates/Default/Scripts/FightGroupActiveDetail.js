                        ...
                        $.post('../CheckBuyNumber', { id: activeid, skuId: sku, count: num }, function (result) {
                            if (result.success) {
                                ...
                                if (hasbuy + num <= maxnum) {
                                    ...
                                    var _url = '/common/site/pay?area=mobile&platform=' + areaName.replace('m-', '') + '&controller=order&action=SubmitGroup&skuId={0}&count={1}&groupActionId={2}&ProductId={4}&returnUrl={3}'.format(sku, num, activeid, returnHref, result.data.ProductId);
                                    if (groupid) {
                                        var canjoin = false;
                                        $.ajax({
                                            type: "post",
