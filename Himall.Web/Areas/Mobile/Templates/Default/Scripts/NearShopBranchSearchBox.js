...
$(function () {
    ...
    $('#clearKeyWord').click(function() {
        himallStorage.removeItem('wxKeyWordList');
        $('#keyWordList').html('');
        //location = "?keywords=";
    });
});
...
function setKeyWordStorage(keyWord) {
    var arry = new Array();
    var value = himallStorage.getItem('wxKeyWordList');
    if (value) {
        arry = value.split(',');
    }
    ...
    himallStorage.setItem('wxKeyWordList', arry.join(','));
}
//获取缓存关键词
function getKeyWordStorage() {
    var html = '';
    var KeyWordList = himallStorage.getItem('wxKeyWordList');
    if (KeyWordList) {
