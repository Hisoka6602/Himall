    ...
    $.post(url, { pId: pid, pageNo: page, commentType: commentType, pageSize: 8, shopBranchId: $("#shopBranchId").val() }, function (result) {
        ...
        if (result.data.length > 0) {
            $.each(result.data, function(i, items) {
                // console.log(items);
                html += '<li><div class="gray"><span class="name">' + items.UserName + '</span>';
                ...
                if (items.AppendDate != null && items.AppendDate != "") {
                    ...
                    html += '<dl class="comment-ago"><dt>收货' + GetDateDiff(items.FinishDate, items.AppendDate) + '后追加</dt><dd>' + items.AppendContent + '</dd></dl><dl class="comment-img" data-img=\''+dataImg+'\'>'+showImg+'</dl>';
                }
