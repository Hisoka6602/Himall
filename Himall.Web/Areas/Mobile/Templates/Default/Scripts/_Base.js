var spreadidparaname = "spreadid", needclearspreadidckname = "himall-needclearspreadid";
$(function () {
    if ($(document).width() <= 640) {
        try {
            $("html").niceScroll({ cursorwidth: 0, cursorborder: 0 });
        } catch (ex) { }//有些没必要引用没引用jquery.scrollLoading.min.js，可能报错trycatch
    }
    ...
    if (isWeiXin()) {
        ...
        if (localhref.indexOf("/order/") != -1 && localhref.indexOf("/order/shopbranchs") == -1) {
            //写入空白历史记录
            pushHistory();
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success) {
                    ...
                    if (getQueryString("source") === 'applet') {
