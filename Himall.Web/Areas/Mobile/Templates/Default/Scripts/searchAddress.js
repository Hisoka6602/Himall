...
function SearchRegionKey() {
    $.ajax({
        url: "/Common/PublicCommon/GetMapAddressPos",
        type: 'GET',
        data: { key: $("#hdQQMapKey").val(), keyword: cityName + $("#txtKeyWord").val() },
        dataType: "json",
        success: function (result) {
            if (result.status == 0) {
                var lnum = result.data.length;
                if (lnum > 0) {
                    for (var i = 0; i < lnum; i++) {
                        var poi = result.data[i];
                        if (typeof (poi.address) != "undefined") {
                            var parText = "\'" + poi.location.lat + ',' + poi.location.lng + "\'" + "," + "\'" + poi.address + "\'" + "," + "\'" + poi.title + "\'";
                            $("#nearAddress").append('<li data-addr=\"' + poi.title + '\" onclick="choosePosition(' + parText + ')"> <h3>' + poi.title + '</h3><p>' + poi.address + '</p></li>');
                        }
                    }
                    $("#divAdr").show();
                }
...
function mapServiceRequest(data) {
	$.ajax({
		url: 'https://apis.map.qq.com/ws/place/v1/search?key=' + $('#hdQQMapKey').val(),
		data: data,
		type: 'get',
		dataType: 'jsonp',
