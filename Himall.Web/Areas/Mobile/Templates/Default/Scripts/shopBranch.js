...
//  $('.index-category').height($(window).height() - ($('.index-topimg').height() + $('.index-address').height()));
    ...
    $('.categoryLeft li').each(function (i) {
        if (i == 1 && getUrlParam("keywords") == null) {
            ...
            var selcate = window.sessionStorage.getItem("selectCategory");//加载原来选中目录
            if (selcate == null) {
                window.sessionStorage.setItem("selectCategory", $(this).text());//设置选中目录
            }
        }
    ...
    $('.categoryLeft li').click(function () {
        ...
        $("#categoryRight").scrollTop(0);//右侧默认初始位置
        storeObj.LoadView(shopCategoryId);

        window.sessionStorage.setItem("selectCategory", $(this).text());//设置选中目录
    });
    var initcate = true;//是否加载商品
    var category = window.sessionStorage.getItem("selectCategory");//加载原来选中目录
    if (category != null) {
        var selectProduct = window.sessionStorage.getItem("selectProduct");//选中的产品,它保存值格式(分类Id|当前已加载第几页|右侧滚动距离|右侧框div高度)
        $(".categoryLeft li").each(function () {
            if ($(this).text() == category) {
                ...
                var selcategoryid = 0;//当前选择的分类Id
                var selpageindex = 1;//选择第几页
                var selpos = 0;//滚动距离
                var cheight = "400";//右侧框div高度
                if (selectProduct != null) {
                    var strs = selectProduct.split("|");
                    selcategoryid = strs[0];
                    selpageindex = (strs.length > 1 ? strs[1] : selpageindex);
                    selpos = (strs.length > 2 ? strs[2] : selpos);
                    cheight = (strs.length > 3 ? strs[3] : cheight);
                }
                
                if (categoryid == selcategoryid) {
                    initcate = false;//这里已加载
                    $("#categoryRight").height(cheight + "px");//右侧div滚动窗口高度，如默认100%它设置滚不了，则先设置
                    if (selpageindex > 1) {
                        curpageindex = 1;
                        loadData(0);//加载
                        for (var i = 2; i <= selpageindex; i++) {
                            curpageindex = i;//第几页
                            loadData(2);//加载
                        }
                    } else {
                    ...
                    selpos = parseInt(selpos);
                    selpos = (selpos <= 0 ? 0 : selpos);
                    $("#categoryRight").scrollTop(selpos);//还原滚动距离
                    //alert(selpos + "--" + $("#categoryRight").height() + "--" + $("#categoryRight").scrollTop() + "--" + selpageindex);
                }
                //--滚动原来商品位置--结束---
            }
        });
                ...
                if (data) {
                    ...
                    if (data.TopModels && data.TopModels.length > 0) {
                        $.each(data.TopModels, function (i, model) {
                            var userhtml = '<li><ul class="pros">' + getProductHtml(model, data.ProductSaleCountOnOff, curpageindex) + '</ul></li>';
                    ...
                    if (data.Models && data.Models.length > 0) {
                        ...
                        $.each(data.Models, function (i, model) {
                            ...
                            var userhtml = typehtml + '<li><ul class="pros">' + getProductHtml(model, data.ProductSaleCountOnOff, curpageindex) + '</ul></li>';
                            databox.append(userhtml);
...
function clickproductpage(obj, selshopCategoryId) {
    var ctop = $("#categoryRight").scrollTop();//它当前右侧滚动距离
    var cheight = $("#categoryRight").height();//右侧框div高度大小
    var selectProduct = selshopCategoryId + "|" + curpageindex + "|" + ctop + "|" + cheight;//选中当前商品：分类Id|当前已加载第几页|右侧滚动距离|右侧框div高度大小
    window.sessionStorage.setItem("selectProduct", selectProduct);//设置选产品位置
}
//组合商品数据
function getProductHtml(obj, productSaleCountOnOff, shoppageindex) {
    shoppageindex = (shoppageindex <= 0 ? 1 : shoppageindex);//第几页
    var link = obj.isvirtual ? "vdetails" : "detail";
    var arr = new Array();
    var showUnit = obj.MeasureUnit || "";
    arr.push('<li>');
    arr.push('<a href="/m-wap/branchproduct/' + link + '/' + obj.Id + '?shopBranchId=' + shopBranchId + '" onclick="clickproductpage(this,' + shopCategoryId+')"><img src="' + obj.RelativePath + '"/></a>');
    arr.push('<div class="content">');
    arr.push('<a href="/m-wap/branchproduct/' + link + '/' + obj.Id + '?shopBranchId=' + shopBranchId + '" onclick="clickproductpage(this,' + shopCategoryId +')"><h3>' + obj.ProductName + '</h3>');
    arr.push('<p>')
