...
$(function () {
    ...
    $(document).on('click', '.buy-info .iconbtn,.buy-info .el-image,.buy-info .btn-limit', function () {
        var _t = $(this)
        var pid = _t.data("pid")
        const activitytype = parseInt(_t.data("activitytype"))
        const producttype = parseInt(_t.data("producttype"))
        const href = _t.data('href')
        if (activitytype == 1 || activitytype == 2 || producttype) {
        ...
        if (ishas) {
            ShowSKUInfo(pid, function () {
                ...
                $('#choose').himallSku({
                    ...
                    ajaxUrl: '/m-wap/Product/GetSKUInfo',
...
$(document).on('click','.cap-search',function(){
    const url = '/m-Wap/search/?vshopid='+curvshopid
