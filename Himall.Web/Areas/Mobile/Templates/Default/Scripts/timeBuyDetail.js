...
function justBuy(callBack) {
    ...
    if (len === $(".spec").length) {
        ...
        $.post('../CheckLimitTimeBuy', { skuIds: sku, counts: num }, function(result) {
            if (result.success) {
                var url = '/common/site/pay?area=mobile&platform=' + areaName.replace('m-', '') + '&controller=order&action=submit&skuIds={0}&counts={1}&isLimit=1&FlashSaleId={3}&ProductId={4}&returnUrl={2}'.format(sku, num, returnUrl, result.data.FlashSaleId, result.data.ProductId);
                location.href = url;
