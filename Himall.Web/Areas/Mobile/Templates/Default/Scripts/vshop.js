    ...
    $.post(url, { page: page, pageSize: 8 }, function (result) {
        ...
        if (result.data.length > 0) {
            $.each(result.data, function (i, vshop) {
                ...
                html += '<div class="item">\
		    		<div class="info">\
		    			<a href="javascript:goShop(\''+ url + '\')"><img src="' + vshop.image + '" /></a>\
		    			<h3><a href="javascript:goShop(\''+ url + '\')">' + vshop.name + '</a><i type="addFavorite" class="iconfonts '+(vshop.favorite?'icon-aixin':'icon-guanzhu')+'" shopId="'+vshop.shopId+'"></i></h3>\
...
function goShop(url) {
    ...
    sessionStorage.setItem('data', $("#shopList").html());
