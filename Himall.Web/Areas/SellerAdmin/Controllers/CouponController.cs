...
namespace Himall.Web.Areas.SellerAdmin.Controllers
{
    ...
    public class CouponController : BaseSellerController
    {
        ...
        public CouponController(CouponService CouponService, MarketService MarketService, ShopService ShopService)
        {
        ...
        {
            //处理错误同步结果
            _CouponService.ClearErrorWeiXinCardSync();
            
            var settings = MarketApplication.GetServiceSetting(MarketType.Coupon);
            if (settings == null)
                return View("Nosetting");

            var market = _CouponService.GetCouponService(CurrentSellerManager.ShopId);
        ...
        public ActionResult Edit(long id)
        {
            ...
            var market = _MarketService.GetMarketService(CurrentSellerManager.ShopId, MarketType.Coupon);
        ...
        public JsonResult Edit(CouponInfo info)
        {
            ...
            var shopId = CurrentSellerManager.ShopId;
            info.ShopId = shopId;
            var shopName = _ShopService.GetShop(shopId).ShopName;
        ...
        {
            var model = _CouponService.GetCouponInfo(CurrentSellerManager.ShopId, Id);
            if (model != null)
            {
                if (model.IsSyncWeiXin == 1 && model.WXAuditStatus != (int)WXCardLogInfo.AuditStatusEnum.Audited)
                {
        ...
        public ActionResult BuyService()
        {
            ...
            ViewBag.Price = _MarketService.GetServiceSetting(MarketType.Coupon).Price;
            return View();
        }
        [HttpPost]
        [UnAuthorize]
