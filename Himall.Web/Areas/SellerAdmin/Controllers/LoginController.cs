...
namespace Himall.Web.Areas.SellerAdmin.Controllers
{
    public class LoginController : BaseController
    {
        ...
        public LoginController(ShopService ShopService, ManagerService ManagerService)
        {
        ...
        public JsonResult Login(string username, string password, string checkCode)
        {
            try
            {
                ...
                var manager = _ManagerService.Login(username, password);
                if (manager == null)
                {
                    throw new Himall.Core.HimallException("用户名和密码不匹配");
                }

                var shopinfo = _ShopService.GetShop(manager.ShopId);
