    ...
    public class OrderController : BaseSellerController
    {
        ...
        public OrderController(OrderService OrderService,
            ...
            RefundService RefundService,
             ShopOpenApiService ShopOpenApiService,
        ...
        public ActionResult Detail(long id, bool updatePrice = false)
        {
            ...
            //补充商品货号
            var orderiItems = _OrderService.GetOrderItemsByOrderId(order.Id);
            ...
            var service = ObjectContainer.Current.Resolve<ProductService>();
            ...
            if (order.OrderType != OrderInfo.OrderTypes.Virtual)
            {
                ...
                {
                    var regionService = ObjectContainer.Current.Resolve<RegionService>();
        ...
        public JsonResult List(OrderQuery query, int page, int rows)
        {
            ...
            if (ordidl.Count > 0)
            {
                ...
                foreach (var item in orderModels)
                {
                    ...
                    {
                        var fgord = _FightGroupService.GetFightGroupOrderStatusByOrderId(item.OrderId);
                        if (fgord != null)
                        {
        ...
        public JsonResult CloseOrder(long orderId)
        {
            ...
            try
            {
                ...
                if (order.OrderType == OrderInfo.OrderTypes.FightGroup)
                {
                    ...
                    {
                        needRunClose = false;
                        _FightGroupService.OrderBuildFailed(ford, CurrentSellerManager.UserName, "商家取消订单");
                    }
                }
                if (needRunClose)
                {
                    _OrderService.SellerCloseOrder(orderId, CurrentSellerManager.UserName);
                }
                result.success = true;
            }
            catch (Exception ex)
        ...
        public void DownloadProductList(string ids)
        {
            ...
            var orderItems = _OrderService.GetOrderItemsByOrderId(idList);
            ...
            {
                sku = _ProductService.GetSku(orderItem.SkuId);
                if (sku != null)
                    stock = sku.Stock;
        ...
        public FileResult DownloadOrderList(string ids)
        {
            ...
            //IEnumerable<OrderInfo> orders = _OrderService.GetOrders(idList);
        ...
        private NPOI.HSSF.UserModel.HSSFWorkbook writeProductToExcel(string ids)
        {
            ...
            var orders = _OrderService.GetOrders(idList);
            ...
            NPOI.SS.UserModel.IRow row0 = excelSheet.CreateRow(0);
            string[] columns ={
                                  "商品名称","货号","规格","拣货数量","现库存数"
                             };
            for (int i = 0; i < columns.Length; i++)
            {
                NPOI.SS.UserModel.ICell cell = CreateCell(i, row0);
            ...
            foreach (var orderItem in prepares)
            {
                ...
                NPOI.SS.UserModel.IRow row = CreateRow(rows, excelSheet);
                NPOI.SS.UserModel.ICell cell0 = CreateCell(0, row);
                NPOI.SS.UserModel.ICellStyle style = hssfworkbook.CreateCellStyle();
                NPOI.SS.UserModel.IDataFormat format = hssfworkbook.CreateDataFormat();
        ...
        private NPOI.SS.UserModel.IRow CreateRow(int rowID, NPOI.HSSF.UserModel.HSSFSheet excelSheet)
        {
            NPOI.SS.UserModel.IRow row = excelSheet.GetRow(rowID);
            if (row == null)
            {
        ...
        private NPOI.SS.UserModel.ICell CreateCell(int cellID, NPOI.SS.UserModel.IRow row)
        {
            NPOI.SS.UserModel.ICell cell = row.GetCell(cellID);
            if (cell == null)
            {
        ...
        public JsonResult UpdateAddress(long orderId, string shipTo, string cellPhone, int topRegionId, int regionId, string address)
        {
            ...
            try
            {
                ...
                _OrderService.SellerUpdateAddress(orderId, CurrentSellerManager.UserName, shipTo, cellPhone, topRegionId, regionId, regionFullName, address);
                result.success = true;
            }
            catch (Exception ex)
            {
        ...
        public JsonResult UpdateItemDiscountAmount(long orderItemId, decimal discountAmount)
        {
            ...
            try
            {
                _OrderService.SellerUpdateItemDiscountAmount(orderItemId, discountAmount, CurrentSellerManager.UserName);
                result.success = true;
            }
            catch (Exception ex)
            {
        ...
        {
            _OrderService.SellerUpdateOrderFreight(orderId, frieght, CurrentSellerManager.UserName);
        ...
        {
            return Json(_RegionService.GetRegionPath(regionId));
        }
        ...
        public JsonResult Print(string orderIds, string expressName, string startNo)
        {
            ...
            else
            {
                ...
                //保存订单信息
                _OrderService.SetOrderExpressInfo(CurrentSellerManager.ShopId, expressName, startNo, orderIds_long);
        ...
        public JsonResult AllotStore(long orderId, long shopBranchId, long shopId)
        {
            try
            {
                ...
                if (orderInfo.OrderStatus == Entities.OrderInfo.OrderOperateStatus.WaitDelivery || orderInfo.OrderStatus == Entities.OrderInfo.OrderOperateStatus.WaitPay)
                {
                    ...
                    bool isrefund = false;
                    foreach (var item in refunds.Models)
                    {
                        if (item.SellerAuditStatus != OrderRefundInfo.OrderRefundAuditStatus.UnAudit || (item.ManagerConfirmStatus == OrderRefundInfo.OrderRefundConfirmStatus.UnConfirm && item.SellerAuditStatus == OrderRefundInfo.OrderRefundAuditStatus.Audited))
                        {
                            isrefund = true;
                        }
                    }
                    if (isrefund) return Json(new { Success = false, Message = "订单正在申请售后，请先处理售后申请！" });
        ...
        public JsonResult VerifyVerification(string verificationCode, long shopId)
        {
            ...
            try
            {
                ...
                if (!Himall.Core.Helper.ValidateHelper.IsNumeric(verificationCode))
                {
                    result.success = false; result.code = 1;//该核销码无效
                    return Json(result);
                }
        ...
        public JsonResult GetVerficationDetail(long orderId, long shopId)
        {
            ...
            if (verifications != null && verifications.Count() > 0)
            {
                ...
                if (orderItem != null)
                {
                    ...
                    if (productInfo != null)
                    {
                        productInfo.ImagePath = productInfo.GetImage(Himall.CommonModel.ImageSize.Size_220);
                    }
