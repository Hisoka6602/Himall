...
namespace Himall.Web.Areas.SellerAdmin.Controllers
{
    public class ProductController : BaseSellerController
    {
        ...
        #region 构造函数
        public ProductController(ShopCategoryService ShopCategoryService,
        ...
        public ActionResult Edit(ProductCreateModel model)
        {
            ...
            //处理限时购 逻辑完全copy之前的
            #region 更改限时购数据
            LimitTimeBuyService ilimit = ObjectContainer.Current.Resolve<LimitTimeBuyService>();
            ...
            try
            {
                ...
                //修正门店规格
                ShopBranchApplication.CorrectBranchProductSkus(curpro, curpro.ShopId);
        ...
        {
            var brands = ObjectContainer.Current.Resolve<BrandService>().GetBrandsByCategoryIds(this.CurrentShop.Id, categoryId).Select(p => new
            {
                p.Id,
                p.Name
            }).ToList();
        ...
        public JsonResult GetShopProductCategory(long productId = 0)
        {
            ...
            if (0 != productId)
            {
                var cates = _ProductService.GetProductShopCategories(productId);
        ...
        public JsonResult GetAttributes(long categoryId, long productId = 0, long isCategoryId = 0)
        {
            ...
            else
            {
                ...
                foreach (var item in attrs)
                {
                    ...
                    categoryId = _ProductService.GetProduct(productId).CategoryId;
                    json.Add(attrItem);
                }
        ...
        private SpecJosnModel GetPlatformSpec(long categoryId, long productId = 0)
        {
            ...
            var skus = _ProductService.GetSKUs(productId).OrderBy(s => s.Color).ThenBy(s => s.Size).ThenBy(s => s.Version);
        ...
        public JsonResult GetSpecifications(long categoryId, long productId = 0, long isCategoryId = 0)
        {
            ...
            //商家规格
            var sellerSpec = _ProductService.GetSellerSpecifications(shopId, cate.TypeId);
        ...
        public JsonResult Specifications(long categoryId, long productId = 0)
        {
            ...
            if (productId > 0)
            {
                skus = _ProductService.GetSKUs(productId).ToDictionary(p => SKUEx.SplitId(p.Id), p => p);
                productInfo = _ProductService.GetProduct(productId);
            }

            List<Himall.Entities.SpecificationValueInfo> specifications = ObjectContainer.Current.Resolve<SpecificationService>().GetSpecification(categoryId, this.CurrentShop.Id);
            Himall.Entities.SpecificationValueInfo info = new Himall.Entities.SpecificationValueInfo();
        ...
        private string GetAlias(Entities.ProductInfo productInfo, string text, SpecificationType key, Himall.Entities.SpecificationValueInfo info)
        {
        ...
        private string GetSpecificationTypeAlias(SpecificationType typeId, Himall.Entities.SpecificationValueInfo item)
        {
            string alias = "";
        ...
        public ActionResult PublicStepTwo(string categoryNames = "", long categoryId = 0, long productId = 0)
        {
            ...
            var brands = ObjectContainer.Current.Resolve<BrandService>().GetBrandsByCategoryIds(shopId, categoryId);
        ...
        public ActionResult Management()
        {
            ...
            var auditOnOff = site.ProdutAuditOnOff;
            ...
            ViewBag.IsOpenSmallProg = site.IsOpenMallSmallProg;
            return View();
        }
        ...
        public JsonResult List(ProductQuery query, string auditStatuses, int page, int rows, bool? isAsc)
        {
            ...
            dataGrid.rows = products.Models.Select(item =>
            {
                ...
                return new ProductModel()
                {
                    ...
                    MaxPrice = skus.Where(sku => sku.ProductId == item.Id).Max(sku => sku.SalePrice),
                    Url = "",
        ...
        public ActionResult ExportToExcel(long? categoryId = null, string productCode = "", string brandName = "", int? auditStatus = null, string auditStatuses = null, int? saleStatus = null, string ids = "", string keyWords = "", DateTime? startDate = null, DateTime? endDate = null, long? freightTemplateId = null, sbyte? productType = null)
        {
            ...
            List<Himall.DTO.ProductShopCategory> listshopcate = new List<DTO.ProductShopCategory>();//商铺分类
        ...
        [UnAuthorize]
        [HttpPost]
        public JsonResult Browse(long? categoryId, int? auditStatus, bool? HasLadderProduct, string ids, int page, string keyWords, string shopName,
            int? saleStatus, bool? isShopCategory, int rows = 10, bool isLimitTimeBuy = false, bool showSku = false, long[] exceptProductIds = null, sbyte productType = -1, bool isFilterPromotion = true)
        {
            var query = new ProductQuery()
            {
                PageSize = rows,
                ...
                HasLadderProduct = HasLadderProduct
                //IsFilterStock = true,
        ...
        public JsonResult BatchSaleOff(string ids)
        {
            ...
            _ProductService.SaleOff(ids_long, CurrentSellerManager.ShopId);
            ObjectContainer.Current.Resolve<OperationLogService>().AddSellerOperationLog(new Entities.LogInfo
            {
                Date = DateTime.Now,
        ...
        public JsonResult BatchOnSale(string ids)
        {
            ...
            _ProductService.OnSale(ids_long, CurrentSellerManager.ShopId);
            ObjectContainer.Current.Resolve<OperationLogService>().AddSellerOperationLog(new Entities.LogInfo
            {
                Date = DateTime.Now,
        ...
        public JsonResult SetProductOverSafeStock(string ids, string stock)
        {
            ...
            {
                throw new HimallException("您输入的信息有误，此处只能输入大于等于0的整数！");
            }
            if (safeStock < 0)
            {
                throw new HimallException("库存必须大于0");
            }
            var products = ids.Split(',').Select(item => long.Parse(item)).ToList();
            ProductManagerApplication.SetProductOverSafeStock(products, safeStock);
            return Json(new
            {
                success = true
            });
        ...
        public JsonResult BatchSettingStock(List<StockModel> stocks, string productIds)
        {
            ...
            var products = productIds.Split(',').Select(p => long.Parse(p)).ToList();
            var items = stocks.ToDictionary(key => key.AutoId, value => value.Stock);
            ProductManagerApplication.SetStock(products, items);
            return Json(new
            {
                success = true,
                msg = ""
        ...
        {
            string DirUrl = Server.MapPath("~/videos/");
        ...
        {
            var importCache = Cache.Get<object>(CacheKeyCollection.CACHE_UPLOADVIDEO("SellerAdmin_UploadSpeed_" + Session.SessionID));
        ...
        /// 视频号二维码
        /// </summary>
        public JsonResult GetVideoNumberCode(long productId)
        {
            var videoNumberCode = ProductManagerApplication.GetVideoNumberCode(productId);
            return SuccessResult(data: new
            {
                videoNumberCode
            });
        }
        ...
        private bool CanCreate(out string message)
        {
            ...
            if (grade != null)
            {
                var count = _ProductService.GetShopAllProducts(CurrentSellerManager.ShopId);
        ...
        private ProductCreateModel InitCreateModel(ProductCreateModel model = null)
        {
            ...
            var categories = _iShopCategoryService.GetBusinessCategory(shopId);
        ...
        private ProductCreateModel InitEditModel(ProductInfo product, ProductCreateModel model = null)
        {
            if (model == null)
            {
                ...
                model.Description = Mapper.Map<ProductDescription>(description);
