...
namespace Himall.Web.Areas.SellerAdmin.Controllers
{
    public class ShopController : BaseSellerController
    {
        ...
        public ActionResult ShopDetail()
        {
            ...
            var shop = _ShopService.GetShop(shopid, true);
            ...
            //var model= _ShopService.GetShopBasicInfo(shopid);
            ...
            mMemberAccountSafety.Block(p => p.Phone);//收货人手机
        ...
        public ActionResult Renew()
        {
            ...
            var shop = _ShopService.GetShop(CurrentSellerManager.ShopId);
            ...
            var enableGrade = _ShopService.GetShopGrades().Where(c => c.ChargeStandard >= grade.ChargeStandard && c.Id != shop.GradeId);//要排除自己
        ...
        {
            var shopInfo = _ShopService.GetShop(CurrentSellerManager.ShopId);
            var shopGrade = _ShopService.GetShopGrades().Where(c => c.Id == shopInfo.GradeId).FirstOrDefault();
        ...
        public JsonResult GetInfoAfterGradeSelect(int grade)
        {
            var shopInfo = _ShopService.GetShop(CurrentSellerManager.ShopId);
            var shopGrade = _ShopService.GetShopGrades().Where(c => c.Id == shopInfo.GradeId).FirstOrDefault();
            var newGrade = _ShopService.GetShopGrades().Where(c => c.Id == (long)grade).FirstOrDefault();
        ...
        public JsonResult PaymentList(decimal balance, int type, int value)
        {
            ...
            models = models.OrderByDescending(d => d.Id).Where(item => !string.IsNullOrEmpty(item.RequestUrl) && item.Id != "Himall.Plugin.Payment.WeiXinPay");//只选择正常加载的插件 && item.Id != "Himall.Plugin.Payment.WeiXinPay_Native"
            return Json(models);
        }
        ...
        public ActionResult ReNewPayReturn(string id, decimal balance)
        {
            ...
            try
            {
                ...
                if (!result)
                {
                    ...
                    //    _ShopService.AddShopRenewRecord(model);

                    //    //店铺续费
                    //    _ShopService.ShopReNew(CurrentSellerManager.ShopId, value);
                    //}
                    ////升级操作
                    //else
                    //{
                    ...
                    //    var shopInfo = _ShopService.GetShop(CurrentSellerManager.ShopId);
                    //    var shopGrade = _ShopService.GetShopGrades().Where(c => c.Id == shopInfo.GradeId).FirstOrDefault();
                    //    var newshopGrade = _ShopService.GetShopGrades().Where(c => c.Id == (long)value).FirstOrDefault();
                    //    model.OperateContent = "将套餐‘" + shopGrade.Name + "'升级为套餐‘" + newshopGrade.Name + "'";
                    //    _ShopService.AddShopRenewRecord(model);

                    //    //店铺升级
                    //    _ShopService.ShopUpGrade(CurrentSellerManager.ShopId, (long)value);
                    //}
        ...
        public JsonResult Recordlist(int page, int rows)
        {
            ...
            QueryPageModel<ShopRenewRecordInfo> accounts = _ShopService.GetShopRenewRecords(query);
        ...
        public ActionResult UpRenew(ShopRenewRecordInfo model)
        {
            ...
            if (model.Type == ShopRenewRecordInfo.EnumOperateType.ReNew)
            {
                ...
                var shopInfo = _ShopService.GetShop(model.ShopId);
                if (shopInfo != null)
                {
                    ...
                    _ShopService.AddShopRenewRecord(model);
                    _ShopService.ShopReNew(model.ShopId, model.Year); //店铺续费
            ...
            else
            {
                ...
                {
                    var shopGrade = _ShopService.GetShopGrades().Where(c => c.Id == shopInfo.GradeId).FirstOrDefault();
                    var newshopGrade = _ShopService.GetShopGrades().Where(c => c.Id == model.GradeId).FirstOrDefault();
                    model.OperateContent = "将套餐‘" + shopGrade.Name + "'升级为套餐‘" + newshopGrade.Name + "'";
                    _ShopService.AddShopRenewRecord(model);
                    _ShopService.ShopUpGrade(model.ShopId, model.GradeId); //店铺升级
                    ShopApplication.ClearCacheShop(CurrentSellerManager.ShopId);
                    return Json(new Result() { success = true });
                }
            }
