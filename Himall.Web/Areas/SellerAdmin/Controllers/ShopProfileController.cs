...
namespace Himall.Web.Areas.SellerAdmin.Controllers
{
    ...
    public class ShopProfileController : BaseMemberController
    {
        ...
        public ShopProfileController(ManagerService ManagerService,
        ...
        /// <param name="destination">手机号或邮箱</param>
        /// <returns></returns>
        [HttpPost]
        public JsonResult CheckMemmberContact(string pluginId, string destination)
        {
            var membercontacct = MemberApplication.GetMemberContactsInfo(pluginId, destination, Entities.MemberContactInfo.UserTypes.General);
            bool bo = membercontacct == null || membercontacct.Id <= 0;//true不存在可新绑定，false已存在
            return Json(new Result() { success = bo, msg = bo? SendMemberCodeReturn.success.ToDescription(): SendMemberCodeReturn.repeat.ToDescription() });
        }
        ...
        {
            if (SiteSettingApplication.SiteSettings.IsTheftBrush && pluginId.ToLower().Contains(".sms"))
