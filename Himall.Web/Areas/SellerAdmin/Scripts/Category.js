...
//批量转移商品分类
function BatchMoveProCategory(cId) {
    $('#selectProcutCategory1').val(cId);
   // $("#selectProcutCategory2").empty();
    //$("#selectProcutCategory2").append($("#selectProcutCategory1").find("*").clone(true));
    //$("#selectProcutCategory2 option[value='" + cId + "']").remove();
    //$("#selectProcutCategory2 option[data-parentid=" + cId + "]").remove();
    $('#selectProcutCategory2').val(0);
    //$("#selectProcutCategory2").html();
    $.dialog({
        title: '批量转移商品',
        lock: true,
        id: 'batchSettingProductCategory',
        content: document.getElementById("batchSettingProductCategory"),
        padding: '0 40px',
        okVal: '确定',
        ok: function () {
            var close = false;
            var categoryId1 = $('#selectProcutCategory1').val();
            var categoryId2 = $('#selectProcutCategory2').val();
            var loading = showLoading();
            $.ajax({
                type: "post",
                url: "batchMoveProductCategory",
                data: { cid1: categoryId1, cid2: categoryId2 },
                dataType: "json",
                async: false,
...
$(function () {
    ...
    $('.container').on('click', '.moveCategory', function () {
        var id = $(this).attr('value');
        BatchMoveProCategory(id);
    });
            ...
            success: function (data) {
                ...
                if (data.success === true) {
                    ...
                    for (var i = 0; i < data.Category.length; i++) {
                        ...
                        sub.push('<span class="btn-a">');
                        sub.push('<a onclick="BatchMoveProCategory(' + data.Category[i].Id+')" >转移商品</a></span>');
