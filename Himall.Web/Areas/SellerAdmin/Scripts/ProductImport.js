...
$(function () {
    $("#btnFile").bind("change", function () {
        if ($("#btnFile").val() != '') {
            ...
            if (filename.indexOf(".zip") == -1) {
                $.dialog.tips('选择的数据包不对,数据应是.zip文件!');
                return;
            }
    ...
    //本地数据包
    $('#btnHimallUpload').bind('click', function () {
        paraSaleStatus = -1;
        $('input[name=paraSaleStatus]').each(function (idx, el) {
            if ($(el).attr('checked') == 'checked') {
                paraSaleStatus = $(el).attr('status');
...
function fnUploadFileCallBack(filename) {
    ...
    $.ajax({
        ...
        , error: function (err) {
            $('.ajax-loading').remove();
            var ms = (err.message == null || err.message == "") ? "已导入图片已在后台处理！" : err.message;//它有时是后台图片处理超时这里报错，则这里加提示
            $.dialog.alert(ms, function () {
                window.history.go(0);
            }, 3);
        }
    });
    ...
    $.ajax({
        ...
        , success: function (data) {
            ...
            dom_iframe.onload = function () {
                var filename = this.contentDocument.body.innerHTML;
                if (filename != 'NoFile' && filename != 'Error') {
                    fnUploadHimallFileCallBack(filename);//上传文件后，继续导入商品操作
                    $('#inputFile').val('请选择文件');
                }
                else {
                    $.dialog.tips('上传文件异常');
...
function fnUploadHimallFileCallBack(filename) {
    $.ajax({
        type: 'get'
        , url: '../AsyncProductImport/ImportHimallProductJson'
        , data: {paraSaleStatus: paraSaleStatus, _shopid: $('#inputShopid').val(), _userid: $('#inputUserid').val(),file: filename }
        , datatype: 'json'
        , timeout: 10000
        , success: function (data) {
            $('.ajax-loading').remove();
            if (data.success) {//成功
                ...
                $.dialog.alert(data.message, function () {
                    if (data.fielname != undefined) {
                        window.open(data.fielname);
                    } 
                     window.history.go(0);
                }, 3);
            }
