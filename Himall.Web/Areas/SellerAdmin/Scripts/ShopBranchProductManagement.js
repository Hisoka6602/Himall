    ...
    $("#shopDatagrid").hiMallDatagrid({
        ...
        columns:
            [[
                ...
                {
                    field: "ProductName", title: '商品', align: 'left', width: 200,
                ...
                { field: "ShopCategoryName", title: "商家分类", width: 90, align: "center" },
                {
                    field: "SalePrice", title: "门店价格", width: 150, align: "center",
                    formatter: function (value, row, index) {
                        var html = row.SalePrice.toFixed(2);
                        if (row.MinSalePrice != row.MaxSalePrice) {
                            html = row.MinSalePrice.toFixed(2) + '-' + row.MaxSalePrice.toFixed(2);
                        }
                        return html;
                    }
                },
                ...
                { field: "Stock", title: "门店库存", width: 90, align: "center" },
                {
                    ...
                    formatter: function (value, row, index) {
                        ...
                        html += '<a class="good-check"  onclick="batchSettingStock(' + row.ProductId + ')">编辑</a>';
                        html += '<a class="good-del" onclick="StoresLink(' + row.ProductId + ',' + para.ShopBranchId + ')">链接</a>';
                        html += '<a class="good-check"  onclick="doBatchUnSale(' + row.ProductId + ',1)">下架</a>';
                        return html;
                    }
                }
            ]],
...
//商品链接
function StoresLink(productId, shopBranchId) {
    var url = window.location.protocol + "//" + window.location.host + '/m-wap/branchproduct/detail/' + productId + '?shopBranchId=' + shopBranchId;
    ...
    $.dialog({
        ...
        init: function () {
            $.ajax({
                ...
                url: 'StoresLink',
                cache: false,
                data: { 'vshopUrl': url, shopBranchId: shopBranchId },
                dataType: 'json',
...
function getSelectedIds() {
    var selecteds = $("#shopDatagrid").hiMallDatagrid('getSelections');
    var ids = [];
