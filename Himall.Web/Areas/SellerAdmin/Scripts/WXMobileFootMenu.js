...
$(function () {
    var obj = $("#uploaderpicsel");
    if (obj.length <= 0)
        obj = $("#uploaderpic");
    CreateDropdown($("#txtContent"), obj, { createType: 3, showTitle: true, style: "margin-left: 30px;", txtContinuity: false, reWriteSpan: false, callback: callBackDrpMenu }, $("#clientName").val(), $("#hidOpenstore").val());
    loadmenu();
    //initImageUpload();
})
var setmenuid = "";
...
function addandeditmenu(type, id, parentmenuid, oneortwo) {
    ...
    if (type == 0) {
        ...
        $("#menuiconUrlSel").himallUpload({
            title: '选中图标：',
            imageDescript: '请上传60*60的图片',
            displayImgSrc: $('#MenuIconBoxSel').val(),
            imgFieldName: "menuiconUrlSel",
            dataWidth: 7
        });
    }
    else {
        ...
        $.post('GetFootMenuInfoById', { id: id }, function (d) {
            if (d.status == "0") {
                ...
                $('#MenuIconBoxSel').val(data.shopmenupicsel);
...
function submitaddandedit() {
    ...
    var picsel = $("#menuiconUrlSel").himallUpload('getImgSrc');
    ...
    if (andedit == 0) {//添加一级和二级菜单
        $.post('AddFootMenu', { Id: editid, Name: $("#txttitle").val(), MenuIcon: pic, MenuIconSel: picsel, Url: url }, function (d) {
            if (d.status == "0") {
                $.dialog.tips('添加成功');
                loadmenu();
                $('#myModal').modal('hide')
...
function delmenu(id, type) {
    if (confirm("确定要删除数据吗？")) {
        if (menulength <= 1) {
            $.dialog.tips('不要在删啦，底部导航菜单必须保存1个以上！');
            return;
        }
        var mid= id;
