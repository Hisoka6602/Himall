...
$(function () {
  ...
  function typeChoose(val) {
    ...
    $("#list").hiMallDatagrid({
      ...
      rowHeadFormatter: function (target, index, row) {
          return '<tr class="child-title"><td colspan="8" style="padding:10px 15px; background:#fff; border:0;font-size: 12px; color:#6b6c6e;"><img src="' + row.IconSrc + '" title="' + (row.PlatformText == 'Android' ? 'App' : row.PlatformText) + '订单' + '" width="16" style="margin:0 8px 0 0;position:relative;top:-1px;" /> 订单号:' + row.OrderId + (row.IsVirtual == true ? '&nbsp;<span>(虚拟)</span>' : '') + (row.OrderType == 3 ? '<span style="margin-left:10px">(拼团订单)</span>' : '') + (row.IsLive == true ? '<span style="margin-left:10px">(直播订单)</span>' : '') + ' &nbsp;&nbsp;&nbsp;&nbsp;  ' + row.OrderDate + '<span class="pull-right">' + (row.PaymentTypeStr || '') + '</span></td></tr>';
      },
      ...
      columns:
        [[
          ...
          {
            field: "UserName", title: "买家", width: 120, align: "center", formatter: function (value, row, index) {
          ...
          },
          {
            ...
            formatter: function (value, row, index) {
              ...
              if (row.OrderStatus == "待收货") {
                if (row.PaymentType == 3 && row.OrderType != 3) {
                  html.push("<a class=\"good-check\" onclick=\"OpenCloseOrder('" + id + "')\">取消</a>");//货到付款订单发货后商家也可以关闭；
                }
  ...
  $('#searchButton').click(function (e) {
    ...
    //var fgvirtual = $('.fg-virtual input:checkbox').get(0);
      var orderProType = $.trim($("#orderProType").val());
      var isVirtual = null;
      if (orderProType != null && orderProType.length > 0 && parseInt(orderProType) > 0) {
          isVirtual = orderProType == 1 ? true : false;
      }
    var fglive = $('.fg-live input:checkbox').get(0);
      var isLive = fglive.checked ? true: null;
    ...
    $("#list").hiMallDatagrid('reload', {
      ...
      userContact: txtUserContact, AllotStore: allotStore, ShopBranchId: shopBranchId, IsVirtual: isVirtual, IsLive: isLive, InvoiceType: invoiceType
    });
  });
...
function ExportExecl() {
  ...
  var txtUserContact = $.trim($('#txtUserContact').val());
    ...
    var orderProType = $("#orderProType").val();
  if (shopBranchId == '全部')
    shopBranchId = '';
   var isVirtual = null;
    if (orderProType != null && orderProType.length > 0 && parseInt(orderProType) > 0) {
  ...
  var href = "/SellerAdmin/Order/ExportToExcel?status={0}&startDate={1}&endDate={2}&orderId={3}&userName={4}&orderType={5}&PaymentTypeGateway={6}&userContact={7}&ShopBranchId={8}&IsVirtual={9}&invoiceType={10}&PaymentType={11}"
    .format(status, startDate, endDate, orderId, userName, orderType, paymentTypeGateway, txtUserContact, shopBranchId, isVirtual, invoiceType, selectPaymentType);
          ...
          $.each(data.VirtualOrderItemInfos, function (i) {
            ...
            if (data.VirtualOrderItemInfos[i].VirtualProductItemType == 6) {
              ...
              if (imgContent.length > 0) {
                ...
                if (data.VirtualOrderItemInfos[i].Content && data.VirtualOrderItemInfos[i].Content.length > 0) {
