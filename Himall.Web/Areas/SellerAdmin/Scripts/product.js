...
(function ($) {
    ...
    window.productInit = function () {
        ...
        $('#LadderPrices').on('click', 'tr td .del', function () {
            ...
            if ($('#catePriceBody').find('tr').length > 1) {
                ...
                //alert($('#catePriceBody').find('tr').length);
                if ($('#catePriceBody').find('tr').length > 0) {
                    var tr0 = $('#catePriceBody').find('tr').eq(0);
                    tr0.find('input').eq(0).attr('name', 'Prices[0].MinBath');
                    tr0.find('input').eq(1).attr('name', 'Prices[0].Price');
                }
                if ($('#catePriceBody').find('tr').length > 1) {
                    var tr1 = $('#catePriceBody').find('tr').eq(1);
                    tr1.find('input').eq(0).attr('name', 'Prices[1].MinBath');
                    tr1.find('input').eq(1).attr('name', 'Prices[1].Price');
                }
        ...
        $('.btn.step').click(function () {
            ...
            //alert($.isNullOrEmpty(categoryIdValue));
            if ($.isNullOrEmpty(categoryIdValue)) {
                //说明是编辑它可能之前设置的是二级分类它没有选择改分类值了，默认原来分类Id值
                categoryIdValue = $("#sourceCategoryId").val();
            }
            ...
            $.dialog({
                ...
                ok: function () {//确认时生成表格
                    ...
                    $(".j_Alias").each(function (i) {
                        var chkspes = $(this).parent().find("input:checked");//当前行规格是否有选中的
                        if (chkspes == null || chkspes.length <= 0)
                            return true;

                        if (typeof (spes[aliindex]) != "undefined") {
                            spes[aliindex].specification["Alias"] = $(this).val();
                        }
                        if (typeof (changeSpecs[aliindex]) != "undefined") {
                            changeSpecs[aliindex].specification["Alias"] = $(this).val();
                        }
                        if (spes[aliindex] != null && typeof (spes[aliindex]) != "undefined") {
                            if (spes[aliindex].specification["Name"] == "Color") {
                                $("#ColorAlias").val($(this).val());
                            } else if (spes[aliindex].specification["Name"] == "Size") {
                                $("#SizeAlias").val($(this).val());
                            } else if (spes[aliindex].specification["Name"] == "Version") {
    ...
    function skuPicChange() {
        ...
        var valueid = $(this).closest('tr').attr("id").split("_")[1]
        $("table.sku input[type='hidden'][value='" + valueid + "']").each(function () {
            var lastTd = $(this).closest("tr").find("td").last();
    ...
    function generateSKUTable(spes) {
        ...
        for (var i = 0; i < skus.length; i++) {
            ...
            if (oldTr.length > 0) {
                ...
                v5 = $('input[name$=".ShowPic"]', oldTr).val();
            }
            v4 = sourceV4 + '-' + (i + 1);
            ...
            tbodyTrs += ('<tr  {0}>{1}<td class="openladder ' + (isOpenLadder == true ? 'hidden' : '') + '"><input number="true" min="0.01" required name="SKUExs[{6}].SalePrice" value="{2}" onchange="SKUSalePriceChange(this)" class="valid form-control"/></td>' +
                ...
                '<td><input class="form-control" onblur="validSku()" name="SKUExs[{6}].Sku" value="{5}"/>'
                + '<input name="SKUExs[{6}].ShowPic" type="hidden" value="{7}"></td></tr>')
                .format(item.attrs.join(' '), item.tds.format(i), v1, v2, v3, v4, i, v5);
        }
...
function validVirtual() {
    ...
    if (effectiveType == 2) {
        var hour = $("#Hour").val();
        if (hour.length == 0 || parseInt(hour)<=0) {
