...
$(function () {
  ...
  $("#list").on("click", ".good-live", function () {
    var id = $(this).siblings('.thisId').val();
    var name = $(this).siblings('.thisName').val();
    var price = $(this).siblings('.thisPrice').val();
    var shopId = $(this).siblings('.thisShopId').val();
    $.dialog({
      title: '小程序直播链接',
      lock: true,
      id: 'StoresLink',
      ...
      padding: '0 40px',
      init: function () {
        $("#storesLink-form #link").val("pages/productdetail/productdetail?id=" + id);
        $("#storesLink-form #pname").val(name);
        $("#storesLink-form #spanprice").text(price);
        $("#storesLink-form #imgsrc").attr("src", "/Storage/Shop/" + shopId + "/Products/" + id + "/1_220.png");
      }
    });
  });
    ...
    columns:
      [[
        ...
        },
        {
          ...
          formatter: function (value, row, index) {
            var html = "";
            html = '<span class="btn-a"><input class="thisId" type="hidden" value="' + row.Id + '"/><input class="thisShopId" type="hidden" value="' + row.ShopId + '"/><input class="thisName" type="hidden" value="' + row.Name + '"/>';
            if (row.IsOpenLadder)
...
function initGrid(params) {
  ...
  if (window.location.href.toLowerCase().indexOf('tar=indraft') > 0) {
    ...
    if (!params) {
      params = {};
      params.saleStatus = _saleStatus = "3";
      _auditStatus = null;
    }
  }
  if (params) {
    ...
    var hidIsOpenH5 = $("#hidIsOpenH5").val();
    var isOpenSmallProg = $("#hidisOpenSmallProg").val();
  //商品表格
  $("#list").hiMallDatagrid({
    url: 'list',
    nowrap: false,
    ...
    columns:
      [[
        ...
        {
          field: "Name", title: '商品', align: 'left', width: 250,
          formatter: function (value, row, index) {
            var html = '';
            if (row.ProductType == 1) {
              html = '<img class="pull-right" src="/Images/virtual.png"/>';
            }
            html += '<img class="ml15" width="40" height="40" src="' + row.Image + '" /><a class="single-ellipsis w150 lh20" title="' + value + '" target="_blank" href="/product/detail/' + row.Id + '">' + value + '</a>';
        ...
        { field: "PublishTime", title: "发布时间", width: 90, align: "center" },
        {
          ...
          formatter: function (value, row, index) {
            html = '<span class="btn-a text-left inline-block"><input class="thisId" type="hidden" value="' + row.Id + '"/><input class="thisShopId" type="hidden" value="' + row.ShopId + '"/><input class="thisName" type="hidden" value="' + row.Name + '"/><input class="thisPrice" type="hidden" value="' + (row.Price == row.MaxPrice ? row.Price.toFixed(2) : row.Price.toFixed(2) + "~" + row.MaxPrice.toFixed(2)) + '"/>';
            if (row.IsOpenLadder)
              html += '<input name="hidIsOpenLadder" type="hidden" value="' + row.Id + '"/>';
            if (!row.IsLimitTimeBuy) {
              html += '<a class="good-check" href="edit/' + row.Id + '">编辑</a>';
            ...
            if (row.SaleState == 1) {
                ...
                if (hidIsOpenH5 == "True") {
                    html += '<div class="good-share"><i style="color:#337ab7">分享</i>';
                ...
                if (isOpenSmallProg == "True") {//开启小程序
                    html += '<a class="recommend" href="javascript:" data-product-id="{0}"></a>'.format(row.Id);
                }
            }
...
function InitialApplteProduct(pid) {
    $.dialog({
        title: option.title,
        lock: true,
        id: 'ImgCodeView',
        content: ['<div class="dialog-form">',
            '<img id="imgcode" class="img"/>',
            '</div>'].join(''),
        init: function () {
            $.ajax({
                type: 'GET',
                url: '/selleradmin/product/GetVideoNumberCode',
                cache: false,
                data: { id: option.id },
                dataType: "json",
                success: function (res) {
                    $("#imgcode").attr("src", res.data.videoNumberCode);
                }
            });
        },
        padding: '0 40px'
