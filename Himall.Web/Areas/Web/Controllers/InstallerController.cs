...
namespace Himall.Web.Areas.Web.Controllers
{
    public class InstallerController : Controller
    {
        ...
        public JsonResult ClearDemoData()
        {
            ...
            string filename = Request.MapPath("/SqlScripts/ClearDemoData.sql");
            string errorMsg = string.Empty;
            if (!System.IO.File.Exists(filename))
            {
        ...
        private bool CreateAdministrator(out string errorMsg)
        {
            ...
            try
            {
                using (connection = new MySqlConnection(GetConnectionString()))
                {
                    ...
                    long shopId = 1;
                    dbCmd.CommandText =
                        "SELECT Id FROM Himall_Shop limit 1";
                    ...
                    dbCmd.CommandText =
                        "INSERT INTO Himall_Manager  (shopId, RoleId, UserName, Password, PasswordSalt, CreateDate)" +
                    ...
                    dbCmd.CommandText = "SET FOREIGN_KEY_CHECKS=0;" +
                       "INSERT INTO Himall_Member  (id,UserName, Password, PasswordSalt,TopRegionId,RegionId,OrderNumber,Disabled,Points,Expenditure,CreateDate,LastLoginDate)" +
                    ...
                    dbCmd.Parameters.Clear();
                    dbCmd.CommandText =
                       "update Himall_SiteSetting set Value=@SiteName WHERE  `Key`='SiteName'";
        ...
        private bool UpdateSliderImage(out string errorMsg)
        {
            ...
            try
            {
                using (connection = new MySqlConnection(GetConnectionString()))
                {
                    ...
                    dbCmd.Parameters.Clear();
                    dbCmd.CommandText = "update Himall_ImageAd set ImageUrl=@ImageUrl WHERE  `Id`>1 and `Id`<8";
                    dbCmd.Parameters.Add(new MySqlParameter("@ImageUrl", "../Images/226x288.png"));
                    dbCmd.ExecuteScalar();
                    dbCmd.Parameters.Clear();
                    dbCmd.CommandText =
                       "update Himall_ImageAd set ImageUrl=@ImageUrl WHERE  `Id`=8";
                    dbCmd.Parameters.Add(new MySqlParameter("@ImageUrl", "../Images/464x288.png"));
                    dbCmd.ExecuteScalar();

                    dbCmd.Parameters.Clear();
                    dbCmd.CommandText =
                       "update Himall_ImageAd set ImageUrl=@ImageUrl WHERE  `Id`>8 and `Id`<13";
                    dbCmd.Parameters.Add(new MySqlParameter("@ImageUrl", "../Images/226x288.png"));
                    dbCmd.ExecuteScalar();

                    dbCmd.Parameters.Clear();
                    dbCmd.CommandText =
                       "update Himall_ImageAd set ImageUrl=@ImageUrl WHERE  `Id`=1";
                    dbCmd.Parameters.Add(new MySqlParameter("@ImageUrl", "../Images/464x288.png"));
                    dbCmd.ExecuteScalar();

                    dbCmd.Parameters.Clear();
                    dbCmd.CommandText =
                       "update Himall_ImageAd set ImageUrl=@ImageUrl WHERE  `Id`=13";
