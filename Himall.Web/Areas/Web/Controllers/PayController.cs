...
namespace Himall.Web.Areas.Web.Controllers
{
    public class PayController : BaseWebController
    {
        ...
        public PayController(OrderService OrderService, CashDepositsService CashDepositsService, MemberCapitalService MemberCapitalService
            , RefundService RefundService, ShopService ShopService, FightGroupService FightGroupService)
        {
        ...
        public ActionResult Index(string pmtid, string ids)
        {
            ...
            //获取待支付的所有订单
            var orders = _OrderService.GetOrders(orderIdArr).Where(item => item.OrderStatus == Entities.OrderInfo.OrderOperateStatus.WaitPay && item.UserId == CurrentUser.Id).ToList();
            ...
            //保存支付订单
            long payid = _OrderService.SaveOrderPayInfo(orderPayModel, PlatformType.PC);
            #endregion
        ...
        {
            var orderitems = _OrderService.GetOrderItemsByOrderId(orders.Select(p => p.Id));
        ...
        public ContentResult RefundNotify_Post(string id)
        {
            ...
            try
            {
                ...
                if (payInfo != null)
                {
                    ...
                    try
                    {
                        _RefundService.NotifyRefund(refund_batch_no);
                        response = payment.Biz.ConfirmPayResult();
                    }
                    catch (Exception e)
                    {
        ...
        public ContentResult EnterpriseNotify_Post(string id, string outid)
        {
            ...
            string errorMsg = string.Empty;
            string response = string.Empty;
            var _iOperationLogService = ObjectContainer.Current.Resolve<OperationLogService>();
        ...
        public ContentResult ReNewPayNotify_Post(string id, string str)
        {
            ...
            try
            {
                ...
                if (!result)
                {
                    ...
                    //    //店铺续费
                    //    _ShopService.ShopReNew(shopId, value);
                    //}
                    ////升级操作
                    //else
                    //{
                    ...
                    //    _ShopService.AddShopRenewRecord(model);
                    //    //店铺升级
                    //    _ShopService.ShopUpGrade(shopId, (long)value);
                    //}
                    #endregion
                    response = payment.Biz.ConfirmPayResult();
