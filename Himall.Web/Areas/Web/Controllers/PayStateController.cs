            ...
            Task.Factory.StartNew(() =>
               {
                   ...
                   while (true)
                   {
                       ...
                       if (!payStateObj)
                       {
                           ...
                           var service = ObjectContainer.Current.Resolve<OrderService>();
            ...
            Task.Factory.StartNew(() =>
            {
                ...
                while (true)
                {
                    ...
                    if (!payStateObj)
                    {
                        //没有进入缓存
                        //检查对应订单是否已经支付
                        var service = ObjectContainer.Current.Resolve<MemberCapitalService>();

                        var model = service.GetChargeDetail(long.Parse(orderIds));
                        var payDone = model != null && model.ChargeStatus == Himall.Entities.ChargeDetailInfo.ChargeDetailStatus.ChargeSuccess;
                        Cache.Insert(payStateKey, payDone, 15);//标记支付状态
                    }
            ...
            Task.Factory.StartNew(() =>
            {
                ...
                while (true)
                {
                    ...
                    {//没有进入缓存
                        ...
                        //    using (var service = ObjectContainer.Current.Resolve<ShopService>())
                        //    {
