...
namespace Himall.Web.Areas.Web.Controllers
{
    ...
    public class UserOrderController : BaseMemberController
    {
        ...
        public UserOrderController(
            ...
            CashDepositsService CashDepositsService,
            ...
            FightGroupService FightGroupService, TypeService TypeService
            )
        {
        ...
        public ActionResult Index(string orderDate, string keywords, string orderids, DateTime? startDateTime, DateTime? endDateTime, int? orderStatus, int pageNo = 1, int pageSize = 10)
        {
            ...
            {
                ViewBag.Grant = _ShopBonusService.GetByOrderId(long.Parse(orderids));
            }
            ...
            var shopBonus = _ShopBonusService;
            ...
            var orderList = orders.Models.Select(item => new OrderListModel
            {
                ...
                OrderItemList = orderItems.Where(oi => oi.OrderId == item.Id).Select(oItem =>
                {
                    ...
                    var orderItem = new OrderItemListModel
                    {
                        ...
                        CashDepositsObligation = cashDepositsService.GetProductEnsure(oItem.ProductId),
                    };
            ...
            if (ordidl.Count > 0)
            {
                foreach (var item in orderList)
                {
                    if (item.OrderType != OrderInfo.OrderTypes.Virtual)
                    {
                        ...
                        {
                            var fgord = _FightGroupService.GetFightGroupOrderStatusByOrderId(item.Id);
                            if (fgord != null)
                            {
        ...
        private bool HasAppendComment(OrderListModel list)
        {
            ...
            var result = false;
            if (item != null)
                result = _CommentService.HasAppendComment(item.Id);
            return result;
        }
        ...
        {
            var model = _CommentService.GetProductEvaluationByOrderIdNew(orderId, userId).FirstOrDefault();
            if (model == null)
            {
        ...
        {
            var model = CustomerServiceApplication.GetAfterSaleByShopId(shopId);
            return PartialView(model);
        }

        public ActionResult Detail(long id)
        {
            var order = _OrderService.GetOrder(id, CurrentUser.Id);//限制到用户
            ...
            ViewBag.Coupon = 0;
            var coupon = _CouponService.GetCouponRecordInfo(order.UserId, order.Id);
            var bonus = _ShopBonusService.GetUsedPrice(order.Id, order.UserId);
            if (coupon != null)
            {
            ...
            orderModel.OrderItemList = AutoMapper.Mapper.Map<IEnumerable<OrderItemInfo>, IEnumerable<OrderItemListModel>>(orderItems);
        ...
        public JsonResult GetExpressData(string expressCompanyName, string shipOrderNumber)
        {
            ...
            string kuaidi100Code = ObjectContainer.Current.Resolve<ExpressService>().GetExpress(expressCompanyName).Kuaidi100Code;
        ...
        public JsonResult CloseOrder(long orderId)
        {
            ...
            if (order != null)
            {
                _OrderService.MemberCloseOrder(orderId, CurrentUser.UserName);
            }
            else
            {
        ...
        public JsonResult ConfirmOrder(long orderId)
        {
            ...
            // var data = _OrderService.GetOrder(orderId);
            //确认收货写入结算表
