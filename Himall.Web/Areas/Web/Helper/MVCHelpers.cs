        ...
        public static MvcHtmlString CategoryPath(string path, string pName)
        {
            ...
            try
            {
                ...
                if (catepath.Length > 0)
                    mian = ObjectContainer.Current.Resolve<CategoryService>().GetCategory(long.Parse(catepath[0])).Name;
                if (catepath.Length > 1)
                    second = ObjectContainer.Current.Resolve<CategoryService>().GetCategory(long.Parse(catepath[1])).Name;
                if (catepath.Length > 2)
                    third = ObjectContainer.Current.Resolve<CategoryService>().GetCategory(long.Parse(catepath[2])).Name;
