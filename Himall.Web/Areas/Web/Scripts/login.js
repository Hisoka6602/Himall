        ...
        $.post('/Login/Login', { username: username, password: password, checkCode: checkCode, keep: keep }, function (data) {
            ...
            if (data.success) {//登录成功
                ...
                if (IsOpenPC == 0) {
                    ...
                    var returnUrl = DecodeUrl(QueryString('returnUrl'));
                    ...
                    if (returnUrl) {
                        ...
                        if (reUrl != siteUrl && reUrl!="") {
                            returnUrl = location.href.indexOf("https://") != -1 ? "https://" : "http://" + siteUrl + "/";
                        }
                        try {
...
function DecodeUrl(url) {
    var urlstr = ["/userCenter/home", "/SellerAdmin", "/OrderComplaint"];
    var linkurl = decodeURIComponent(url).replace('&amp;', '&');
    var caseresult = Base64.decode(linkurl);
    if (urlstr.indexOf(caseresult)>=0) {//存在指定目录
        linkurl = caseresult;
    }
    return linkurl;
}
