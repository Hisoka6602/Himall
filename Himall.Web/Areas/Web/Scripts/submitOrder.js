...
$(function () {
    ...
    //重新设定优惠券选项,同一张优惠券只能使用一个订单payPriceId
    function CouponChange(shopid, id) {
        $('.shopb').each(function (i, e) {
            var _t = $(this);
            var sid = _t.data("shopid");
            if (shopid != sid) {
                $(e).children().each(function (l, k) {
                    $(k).removeAttr('disabled');
                    if ($(k).attr('value') == id && id > 0) {
    ...
    function changePlatConditionReload(shopid, type, p) {
        var couponIds = "";

        couponIds = p + '_' + type + '_' + 0;

        var url = location.href;
        if (url.toLowerCase().indexOf("platcouponids") < 0) {
            var splitText = "?";
        ...
        url = url.replace(/([\?&]platcouponids=)[^\?&]*/, "$1" + couponIds);
        SaveVirtualContentToStorage();
        location.href = url;
    }
    ...
    function changeConditionReload() {
        ...
        $('.shopb').each(function (i, e) {
            ...
            var shopid = _s.data("shopid");
            var type = _s.data("type");
            var id = _s.val();
            couponIds += id + '_' + type + '_' + shopid + ",";
        })
        if (couponIds != '') {
            couponIds = couponIds.substring(0, couponIds.length - 1);
        }
        var url = location.href;
        if (url.toLowerCase().indexOf("shopcouponids") < 0) {
            var splitText = "?";
            var _tmparr = url.split(splitText);
            console.log(_tmparr);
            url = _tmparr[0];
        ...
        url = url.replace(/([\?&]shopcouponids=)[^\?&]*/, "$1" + couponIds);
        SaveVirtualContentToStorage();
        location.href = url;
    }

    //平台优惠券切换绑定
    $('.platshopb').bind('change', function (t) {
        var _s = $(".platshopb").find("option:selected");
        var shopid = _s.data("shopid");
        var type = _s.data("type");
        var id = _s.val();
        changePlatConditionReload(shopid, type, id);
    ...
    $('input.express').change(function (e) {
        if (this.checked) {
            ...
            var freight = d_freight.data('expreefreight');
            var shopd = d_box.find('.shopd');

            var shopprice = parseFloat(shopd.attr('data')).toAdd(MoneyRound(parseFloat(freight)));
            shopd.attr("data", parseFloat(shopprice).toFixed(2));
            ComputeOrder(shopId);
            if (e && shopId == 1)
    ...
    if (productType == 1) {
        ...
        //$("#VProductItemImages1").himallUpload({
                            ...
                            callback: function () {
                                ...
                                $.post('ValidPayPwd', { pwd: pwd }, function (result) {
                                    if (result.success) {
                                        ...
                                        $("#IsUsedCapital").prop("checked", false);
                                        return false;
                                    }
                                });
                            },
        ...
        if (result.success == true) {
            for (var i = 0; i < result.freights.length; i++) {
                ...
                if (freeFreight <= 0 || amount < freeFreight) {
                    //alert("MoneyRound(amount + freight):" + MoneyRound(amount + freight));
                    $('.shopd', shopDiv).attr('data', MoneyRound(amount.toAdd(freight))).html('￥' + MoneyRound(parseFloat(amount.toAdd(freight))));
...
function CalInvoiceRate(orderTotalIntegral) {
    ...
    $('.invoicerate').each(function (i, e) {
        ...
        if (_rate > 0) {
            var shopid = $(this).attr('shopid');
            var d_box = $("#orderdata_" + shopid);
            var total = parseFloatOrZero(d_box.find('.shopd').data('nofreight'));
            var freight = parseFloatOrZero(d_box.find('.shopf').attr('data'));
            if (freight > 0 && d_box.find('.shopf').is(":visible")) {
                total = total.toAdd(freight);
            }
            if (orderTotalIntegral > 0) {
                var productTotal = 0;//商品总价（商品价-满额减）
                $('.shopd').each(function () {
                    productTotal = parseFloatOrZero(productTotal).toAdd(parseFloatOrZero($(this).attr('data')));
                });
                var cTotal = 0;
                //$('.shopc').each(function () {
                //    cTotal = parseFloatOrZero(cTotal) + parseFloatOrZero($(this).attr('data'));
                //});
                productTotal = parseFloat(productTotal.toSub(cTotal));
                var integralAmount = parseFloat(orderTotalIntegral * (total / productTotal) * 100) / 100;
...
function freeFreight(shopId) {
    ...
    var shopprice = MoneyRound(parseFloat(shopd.attr('data')).toSub(MoneyRound(parseFloat(freight))));
    ...
    //alert("MoneyRound(shopprice - ordSubDisPrice):" + MoneyRound(shopprice - ordSubDisPrice) + "  ordSubDisPrice:" + ordSubDisPrice);
    shopd.html('￥' + MoneyRound(shopprice.toSub(ordSubDisPrice)));
    shopd.attr("data", shopprice);
...
function InvoiceInit() {
    ...
    $(".btnOk").click(function () {
        ...
        var sptxt = $form.find('.invoice-item-selected .invoice-item-span').html();
...
function initVirtualContent() {
    if ($("div.virtual").length == 0) {
        return false;
    }

    virtualContents = JSON.parse(window.localStorage.getItem("virtualContents"));
    if (virtualContents && virtualContents.length > 0) {
        $(virtualContents).each(function (index, item) {
            var obj = $("div.virtual").eq(index);
            if (item.Content && item.Content.length > 0) {
                if (item.Type == 6) {
                    var imgArr = item.Content.split(",");
                    $(obj).find(".pic-upload").himallUpload({
                        displayImgSrc: item.Content,
                        imgFieldName: "VProductItemImages_" + index,
                        imagesCount: 3,
                        maxSize: 3,
                        canDel: true,
                        isMobile: false
                    ...
                    $(obj).find("input[type=text]").val(item.Content);
                }
            }
...
///刷新页面时将自定义内容保存到缓存
function SaveVirtualContentToStorage() {
    if ($("div.virtual").length == 0) {
        return;
    }
    //虚拟商品用户信息项
    ...
    $("div.virtual").each(function () {
        var obj = new Object();
        obj.Name = $(this).find(".j_name").html();
        ...
        virtualContents.push(obj);
    });
    window.localStorage.setItem("virtualContents", JSON.stringify(virtualContents));
}
...
function bindSubmit() {
    $('#submit').click(function () {
        if (!isSubmitLoading) {
            ...
            var platcoupon = function () {
                var arr = [];
                $('.platshopb').each(function (i, e) {
                    var shopid = $(e).attr('data-shopid');
                    $(e).children().each(function (l, k) {
                        if ($(k).attr('selected')) {
                            var b = parseInt($(k).val());
            ...
            integral = isNaN(integral) ? 0 : integral < 0 ? 0 : integral;
            //预存款文本框值
            ...
            if (!recieveAddressId && productType == 0) {
                ...
                var couponIds = fn();
                //选中的平台优惠券
                var platcouponIds = platcoupon();
                var choicecoupons = [];
                for (var i = 0; i < couponIds.length; i++) {
                    choicecoupons.push(couponIds[i]);
                }
                for (var i = 0; i < platcouponIds.length; i++) {
                    choicecoupons.push(platcouponIds[i]);
                }
                model.Choices = choicecoupons;
                ...
                $('.order-table[shopid]').each(function () {
                    ...
                    $('tbody tr', this).each(function () {
                        ...
                        productItem.ProductId = $(this).attr('productid');
                        productItem.SkuId = $(this).attr('skuid');
                        productItem.Price = $(this).attr('price');
                        productItem.Quantity = $(this).attr('quntity');
                        ...
                        $("div.virtual").each(function () {
                            ...
                            productItem.VirtualContents.push(obj);
                        });
                    ...
                    orderShop.ShopBranchId = selectShopBranch[shopId];
                    if (productType == 0) {
                        if (orderShop.ShopBranchId == null && checkbox.hasClass('express') == false) {
                            $.dialog.tips('请选择自提门店');
                            hasError = true;
                            return false;
                        }
                    ...
                    orderShop.IsCashOnDelivery = isCashOnDelivery;
                ...
                var tax = CalInvoiceRate(parseFloat($("#integralPrice").text().replace("-￥", "")));
                var total = parseFloat($("#total").val());
                var integralAmount = 0;
                var enabledIntegral = $('#IsUsedIntegral').is(':checked');
                if (enabledIntegral) {
                    integralAmount = parseFloat($("#integralPrice").text().replace("-￥", ""));
                    if (isNaN(integralAmount)) { integralAmount = 0; }
                }
                var producttotal = parseFloat(CaclProductTotal());
                ...
                //alert("capital:" + capital + ";integral:" + integral + ";platcouponIds:" + platcouponIds);
                //return;
                $.post('/order/NewSubmit', model,
                    function (result) {
                        var orderIds = "";
                        if (isCashOnDelivery && $("#onlyshop1").val() == "True") {
                            ...
                            if (orderIds != null) {
                                location.replace('/order/pay?orderIds=' + orderIds.toString());
                                loading.close();
                                //isSubmitLoading = false;
                            }
                            else {
                                ...
                                function checkOrderState() {
                                    $.getJSON('/OrderState/Check', { Id: result.Id }, function (r) {
                                        if (r.state == "Processed") {
                                            location.href = '/order/pay?orderIds=' + orderIds.toString();
                                        }
...
function CaclProductTotal() {
    var total = 0;
    //商品总额
    $(".shopd").each(function (l, k) {
        var _t = $(k);
        total = total.toAdd(parseFloatOrZero(_t.data("nofreight")));
    });
    var freight = 0;
    //运费
    $(".shopf").each(function (l, k) {
        var f = parseFloat($(this).text().replace("￥", ""));
        if (!isNaN(f)) {
            freight = freight.toAdd(f);
        }
    });
    return MoneyRound(total.toAdd(freight));
}
...
function ShowPaidPrice() {
    ...
    var orderTotalDisPrice = 0;       //订单商家优惠总价
    var orderTotalFreight = 0;        //订单运费总价
    var orderTotalIntegral = 0;       //订单积分抵扣
    //var orderTotalCapital = 0;        //订单使用预存款
    var orderPlatDisPrice = 0;          //订单平台券优惠
    var fullDiscountPrice = 0;//满额减
    //运费总价
    $(".shopf").each(function (l, k) {
        var _t = $(k);
        orderTotalFreight = orderTotalFreight.toAdd(parseFloatOrZero(_t.attr("data")));
    });
    //总优惠
    $(".shopc").each(function (l, k) {
        var _t = $(k);
        orderTotalDisPrice = parseFloatOrZero(orderTotalDisPrice).toAdd(parseFloatOrZero(_t.attr("data")));
    });

    //平台优惠券
    $(".platshopc").each(function (l, k) {
        var _t = $(k);
        orderPlatDisPrice = parseFloatOrZero(orderPlatDisPrice).toAdd(parseFloatOrZero(_t.attr("data")));
    });
    //满额减优惠
    $(".fulldiscount").each(function (l, k) {
        var _t = $(k);
        fullDiscountPrice += parseFloatOrZero(_t.data("price"));
    });
    ...
    if (intRule > 0) {
        ...
        var useintegral = parseInt(d_integral.data("userintegral"));
        if (isNaN(useintegral)) {
            useintegral = 0;
        }
        var v = d_integral.val(), max = d_integral.data("maxmoney"), rate = d_integral.data("maxuserate");
        var maxpoint = Math.floor(max * intRule);

        if (maxpoint > useintegral) { maxpoint = useintegral; }
        //可抵扣金额
        var max = Math.floor((maxpoint / intRule) * Math.pow(10, 2)) / Math.pow(10, 2);
        //var _dm = MoneyRound(v / rate);
        var _dm = v / intRule;
        _dm = Math.floor(_dm * Math.pow(10, 2)) / Math.pow(10, 2);
        if (_dm > max) {
            _dm = max;
            d_integral.val(maxpoint);
        }
        orderTotalIntegral = _dm;
    ...
    //计算实付(预存款前)
    //orderPaidPrice = (orderTotalPrice - fullDiscountPrice - orderTotalIntegral - orderTotalDisPrice - orderPlatDisPrice + orderTotalFreight);
    orderPaidPrice = (orderTotalPrice.toSub(parseFloat(fullDiscountPrice)).toSub(parseFloat(orderTotalIntegral)).toSub(parseFloat(orderTotalDisPrice)).toSub(parseFloat(orderPlatDisPrice)).toAdd(orderTotalFreight));

    if (orderPaidPrice < 0) {
        orderPaidPrice = 0;
    }
    orderPaidPrice = parseFloatOrZero(orderPaidPrice).toAdd(invoiceAmount);
    ...
    //重新计算实付(预存款之后)
    orderPaidPrice = parseFloatOrZero(orderPaidPrice).toSub(parseFloatOrZero(useCapital));
    ...
    //orderPaidPrice - orderTotalFreight + useCapital > 0
    var pointToPrice = orderPaidPrice.toSub(parseFloatOrZero(orderTotalFreight)).toAdd(parseFloatOrZero(useCapital));
    if ((pointToPrice > 0) && mpirule > 0) {
        d_ordMPI.text(Math.floor(pointToPrice.toDiv(mpirule)));
    }
    else {
        d_ordMPI.text(0);
    }
...
function HidepOption() {
    if (parseFloat($("#payPriceId").attr("v")) <= 0) {
        $("#showIntegral").hide();
        $("#showCapital").hide();
    } else {
        $("#showIntegral").show();
...
function ComputeOrder(shopid) {
    if (shopid == 0) {
        var d_platdissel = $(".platshopb");
        var d_platshowdis = $(".platshopc");
        var ordSubPlatDisPrice = 0;
        if (d_platdissel.length > 0) {
            var d_selopt = d_platdissel.find("option:selected");
            ordSubPlatDisPrice = parseFloatOrZero(d_selopt.data("p"));

            if (ordSubPlatDisPrice > ordSumProPrice) {
                ordSubPlatDisPrice = ordSumProPrice;
            }
        }
        d_platshowdis.html("-￥" + ordSubPlatDisPrice).attr("data", ordSubPlatDisPrice);

        if (parseFloat(ordSubPlatDisPrice) == 0) {
            d_platshowdis.html("-￥0").attr("data", 0);
        }
        ...
        var d_platdissel = $(".platshopb");
        var d_platshowdis = $(".platshopc");
        var ordSumProPrice = 0, ordSubDisPrice = 0; ordSubFreight = 0, shopFreeFrei = 0, ordSubPlatDisPrice = 0;
        ...
        //计算实付
        var ordPaidPrice = ordSumProPrice + ordSubFreight;
