$(function () {
    ...
    HiShop.DIY.Unit.getData = function () {
        ...
        _.each(tmp.PModules, function (item) {
            ...
            if (PlatFormType != 5) {
                item.dom_conitem = HiShop.Base64.encode(HiShop.Base64.utf16to8(image_replacesrc(_.template(head, item)) + HiShop.DIY.Unit.HStyleAddJs(item.content.bg)));
            }
            item.dom_ctrl = null;
            item.ue = null;
        });
        tmp.title = $("#templateName").length > 0 ? $("#templateName").val() : "";
