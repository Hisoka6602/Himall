!function ($) {
    ...
    , setValue: function (item) {
        return { "data-value": item.toString(), "data-value2": item.toString(), "real-value": item.toString() };
    }

    , select: function () {
        var val = this.$menu.find('.active').attr('data-value');
        var val2 = this.$menu.find('.active').attr('data-value2');
        var realVal = this.$menu.find('.active').attr('real-value');
        //console.log("val:" + val + "--val2:" + val2 + "--realVal:" + realVal);
        var setsecondId = this.$element.attr("setsecondId");//第二个值显示
        if (val2 != null && val2 != "" && $("#" + setsecondId).length > 0) {
            $("#" + setsecondId).val(val2);
        }
        this.$element
