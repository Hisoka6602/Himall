...
$.productSelector = {
    ...
    reload: function (selectedProductIds, exceptProductIds) {
        ...
        if (this.serviceType == 'admin')
            ...
            //}
            if (location.href.toLocaleLowerCase().indexOf("selleradmin/pagesettings/") != -1) {
                $.productSelector.params.productType = -1;//店铺页面设置页要能选择虚拟商品
            }
        }
        ...
        if (selectedProductIds && selectedProductIds.length > 0) {
            $.post(url, { Ids: selectedProductIds.toString(), page: 1, rows: selectedProductIds.length, isFilterPromotion: this.params.isFilterPromotion, isLimitTimeBuy: this.params.isLimitTimeBuy}, function (products) {
