...
namespace Himall.Web.Wdgj_Api
{
    ...
    public class WdgjApiHandler : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            //}
            ////根据uCode获取效验码
            //var usign = Himall.ServiceProvider.Instance<ShopService>.Create.GetshopInfoByCode(wdgj.uCode).uSign;
        ...
        public string findProduct(HttpContext context)
        {
            ...
            query.ShopId = 1;
            if (ObjectContainer.Current.Resolve<ShopService>().GetshopInfoByCode(uCode) != null)
                query.ShopId = ObjectContainer.Current.Resolve<ShopService>().GetshopInfoByCode(uCode).ShopId; //根据uCode获取店铺信息
        ...
        public string GetOrderList(HttpContext context)
        {
            ...
            //根据uCode获取店铺信息
            if (ObjectContainer.Current.Resolve<ShopService>().GetshopInfoByCode(uCode) != null)
                query.ShopId = ObjectContainer.Current.Resolve<ShopService>().GetshopInfoByCode(uCode).ShopId;

            var orders = ObjectContainer.Current.Resolve<OrderService>().GetOrders(query);
        ...
        public string GetOrderDetail(HttpContext context)
        {
            ...
            var order = ObjectContainer.Current.Resolve<OrderService>().GetOrder(OrderId);
            if (order == null)
            {
                sb.Append("<Result>0</Result>");
            ...
            else
            {
                ...
                var orderitem = ObjectContainer.Current.Resolve<OrderService>().GetOrderItemsByOrderId(order.Id);
